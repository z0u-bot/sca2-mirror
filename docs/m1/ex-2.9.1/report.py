# title: Experiment 2.9.1 redux: deleting red, now in JAX
...
def schedule_figure() -> plt.Figure:
    sheet = Dopesheet.from_csv(DOPESHEET)
    history = realize_timeline(Timeline(sheet))
    fig, _ = plot_timeline(
        history,
        sheet.as_df(),
        groups=[
...
def scores_figure(metrics: list[dict], best: dict) -> plt.Figure:
...
def error_vs_similarity_figure(best_eval: dict[str, np.ndarray], r2: float) -> plt.Figure:
...
error_vs_similarity_figure(best_eval, best["score"])

# %%
mse_base, mse_abl = best_eval["mse_base"], best_eval["mse_abl"]
sim = best_eval["sim3"]
reds, others = sim > 0.5, sim < 0.01

rf"""
Baseline reconstruction is near-perfect (mean MSE {mse_base.mean():.2e}). After ablation, red-like colors (similarity³ > 0.5) have a mean error of {mse_abl[reds].mean():.3f}, while colors unlike red (similarity³ < 0.01, {int(others.sum())} of {len(sim)} grid points) sit at {mse_abl[others].mean():.2e}: deleting red barely affects them.
"""
...
def latents_figure(best_eval: dict[str, np.ndarray]) -> plt.Figure:
