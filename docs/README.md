...
A literate document is a plain Python file where a top-level string literal is prose (a plain string is a Jinja template over the cells' namespace; an f-string is evaluated as Python, so ruff, ty, and vulture see the names it reads) and the code between prose strings is a cell, with `# title:` (and `# code: show`, for a page about the code; a report hides its cells) as comment lines at the top; ruff, ty, and the IDE see every cell. The same document can be spelled as Markdown with ```` ```{python} ```` fences. `./go lit render docs/m2/ex-2.1.12/report_lit.py` (ex-2.1.12 carried over as a trial, beside its notebook) weaves it to `.mini/lit/<key>/` (HTML, Markdown, and with `--pdf` a print through headless Chromium: a `chromium` on `$PATH`, `sudo apt-get install chromium` in the devcontainer, or `$CHROMIUM` pointing at one); `./go lit serve` re-weaves on save with live reload. See `mini.lit` for the design, and `todo/eng/literate-reports.md` for the trial and what a switch would need. A literate `.py` has no `marimo.App(`, so the site build ignores it as it does any other plain module, and its `# title:` header is how the publish check tells it from a report's inputs (`mini.lit.is_literate_document`); a literate `.md` is skipped explicitly.
