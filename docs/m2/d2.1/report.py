# title: D2.1: figures for the anchoring post
...
# The document's directory is on sys.path while it runs, so the condition list and the
...
alphas: dict[str, dict[str, np.ndarray]] = loaded[0]
alpha_stacks: dict[str, dict[str, np.ndarray]] = loaded[1]
probe_pairs: np.ndarray = loaded[2]
...
ys = {c.key: alphas[c.key]["op1"][:, :, 0].mean(axis=0) for c in ex.CONDITIONS}
envs = {c.key: alpha_stacks[c.key]["op1"][:, :, :, 0].reshape(-1, len(GRID_RGB)) for c in ex.CONDITIONS}
# Wide enough for the envelope's full range, since the clouds draw unclipped.
ylim = (-0.4, 1.05)


@memo
def grading_progression(ys: dict[str, np.ndarray], envs: dict[str, np.ndarray], ylim: tuple[float, float]) -> str:
    """The recipe-progression figure, as figure HTML. Memoized on the arrays, so a re-render after a prose edit skips the draw."""
    ...
    def plot() -> plt.Figure:
        ...
        axes_row = cast(AxesRow, axes)
        for ax, cond in zip(axes_row, ex.CONDITIONS, strict=True):
            GradingCloud(ax, envs[cond.key], ylim, k=20, color=light_dark("#0003", "#fff2"), clip_on=False, zorder=-1, dpr=1)  # fmt: skip
            GradingCloud(ax, ys[cond.key], ylim, k=35, dpr=2, clip_on=False)
            ax.set_title(cond.title, fontsize=9)
            ax.set_xlabel("redness of op1", fontsize=8)
            ax.tick_params(labelsize=7)
            ax.set_aspect("equal")
        axes_row[0].set_xticks([0, 0.5, 1])
        axes_row[0].set_yticks([0, 0.5, 1])
        axes_row[0].set_ylim(0, 1)
        axes_row[0].set_ylabel("α at op1\nact · (1, 0, …)", fontsize=8)
        return fig

    return plot()


grading_progression(ys, envs, ylim)

# %%

bare = alphas["ex-2.1.6/lam0.1"]["op1"][:, :, 0].mean(axis=0)
primary = alphas[ex.PRIMARY]["op1"][:, :, 0].mean(axis=0)

rf"""
The bare anchor aligns everything at once: the response rises for every color, and even the least red colors sit at α ≈ {bare.min():.2f}. Adding the anti-subspace term is what makes the alignment graded, pulling non-red colors back to zero while pure red stays anchored. That grading survives the move to labels that no longer point at a particular token: in the primary condition, the slice-mean response at op1 tracks the sim¹·⁵ affinity shape with r² ≈ {r2_sim(primary):.2f}.
...
partner_red = np.bincount(probe_pairs[:, 0], REDNESS[probe_pairs[:, 1]]) / np.bincount(probe_pairs[:, 0])
...
@memo
def grading_grid_figure(cond: ex.Condition, panels: dict[str, np.ndarray], alt_text: str) -> str:
    """One condition's pair of grading grids, as figure HTML. Memoized on the arrays and the text, so a re-render after a prose edit skips the draw."""
    ...
    def plot() -> plt.Figure:
        return grading_grids(
            {f"Keyed by the {op} operand": (panels[k], POS_NAMES.index(k)) for k, op in KEYINGS.items()}
        )

    return plot()


"\n\n".join(f"### {c.title}\n\n{grading_grid_figure(c, alphas[c.key], GRID_ALT[c.key])}" for c in ex.CONDITIONS)

# %%

# Position slots, as POS_NAMES orders them: op1 + op2 = ans \n.
OP1, PLUS, OP2, EQ, ANS, NL = range(6)
# Mid-stack (slices 1–3) α on the lines whose keyed operand is pure red.
red = lambda key, k, t: float(alphas[key][k][1:4, I_RED, t].mean())  # noqa: E731
r2 = lambda key, k, t: r2_sim(alphas[key][k][:, :, t].mean(axis=0))  # noqa: E731
# Drift: the mean over *all* colors at the last slice, not the pure-red row.
bulk = lambda key, k, t: float(alphas[key][k][4, :, t].mean())  # noqa: E731
flat, bare_key, anti = "ex-2.1.6/lam0", "ex-2.1.6/lam0.1", "ex-2.1.8/end90-hold30"

rf"""
Read across the eight panels and the recipe narrows *where* the pull lands, in three steps.

Un-anchored there is nothing to place. No slot in either keying leaves the noise, and the slice-mean response at op1 scores r² = {r2(flat, "op1", OP1):.2f} against the sim¹·⁵ shape.

**The bare anchor lifts the whole sequence.** With a pure-red first operand, mid-stack α runs {red(bare_key, "op1", OP1):.2f} at op1, {red(bare_key, "op1", PLUS):.2f} at the `+` slot, {red(bare_key, "op1", OP2):.2f} at op2 and {red(bare_key, "op1", EQ):.2f} at the `=` slot — and it does not stop at the anchored span, reaching {red(bare_key, "op1", ANS):.2f} at the answer and {red(bare_key, "op1", NL):.2f} at the newline. Both keyings show the same indiscriminate lift.

**The anti-subspace term confines it to the span.** Outside the span the response falls away ({red(anti, "op1", ANS):.2f} at the answer, {red(anti, "op1", NL):.2f} at the newline), and the drift over all colors at the last slice drops from {bulk(bare_key, "op1", OP1):.2f} to {bulk(anti, "op1", OP1):.2f} — the containment ex-2.1.8 was tuned for. Inside the span the pull is still shared: a pure-red op1 brings the op2 slot to {red(anti, "op1", OP2):.2f} and the `=` slot to {red(anti, "op1", EQ):.2f}, and the op2 slot grades against *op1's* redness at r² = {r2(anti, "op1", OP2):.2f}. That is what op1-keyed labels buy: a red token pulls the span it sits in, without the span having to say which token it was.

**Pooled labels narrow it to the token.** On the primary a pure-red op1 leaves the rest of the span between {min(red(ex.PRIMARY, "op1", t) for t in (PLUS, OP2, EQ)):.2f} and {max(red(ex.PRIMARY, "op1", t) for t in (PLUS, OP2, EQ)):.2f}, and a pure-red op2 leaves the op1 slot at {red(ex.PRIMARY, "op2", OP1):.2f}. Each operand slot answers to its own color — r² = {r2(ex.PRIMARY, "op1", OP1):.2f} at op1 and {r2(ex.PRIMARY, "op2", OP2):.2f} at op2, reaching α ≈ {red(ex.PRIMARY, "op1", OP1):.2f} and {red(ex.PRIMARY, "op2", OP2):.2f} at pure red. So the pull finds the red token in either slot, grades it the same way there, and leaves its neighbours alone, which is the claim the pooled labels were meant to buy.

Two asymmetries survive on the primary. Op2 carries a little more of the drift at the last slice ({bulk(ex.PRIMARY, "op2", OP2):.2f} against {bulk(ex.PRIMARY, "op1", OP1):.2f} at op1), matching ex-2.1.10's reading that op1 hosts less of it once half the pull budget goes to op2. The answer slot barely moves between the two keyings (r² {r2(ex.PRIMARY, "op2", ANS):.2f} against {r2(ex.PRIMARY, "op1", ANS):.2f}): the answer is the mix, so its redness is a function of both operands and neither keying has much claim on it.
"""
