...
def _():
    ...
    Here we carry the term into the transformer, keeping the D2.1 grammar and recipe, before the operation work in the [D2.2 plan](../d2.2/design.md) changes the grammar. Two things are new. First, the readout is no longer a single linear decoder; it is every block after the redirect. So the term trains the blocks, and a stop-gradient keeps it from touching the placement. Second, the concept is continuous, so we have to choose the designed target. The operand-averaged null for *red* is uniform over 27 colors and has no mode, so we take its center: the visible operand mixed with mid-gray.

    The third risk in the plan is that the response to suppression is undesigned. Ex-2.2.1 measured that response, and this experiment either pins it or does not. One limitation carries over from M1: the response is trained at the antipode, while the removal we deploy projects to zero. H5 reads how far the designed response carries between the two, so a limitation of this size shows up as a curve rather than a flat miss.
    """)
    return
...
def _():
    ...
    - **response** — the probability the model puts on the correct answer, read from the log-softmax at the `=` position. **Damage** is the clean response minus the intervened response, per line.
...
def _():
    ...
    There are three conditions. Two of them are ex-2.1.10 checkpoints already in the store, re-scored here through the same code path as the new runs. The third is trained here.
    ...
    - **`redirect`** — γ = 2 at the embedding, at both operand positions. This is the state the fallback was trained at, now applied to every line. A non-red operand has nothing on the axis, so the reflection leaves it alone. H1, H2, and the first clause of H4 read here.
...
def _():
    ...
    **H3.** The term costs nothing on the task or the placement. Clean exact-match accuracy on all probe lines is within 0.02 of the no-fallback condition (partial: within 0.05), and the seed-mean alignment margin at the end of training, the `m_span` of ex-2.1.10, is at least 0.8 of the no-fallback value. Partial: one of the two holds. Contrary: the fallback term competing with the anchor for the operand states, which is what the stop-gradient is there to prevent. The `recipe` arm says whether the code path on its own moved anything.
    ...
    **H4.** This has two clauses. Under `redirect`, seed-mean non-red damage stays at or below 0.02, since the operator touches only operand states and a non-red operand has nothing on the axis to reflect. Under `primary`, non-red damage in the fallback condition is not resolved above the no-fallback figure (0.024 in ex-2.2.1, re-measured here). Partial: one clause holds. Contrary on the second clause: the term widens the edit, which would mean the blocks now read the axis at positions or slices where they did not before.

    /// admonition | TODO
    Non-red damage under `redirect` and under `primary`, seed mean and range, for the fallback and no-fallback conditions and for every arm. Also the ex-2.2.1 write-bound map for the fallback condition under `primary`, on the shared 0–1 scale, so a wider edit shows where it acts.
    ///

    ## Carry from the antipode to zero (H5)

    **H5.** The designed response carries part of the way to zero. Along the carry sweep, γ ∈ {0.5, 1, 1.5, 2} at the training site, seed-mean target accuracy on red lines is non-decreasing in γ, allowing a dip of at most 0.02 between adjacent strengths, and at γ = 1 it is at least half its value at γ = 2. Partial: monotone, but γ = 1 below half. Contrary: target accuracy at γ = 1 sits at the no-fallback level and the rise is confined to γ > 1. That would be the known limitation showing in full, with the response living at the antipode and not reaching the projected state. The remedy for that is the D2.3 [concept swap](/todo/science/redirect-between-two-anchored-ops.md), which targets a state training actually visits, rather than a wider bracket here.

    We report the `primary` row beside the sweep, without a gate. It is the deployment read: γ = 1 at every slice rather than at the training site alone. The difference between the two says how much the projection at later slices costs the designed response.
...
def _():
    ...
    **E3 — off-axis recoverability.** This is the auditing row that the [D2.2 design](../d2.2/design.md) assigns here. We fit a ridge probe for the redness of the concept operand on the intervened operand states, per slice, under `primary` and under `redirect`, five-fold over lines. We report held-out R² for the fallback and no-fallback conditions beside the clean states. If the fallback raised the off-axis R², it would be keeping red readable off the axis so it knows when to emit the target; that is the masked case rather than the removed one. The target depends only on the visible operand, so there is no need for it to.
...
def _():
    ...
    We use the primary recipe from [ex-2.1.10](../ex-2.1.10/report.py) unchanged: the `v216` corpus, d64-L4, 100 epochs with a 10-epoch warm-up, the pooled either-operand labeller at τ = 0.1, the anchor at λ = 0.1 with its anneal, and the anti-subspace term at the ex-2.1.8 operating point. We also use the same seeds as the primary of that experiment, so the fallback and no-fallback conditions differ only in the two new terms. Both are weights on the existing anchored train step.

    **The fallback term.** On each training crop, take the lines with dose ≥ 0.8 whose concept operand has a clean embedding alignment of at least 0.5. For each one, reflect the embedding state of that operand through the axis, $h \mapsto \mathrm{normalize}(h - 2\alpha\, e_1)$, and hold the reflected state fixed with a stop-gradient. Run the blocks forward from there, with every other position clean, and take the cross-entropy at the `=` position against the target token of the line. The term is the mean over qualifying lines, at a constant weight of 0.05 from step 0. The alignment threshold keeps it inert until the anchor has placed the concept, which the ex-2.1.10 trajectories put inside the warm-up.
    ...
    **The anti-anchor term.** This is $\mathrm{mean}(\max(-\alpha, 0))$ over the same live positions and slices the anti-subspace term reads, at a constant weight of 0.1. It is a hinge on negative alignment, so it keeps clean states out of the hemisphere where the fallback lives, and being one-sided it does not oppose the anchor. The anti-subspace term already penalizes $\alpha^2$ there, so the arms that drop one term or the other say whether the hinge adds anything.
    ...
    We run one teacher-forced pass per run per intervention, over all 5,832 lines, through the eval contract in [`sca.intervention`](/src/sca/intervention.py). Each pass gives us the log-softmax at the `=` position, from which we read the probability on the correct answer and on the target, the argmax, and the mass outside the color vocabulary. It also gives the write per (slice, line, position), as in ex-2.2.1, and the clean alignment map. Target accuracy, seed agreement, and the composition all come from the argmax on red lines. The training trajectory records the fallback and anti-anchor losses beside the anchor loss and the margin, every 50 steps.
