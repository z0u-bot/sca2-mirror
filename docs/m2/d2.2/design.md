...
- **The operation as a variable.** As specified in the [backlog item](/todo/science/make-operation-variable-before-d2-2-sca.md): an op table (name, surface form, grid function with defined rounding, closed on 0..15), `op` on `Example`, seen-pair bookkeeping keyed on `(op, pair)`, ops spelled as words, and the infix frame kept for the probes. The first table is `mix` (the D2.1 op), saturating `add`, `lighten` (per-channel max), and `darken` (per-channel min), specified in [ex-2.2.3](../ex-2.2.3/experiment.py). Earlier revisions named `screen` and `multiply`; on the six-level grid neither survives the closed-pair rule (`screen` lands on the grid only where it equals `add`, `multiply` only where it is 0 or the identity), so the prereg replaced them with the two blend modes that are closed everywhere. All three of the new ops depart from `mix` on nearly every pair, so the model has to read the op. `add` and `lighten` agree on a sixth of pairs and so populate the middle *op-relevance* level. The relevance distribution for each candidate anchored op, under the table's own rounding, is rendered in ex-2.2.3's method, since the per-line predictions in [suppress operation](#suppress-the-operation-and-the-operands) rest on it. `divide` needs a saturation rule and is lumpy on a 16-level grid, so it stays out of the first table. Ops that convert through other color spaces (`hue`, `saturation`, `brightness`) are a separate question, filed at [richer op set](/todo/science/richer-op-set-operand-geometry.md).
...
| Recipe is grammar-specific | The proposals from the survey do not reproduce on the new grammar | [ex-2.2.3](../ex-2.2.3/report.py) (H2, H3) |
