...
The **handover** is the preregistered experiment that adopts all of this, drafted as [ex-2.2.9](../ex-2.2.9/report.py). It runs the recipe from ex-2.2.3 on table A+, with the stochastic corpus, the whole-line labeller, and the untied readout, at twenty seeds against the twenty of ex-2.2.3. `mix` stays as the reference op, with `hsvmix` beside it. Two arms each change one thing back, the either-slot labeller and the tied readout, so the selectivity check on the labeller and the cleaning read on the readout are each a two-arm comparison on the new grammar. The removal rows are the ones ex-2.2.8 proposed. Until the handover runs, the grammar of record is the one from ex-2.2.3; after it, plain `mix` will likely be dropped. The prereg settles the two questions the scouting left open: lines per op stay at the corpus size, with an exploratory arm holding them at the count from ex-2.2.3 to read the E4 confound, and the probe draw walks every color as op2 as well, for the non-commutative subset.
