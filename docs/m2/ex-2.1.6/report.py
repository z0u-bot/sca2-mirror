# title: Ex 2.1.6: anchoring red in a transformer
...
dia = np.maximum(sv.grid_diameter(6) * np.sqrt(ex.LABEL_W / ex.LABEL_W.max()), 0.035)
by_redness = np.argsort(ex.REDNESS)
...
cum_x = np.append(ex.REDNESS[by_redness], 1.03)
cum_y = np.append(np.cumsum(ex.LABEL_W[by_redness]), 1.0)
...
def labels_plot() -> plt.Figure:
    fig, axes = plt.subplots(1, 2, figsize=(7.6, 3.4), gridspec_kw={"width_ratios": [1, 1.25]})
    sv.plot_rgb_cube(axes[0], ex.GRID_RGB, diameter=dia)
    axes[0].set_title("share of labels (mark area)")

    grey = light_dark("#888", "#888")
    ax = axes[1]
    x = np.linspace(0, 1, 400)
    ax.plot(x, ex.RED_RATE * x**8, color=grey, lw=1, zorder=1)
    ax.axhline(ex.LABEL_P.mean(), ls=(0, (4, 3)), lw=1, color=grey, zorder=1)
    ax.scatter(
        ex.REDNESS,
    ...
    ax.annotate(f"{int((ex.LABEL_P < 1e-6).sum())} colors below", (0.02, 1.4e-6), fontsize=7, color=grey)
    ax.annotate("corpus mean", (1.0, ex.LABEL_P.mean() * 1.3), fontsize=7, ha="right", color=grey)

    cum = ax.twinx()
    cum.fill_between(cum_x, cum_y, step="post", lw=0, color=light_dark("#0000000f", "#ffffff0f"), zorder=0)
    cum.set_ylim(0, 1.02)
    cum.set_xlim(-0.03, 1.03)
    cum.set_ylabel("cumulative share", color=grey)
    cum.tick_params(colors=grey)
    return fig
...
epochs = np.linspace(0, ex.SCHEDULER.epochs, 2001)
lr = ex.learning_rate(epochs) / ex.PEAK_LR
weight = ex.anchor_weight(epochs)
star = ex.anchor_weight(epochs, anneal_start=ex.STAR_ANNEAL_START)
...
def schedules_plot() -> plt.Figure:
    lr_c, an_c = light_dark("#888", "#999"), light_dark("#c1332a", "#f0665a")
    fig, ax = plt.subplots(figsize=(7.6, 3.2))
    ax.plot(epochs, lr, color=lr_c, lw=1.5, label="learning rate")
    ax.plot(epochs, weight, color=an_c, lw=2.0, label=r"anchor weight $\lambda$")
    ax.plot(epochs, star, color=an_c, lw=1.5, ls=(0, (4, 3)), label=r"$\lambda{=}0.1^{*}$ (star arm)")
    for start, text_at in ((ex.STAR_ANNEAL_START, (40, 0.40)), (ex.ANNEAL_START, (82, 0.26))):
        at = float(ex.learning_rate(start) / ex.PEAK_LR)
        ax.plot([start], [at], "o", ms=4, color=lr_c)
        ax.annotate(
...
def over_seeds(cond: str, fn: Callable[[dict], float]) -> np.ndarray:
...
REF = {"named_seen": 1.000, "named_holdout": 0.9948, "nll": 0.0245, "open": 0.1414}


def cell(cond: str, set_name: str, key: str, fmt: str = "{:.3f}") -> str:
...
rows = "".join(
    ...
    f"<td class='num'>{cell(c, 'named_seen', 'accuracy')}</td>"
    f"<td class='num'>{cell(c, 'named_holdout', 'accuracy')}</td>"
    f"<td class='num'>{cell(c, 'named_holdout', 'nll')}</td>"
    f"<td class='num'>{cell(c, 'open', 'guess_dist')}</td>"
...
ref = (
    ...
    f"<td class='num'>{REF['named_seen']:.3f}</td><td class='num'>{REF['named_holdout']:.3f}</td>"
    f"<td class='num'>{REF['nll']:.3f}</td><td class='num'>{REF['open']:.3f}</td>"
    f"<td class='num'>—</td><td>reference</td></tr>"
)
table = f"""<table class="report-table">
  <thead><tr>
...
"""
figure_html(table, caption=caption, class_="report-figure")
...
rungs = [c for c in RUNGS if c != "lam0"]
resp = {c: grading(c) for c in [*rungs, "lam0"]}
...
def grading_plot() -> plt.Figure:
    fig, axes = plt.subplots(1, len(rungs), figsize=(8.4, 3.0), sharey=True)
    ctrl = windowed(resp["lam0"][0])
    for ax, cond in zip(axes, rungs, strict=True):
        a, rho, r2 = resp[cond]
...
by_layer = {c: margin_map(c)[:, 0] for c in CONDS}
seeds = {c: np.array([arrays[f"{c}-s{s}/margin"][:, 0] for s in ex.SEEDS]) for c in CONDS}
depths = np.arange(len(by_layer["lam0"]))
...
def margin_by_layer_plot() -> plt.Figure:
    fig, ax = plt.subplots(figsize=(7.0, 3.2))
    c = dict(zip(CONDS, light_dark(
        ["#999", "#f2b134", "#c1332a", "#7a2320", "#3d7ea6"],
        ["#888", "#ffcc66", "#f0665a", "#b8564d", "#6ab0d4"],
    ), strict=True))  # fmt: skip
    band = seeds["lam0"]
    ax.fill_between(depths, band.min(0), band.max(0), color=c["lam0"], alpha=0.25, lw=0)
    for cond in CONDS:
        ax.plot(
            depths, by_layer[cond], color=c[cond], lw=2.0 if cond in RUNGS else 1.4,
...
cond = "lam0.1"
POS = ["op1", "+", "op2", "=", "ans", "⏎"]
pos_margins = np.array([arrays[f"{cond}-s{s}/margin"] for s in ex.SEEDS])  # (S, L1, T)
mean = pos_margins.mean(axis=0)
panels = [*range(mean.shape[0])][::-1]  # last layer on top, embedding at the bottom
layer_mean = mean.mean(axis=0)
pos_ratios = [layer_mean[0] / layer_mean[i] if layer_mean[i] > 0 else np.inf for i in (1, 2, 3)]
...
def positions_plot() -> plt.Figure:
    ...
    gs = fig.add_gridspec(len(panels) + 2, 1, height_ratios=[*[1.0] * len(panels), 0.25, 1.0])
    axes = [fig.add_subplot(gs[i]) for i in [*range(len(panels)), len(panels) + 1]]
    for ax in axes[1:]:
        ax.sharex(axes[0])
        ax.sharey(axes[0])
    ink = light_dark("#c1332a", "#f0665a")
    hair = light_dark("#00000055", "#ffffff55")
    x = np.arange(len(POS))
    rows = [(ax, mean[d], pos_margins[:, d], f"{d}") for ax, d in zip(axes[:-1], panels, strict=True)]
    rows.append((axes[-1], layer_mean, pos_margins.mean(axis=1), "mean"))
    for ax, y, spread, name in rows:
        heavy = name == "mean"
        smooth_step_area(ax, x, y, ramp=0.5, color=ink, alpha=light_dark(0.18, 0.24))
        smooth_step(ax, x, y, ramp=0.5, color=ink, lw=1.8 if heavy else 1.1)
        for edge in (spread.min(0), spread.max(0)):
            smooth_step(ax, x, edge, ramp=0.5, color=hair, lw=0.5)
        ...
        ax.set(ylim=(-0.008, 0.508), xlim=(-0.4, len(POS) - 0.6), yticks=[0.0, 0.25, 0.5])
    ...
    axes[-1].set_xticks(x, POS)
    axes[-1].tick_params(axis="x", labelbottom=True, labelsize=8)
    axes[-1].annotate(
        f"op1 : + = {pos_ratios[0]:.1f}×    op1 : op2 = {pos_ratios[1]:.1f}×    op1 : = = {pos_ratios[2]:.1f}×",
...
# %%
lm = margin_map("lam0.1").mean(axis=0)
r = [lm[0] / lm[i] for i in (1, 2, 3)]

rf"""
op1 does lead, but not by the factor H3 named: {r[0]:.1f}× against `+`, {r[2]:.1f}× against `=`, and {r[1]:.1f}× against op2. Had the gate been live, the op2 comparison would have passed and the other two would not. So the outcome sits between condensation and broadcast rather than at either. The ordering itself is informative. op2 is the position where the expected pull carries no information about the token actually there, and it has the least margin. `+` and `=` are constant tokens, so their states are free to encode the context of the line; they carry {lm[1] / lm[0]:.0%} and {lm[3] / lm[0]:.0%} of the margin at op1.
...
The answer position, which the pull never touches, still shows {lm[4]:.2f}, about {lm[4] / lm[0]:.0%} of the margin at op1. That is the confounded quantity the method set aside: a red op1 drags the mixed color toward red, so part of this margin is spillover from an anchored operand, and part is the redness of the answer itself finding the axis. The newline sits at {lm[5]:.2f}. A position that neither carries a color nor feels the pull should sit near zero, so this is a useful check that the map is not simply warm everywhere.
...
ratio = {
    (c, s): float(traj(c, s, "m_op1")[-1] / traj(c, s, "m_op1").max())
    for c in CONDS
    for s in ex.SEEDS
    if traj(c, s, "m_op1").max() >= ex.H4_FLOOR
}
scored = [c for c in CONDS if c != "lam0"]
...
def trajectories_plot() -> plt.Figure:
    fig, axes = plt.subplots(1, len(scored), figsize=(9.0, 3.0), sharey=True)
    ink = light_dark(["#f2b134", "#c1332a", "#7a2320"], ["#ffcc66", "#f0665a", "#b8564d"])
    grey = light_dark("#999", "#777")
    for ax, cond in zip(axes, scored, strict=True):
        epochs_ = traj(cond, 0, "epoch")
        twin = ax.twinx()
        twin.fill_between(
            epochs_, traj(cond, 0, "weight") / max(traj(cond, 0, "weight").max(), 1e-9),
            color=light_dark("#0000000d", "#ffffff12"), lw=0,
        )  # fmt: skip
        twin.plot(epochs_, traj(cond, 0, "lr") / ex.PEAK_LR, color=grey, lw=0.8, ls=(0, (1, 2)))
        twin.set_ylim(0, 1.05)
        # The schedules share a scale across panels, so the last panel numbers it for all.
        last = cond == scored[-1]
        twin.set_yticks([0, 1], ["0", "1"] if last else [])
        twin.tick_params(axis="y", labelsize=7, colors=grey, length=2 if last else 0)
        color = ink[min(scored.index(cond), 2)] if cond != "lam0.1-star" else ink[1]
        for seed in ex.SEEDS:
            ax.plot(epochs_, traj(cond, seed, "m_op1"), color=color, lw=1.2, alpha=0.85)
        ax.axhline(ex.H4_FLOOR, color=grey, lw=0.8, ls=(0, (4, 3)))
        ratios_ = [ratio[(cond, s)] for s in ex.SEEDS if (cond, s) in ratio]
        ax.annotate(
            "ratios " + ", ".join(f"{r:.2f}" for r in ratios_) if ratios_ else "all below floor",
...
# %%
m = {(c, s): np.asarray(cells[f"{c}-s{s}"]["traj"]["m_op1"], dtype=float) for c in CONDS for s in ex.SEEDS}
ratios = {k: float(v[-1] / v.max()) for k, v in m.items() if v.max() >= ex.H4_FLOOR}
fails = [k for k, r in ratios.items() if r < 0.8]
peak_epoch = [float(cells[f"{c}-s{s}"]["traj"]["epoch"][int(m[(c, s)].argmax())]) for (c, s) in ratios]

rf"""
**H4 fails, but not in the way it was written to catch.** All {len(ratios)} anchored runs clear the 0.2 floor, so all of them are scored. {len(fails)} of them end below 0.8× their running maximum, spread across {len({c for c, _ in fails})} of the four anchored conditions, including $\lambda{{=}}0.1$. The partial outcome (violations confined to $\lambda{{=}}0.3$) does not apply. The control runs peak at {max(m[("lam0", s)].max() for s in ex.SEEDS):.3f}. That is under the floor and in line with the 0.06 a noise-only trajectory reaches, so they are reported but not scored, as designed.

The trajectories show a slide rather than a collapse. Every anchored run rises through the LR warmup to a peak between epochs {min(peak_epoch):.0f} and {max(peak_epoch):.0f}, then loses about a quarter of that over the following forty or so epochs and holds flat from there. The anneal is not the trigger. The loss of margin is complete long before epoch 90; the earlier anneal in the $\lambda{{=}}0.1^{{*}}$ arm does not move it; and the end value is the same ({min(m_op1(c).mean() for c in CONDS if c != "lam0"):.2f}–{max(m_op1(c).mean() for c in CONDS if c != "lam0"):.2f}) across a tenfold range of $\lambda$. So we are not seeing the ex-2.9.3 mechanism, where protection is withdrawn while the optimizer is still hot and the task loss reclaims the axis. The schedule did the job it was designed for; the margin settles at a level the pull does not set.
...
leak = {c: np.mean([cells[f"{c}-s{s}"]["leak_r2"] for s in ex.SEEDS], axis=0) for c in CONDS}
axis = {c: np.mean([cells[f"{c}-s{s}"]["axis_r2"] for s in ex.SEEDS], axis=0) for c in CONDS}
...
def leakage_plot() -> plt.Figure:
    fig, axes = plt.subplots(1, 2, figsize=(8.0, 3.0), sharey=True)
    c = dict(zip(CONDS, light_dark(
        ["#999", "#f2b134", "#c1332a", "#7a2320", "#3d7ea6"],
        ["#888", "#ffcc66", "#f0665a", "#b8564d", "#6ab0d4"],
    ), strict=True))  # fmt: skip
    for ax, data, title in ((axes[0], leak, "off the anchor axis"), (axes[1], axis, "on the anchor axis")):
        for cond in CONDS:
            ax.plot(
                depths, data[cond], color=c[cond], marker="o", ms=4,
...
rf"""
**Leakage is essentially undiminished.** Project out the anchor direction, and redness is still readable from the remaining 63 directions: R² ≈ {leak["lam0.1"].mean():.2f} on $\lambda{{=}}0.1$, against {leak["lam0"].mean():.2f} for the control. The difference is a few hundredths, concentrated in the deeper layers. Meanwhile the anchor axis itself goes from carrying {axis["lam0"].mean():.2f} of the redness variance to {axis["lam0.1"].mean():.2f}. So the axis became readable for redness, but redness did not leave anywhere else. Calling that a second copy of the concept would be too strong; the exploratory section below shows why.
...
spread = {c: np.mean([geometry[f"{c}-s{s}"]["spread"] for s in ex.SEEDS], axis=0) for c in CONDS}
dot = {c: np.mean([geometry[f"{c}-s{s}"]["centre_dot_anchor"] for s in ex.SEEDS], axis=0) for c in CONDS}
depths = np.arange(len(spread["lam0"]))
...
def cloud_plot() -> plt.Figure:
    fig, axes = plt.subplots(1, 2, figsize=(8.0, 3.0))
    c = dict(zip(CONDS, light_dark(
        ["#999", "#f2b134", "#c1332a", "#7a2320", "#3d7ea6"],
        ["#888", "#ffcc66", "#f0665a", "#b8564d", "#6ab0d4"],
    ), strict=True))  # fmt: skip
    for ax, data, title in ((axes[0], spread, "extent of the cloud"), (axes[1], dot, "centre · anchor")):
        for cond in CONDS:
            ax.plot(
                depths, data[cond], color=c[cond], marker="o", ms=4,
                lw=2.0 if cond in RUNGS else 1.4, ls="-" if cond in RUNGS else (0, (4, 3)),
...
TARGETS = ["redness", "greenness", "blueness", "R", "G", "B", "sim^1.5"]
DISPLAY = {"sim^1.5": "sim¹·⁵"}
shown = ["lam0", "lam0.1"]


def seeds_of(cond: str, key: str, target: str) -> np.ndarray:
    return np.array([geometry[f"{cond}-s{s}"][key][target] for s in ex.SEEDS], dtype=float)


def cell(cond: str, key: str, target: str) -> str:  # noqa: F811 - a captioned-table helper, redefined per table
    """Seed mean with the half-range beside it, as in the H1 table."""
    v = seeds_of(cond, key, target)
    return f"<td class='num'>{v.mean():.3f} <span class='range'>±{(v.max() - v.min()) / 2:.3f}</span></td>"


rows = "".join(
    f"<tr><th>{DISPLAY.get(t, t)}</th>"
    + "".join(cell(c, "off_axis_r2", t) for c in shown)
    + "".join(cell(c, "on_axis_r2", t) for c in shown)
    + "</tr>"
    for t in TARGETS
)
table = f"""<table class="report-table">
...
"""
figure_html(table, caption=caption)


# %%
def score(cond: str, key: str, target: str) -> float:
    return float(np.mean([geometry[f"{cond}-s{s}"][key][target] for s in ex.SEEDS]))


rf"""
Off the axis, redness is not special. On the anchored model, greenness scores {score("lam0.1", "off_axis_r2", "greenness"):.2f} and blueness {score("lam0.1", "off_axis_r2", "blueness"):.2f}, against {score("lam0.1", "off_axis_r2", "redness"):.2f} for redness, and the raw channels score around {score("lam0.1", "off_axis_r2", "R"):.2f}. The control looks the same. So the off-axis number reflects the color cube: every corner of it is recoverable, because the task needs it to be, and redness is just one function of the colors among many. It is not evidence of a second, red-specific encoding.

On the axis, redness *is* special; this part of the primary reading survives. The anchor component carries {score("lam0.1", "on_axis_r2", "redness"):.2f} of redness variance, against {score("lam0.1", "on_axis_r2", "greenness"):.2f} for greenness and {score("lam0.1", "on_axis_r2", "blueness"):.2f} for blueness, up from {score("lam0", "on_axis_r2", "redness"):.2f} in the control. The anchor put red where we asked for it (and rather more of `sim¹·⁵`, at {score("lam0.1", "on_axis_r2", "sim^1.5"):.2f}), and nothing else came with it.
