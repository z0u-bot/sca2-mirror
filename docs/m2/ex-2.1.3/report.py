# title: Ex 2.1.3: names all the way down
...
v27_lines = "".join(ex.text for ex in nc.sample_corpus(5, 0, nc.GRIDS["v27"]))
v216_lines = "".join(ex.text for ex in nc.sample_corpus(5, 0, nc.GRIDS["v216"]))
f"`v27`:\n\n```\n{v27_lines}```\n\n`v216`:\n\n```\n{v216_lines}```"
...
# The geometry probes need only `arrays`, computed here (invisibly) so the numbers
# are already in hand by the time the narrative reaches the "Embedding geometry"
# section further down.


def geometry_of(g: str, s: int) -> dict:
...
geometry = {(g, s): geometry_of(g, s) for g in GRID_NAMES for s in SEEDS}
...
hold = {g: mean_metric(g, "named_holdout", "accuracy") for g in GRID_NAMES}
null = {g: exact_null(g, evals) for g in GRID_NAMES}

rf"""
**Headline numbers.** Mean held-out accuracy by grid: {", ".join(f"`{g}` **{v:.2f}**" for g, v in hold.items())}. On every grid but the smallest, that is far above what a model knowing only the neighborhood of the answer would score ({", ".join(f"`{g}` {v:.2f}" for g, v in null.items())}), so the model does infer the geometry (except `v27`, which is discussed below). Misses are interesting: at `v4096` they land a mean distance of just {mean_metric("v4096", "named_holdout", "guess_dist"):.3f} from the true mix (chance is {mean_metric("v4096", "named_holdout", "chance_dist"):.2f}). The sections below build that picture up piece by piece, starting with the pairs the corpus sampler actually produced.
"""

corpus_stats = metrics["corpus_stats"]
corpus_rows = "".join(
...
corpus_table = (
    ...
    '<th class="num">open pairs</th></tr>' + corpus_rows + "</table>"
)
figure_html(
    corpus_table,
    caption="""
...
@memo
def val_loss_figure(metrics: dict) -> str:
    @themed(
        name="val-loss",
        alt_text="""
...
@memo
def accuracy_figure(metrics: dict) -> str:
    sets = ["named_seen", "named_holdout"]
    ...
    def plot() -> plt.Figure:
        fig, axes = plt.subplots(1, len(sets), figsize=(8.4, 3.2), sharey=True)
        shades = grid_shades()
        xs = np.arange(len(GRID_NAMES))
        for ax, es in zip(axes, sets, strict=True):
...
accuracy_figure(metrics)


# %%
def z_score(g: str) -> float:
    ...
    """
    n0, n = exact_null(g, evals), len(evals[g]["named_holdout"]) * len(SEEDS)
    return (mean_metric(g, "named_holdout", "accuracy") - n0) / ((n0 * (1 - n0) / n) ** 0.5)


null27 = exact_null("v27", evals)
shell27 = (
    bl.shell_mask(nc.GRIDS["v27"], raw_rgb("v27"), [ex.result for ex in evals["v27"]["named_holdout"]])
    .sum(axis=1)
    .mean()
)
z27, z64, z216, z4096 = (z_score(g) for g in GRID_NAMES)
r2_27 = geom("v27", "r2")


def miss_structure(g: str) -> tuple[int, int, int]:
...
m64, m216, m4096 = (miss_structure(g) for g in ("v64", "v216", "v4096"))

rf"""
Every grid learns its seen pairs almost perfectly, apart from `v4096`, which reaches {mean_metric("v4096", "named_seen", "accuracy"):.2f}. That is the one grid where even the training pairs are hard to memorize, since its corpus visits 99k distinct pairs roughly once each. Held-out accuracy then follows a non-monotonic path: {mean_metric("v27", "named_holdout", "accuracy"):.2f} at `v27` (that's {round(mean_metric("v27", "named_holdout", "accuracy") * 10)} of its ten held-out pairs), {mean_metric("v64", "named_holdout", "accuracy"):.2f} at `v64`, {mean_metric("v216", "named_holdout", "accuracy"):.2f} at `v216`, near enough to solved, and back down to {mean_metric("v4096", "named_holdout", "accuracy"):.2f} at `v4096`.

Only three of those four numbers mean much. On a 27-color grid the true mix has just {shell27:.1f} one-step neighbors on average, so a model that has located the neighborhood of the answer and picks within it scores {null27:.2f} without being able to name anything. The `v27` score of {mean_metric("v27", "named_holdout", "accuracy"):.2f} sits {z27:.1f} std above that, over ten distinct pairs, which is not a difference worth reading. The denser grids clear the same bar easily: {z64:.0f} std at `v64`, {z216:.0f} at `v216` and {z4096:.0f} at `v4096`.

So the `v27` score is not evidence on its own. It is tempting to read it as a shift from the base language, where `named_holdout` sat at zero through every intervention ex-2.1.2 tried, but two of ten pairs on a grid this coarse cannot carry that. The case that the geometry is inferable from names rests on the denser grids here, and on the embedding probes further down, which reach R² ≈ {r2_27:.2f} even at `v27`.

The `v4096` misses are not scattered, either. Pooled over seeds, {m4096[1]} of {m4096[0]} held-out misses differ from the true mix in a single RGB channel, and {m4096[2]} of those are off by one grid level (`v64`: {m64[2]}/{m64[0]}; `v216`: {m216[2]}/{m216[0]}). We checked whether these misses are more likely on rounding cases, meaning channels where the operand sum is odd so the mean has to round, and they don't: about half of the one-channel misses are rounding cases, which matches chance.
"""
...
@memo
def distance_ecdf_figure(arrays: dict[str, np.ndarray], evals: dict) -> str:
    panels = [("named_holdout", "held-out pairs"), ("open", "open pairs")]
    ...
    def plot() -> plt.Figure:
        fig, axes = plt.subplots(1, len(panels), figsize=(8.4, 3.4), sharey=True)
        shades = grid_shades()
        for ax, (es, title) in zip(axes, panels, strict=True):
            for g in GRID_NAMES:
...
distance_ecdf_figure(arrays, evals)

# %%
open_grids = [g for g in GRID_NAMES if evals[g].get("open")]


def open_row(g: str) -> str:
    exs = evals[g]["open"]
    d = dist_matrix(g, exs)
...
@memo
def embedding_pca_figure(arrays: dict[str, np.ndarray], metrics: dict) -> str:
    @themed(
        name="embedding-pca",
        alt_text="""
...
embedding_pca_figure(arrays, metrics)
...
@memo
def embedding_probe_cube_figure(geometry: dict[tuple[str, int], dict]) -> str:
    @themed(
...
embedding_probe_cube_figure(geometry)


# %%
def evr3_mean(g: str) -> float:
    return float(np.mean([cell_of(metrics, g, s)["emb_evr3"] for s in SEEDS]))


def emb_r2_mean(g: str) -> float:
    return float(np.mean([cell_of(metrics, g, s)["emb_r2"] for s in SEEDS]))
...
H4 expected the embedding table itself to turn low-dimensional, and the PCA says it doesn't: even the best three directions hold only {evr3_mean("v27"):.0%} of the variance at `v27`, and that falls to {evr3_mean("v4096"):.0%} at `v4096`. The cube is in there as a subspace the probe can read, but most of the embedding variance is doing something else.

[^folds]: Probes fit per-color with leave-one-out, which matters most at the
small grids. A single half/half split leaves the `v27` probe fitting a 64→3 map from 13 points, and scores it {emb_r2_mean("v27"):.2f} against {geom("v27", "r2"):.2f} here; `v4096` is unmoved either way ({emb_r2_mean("v4096"):.2f} against {geom("v4096", "r2"):.2f}).
"""
...
@memo
def residual_probes_figure(metrics: dict) -> str:
    @themed(
...
completion_tables = [
    figure_html(
        f'<table class="report-table">{completion_head}{completion_rows(g, es, completion_k)}</table>',
        caption=f"`{g}` · {es.replace('_', ' ')}: the {completion_k} widest misses (seed 0).",
        class_="report-figure",
    )
    for g, es in [("v27", "named_holdout"), ("v216", "open"), ("v4096", "named_holdout")]
]
"\n".join(completion_tables)

# %%
exs27 = evals["v27"]["named_holdout"]
rgb27 = raw_rgb("v27")
operand_null = bl.operand_shell_null(
    bl.shell_mask(nc.GRIDS["v27"], rgb27, [ex.result for ex in exs27]),
    rgb27,
    [(ex.lhs, ex.rhs) for ex in exs27],
)
n_operand_guesses = k_operand_guesses = 0
for s in SEEDS:
    for ex, gv in zip(exs27, rgb27[arrays[f"v27-s{s}/logp/named_holdout"].argmax(1)], strict=True):
        n_operand_guesses += 1
        k_operand_guesses += tuple(gv) in (tuple(ex.lhs), tuple(ex.rhs))
...
Several of the `v27` rows return one of the operands, may or may not be an echo. Returning an operand and returning a neighbor are close to the same thing here. {k_operand_guesses} of {n_operand_guesses} guesses are operands, against {operand_null:.0%} for a guess drawn uniformly from the neighbors.
"""
