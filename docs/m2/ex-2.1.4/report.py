# title: Ex 2.1.4: spelling the names
...
# The document's directory is on sys.path while it runs, so refs and sweep
...
def mean_metric(g: str, es: str, key: str) -> float:
    return float(np.mean([cell_of(metrics, g, s)["sets"][es][key] for s in SEEDS]))


headline_hold = {g: mean_metric(g, "named_holdout", "accuracy") for g in GRID_NAMES}
headline_mal = {g: mean_metric(g, "named_holdout", "p_malformed") for g in GRID_NAMES}

rf"""
**Headline numbers.** Mean held-out greedy accuracy: {", ".join(f"`{g}` **{v:.2f}**" for g, v in headline_hold.items())} (word level scored 0.27 and ≈ 1.0 on these grids). Malformed completions: {", ".join(f"{v:.1%} on `{g}`" for g, v in headline_mal.items())}. Held-out guesses land a mean {mean_metric("v216", "named_holdout", "guess_dist"):.2f} from the true mix on `v216` (chance {mean_metric("v216", "named_holdout", "chance_dist"):.2f}). The sections that follow fill this in, starting with the corpus accounting.
"""


def corpus_table(metrics: dict) -> str:
    ...
    return (
        ...
        '<th class="num">corpus tokens</th></tr>' + rows + "</table>"
    )


figure_html(
    corpus_table(metrics),
    caption=f"""
...
@memo
def val_loss_plot(metrics: dict) -> str:
    @themed(
        name="val-loss",
        alt_text="""
...
word_metrics = load_sibling(WORD_METRICS_REF)
ACC_SETS = ["named_seen", "named_holdout"]


@memo
def accuracy_plot(metrics: dict, word_metrics: dict | None) -> str:
    @themed(
        name="accuracy",
        alt_text="""
    ...
    def plot() -> plt.Figure:
        fig, axes = plt.subplots(1, len(ACC_SETS), figsize=(8.4, 3.4), sharey=True)
        shades = grid_shades()
        fg = light_dark("#333", "#ddd")
        for ax, es in zip(axes, ACC_SETS, strict=True):
            for i, g in enumerate(GRID_NAMES):
                ...
                if word_metrics is not None:
                    wl = np.mean([cell_of(word_metrics, g, s)["sets"][es]["accuracy"] for s in SEEDS])
        ...
        def pair(a: float) -> tuple:
            return tuple(plt.Rectangle((0, 0), 1, 1, fc=shades[g], alpha=a) for g in GRID_NAMES)

        handles: list = [pair(1.0), pair(0.55)]
        labels = ["greedy", "candidate"]
        if word_metrics is not None:
            handles.append(plt.Line2D([], [], color=fg, ls="--", lw=1.4))
            labels.append("word level (ex-2.1.3)")
        axes[1].legend(
            handles,
            labels,
...
accuracy_plot(metrics, word_metrics)

# %%


def miss_structure(g: str, arrays: dict, evals: dict) -> tuple[int, int, int]:
...
m216 = miss_structure("v216", arrays, evals)
s2_27 = float(np.mean([cell_of(metrics, "v27", s)["sets"]["named_holdout"]["calibration"]["s2"] for s in SEEDS]))
s2_216 = float(np.mean([cell_of(metrics, "v216", s)["sets"]["named_holdout"]["calibration"]["s2"] for s in SEEDS]))

rf"""
Seen pairs are perfect on both grids (greedy {mean_metric("v27", "named_seen", "accuracy"):.2f} / {mean_metric("v216", "named_seen", "accuracy"):.2f}), and H1 holds: no malformed completion turns up in any eval set or seed, and the probability mass on complete names sums to {mean_metric("v216", "named_holdout", "mass_names"):.2f}. Greedy and candidate accuracy agree everywhere, so it seems that spelling is a settled sub-problem.

Held-out accuracy does drop. `v216` lands at {mean_metric("v216", "named_holdout", "accuracy"):.2f}, below its word-level benchmark of ≈ 0.99 but in line with H2. The misses are similar to the word-level full grid: pooled over seeds, {m216[1]} of {m216[0]} are wrong in exactly one RGB channel, and {m216[2]} of those are off by a single grid level. The model works out roughly the right mix and occasionally rounds one channel to a neighboring level.

`v27` drops much more than H2 expected: zero on every seed (compared to 0.27 for word-level tokens). And the model is confident about it: s₂ = {s2_27:.2f} on those prompts (confidently wrong; `v216` reads {s2_216:.2f}, well calibrated), and the candidate NLL of the true answer is ≈ 13 nats. The failure rows further down show that it expects nearby names — often one of the operands, though the section after them shows that is what the grid geometry predicts on its own.
...
DIST_PANELS = [("named_holdout", "held-out pairs"), ("open", "open pairs")]


@memo
def distance_ecdf_plot(arrays: dict, evals: dict) -> str:
    @themed(
        name="distance-ecdf",
        alt_text="""
    ...
    def plot() -> plt.Figure:
        fig, axes = plt.subplots(1, len(DIST_PANELS), figsize=(8.4, 3.4), sharey=True)
        shades = grid_shades()
        for ax, (es, title) in zip(axes, DIST_PANELS, strict=True):
            for g in GRID_NAMES:
                exs = evals[g][es]
                d = dists_for(g, exs)
...
distance_ecdf_plot(arrays, evals)

# %%

shape_27 = miss_shape("v27", arrays, evals)


def blind_dist(g: str, es: str, evals: dict) -> float:
...
`v216` tracks its floor: held-out guesses land a mean {mean_metric("v216", "named_holdout", "guess_dist"):.3f} from the true mix, and open-pair guesses {mean_metric("v216", "open", "guess_dist"):.2f} against a floor of {mean_metric("v216", "open", "floor_dist"):.2f}, a prompt-blind constant of {blind_dist("v216", "open", evals):.2f} and chance of {mean_metric("v216", "open", "chance_dist"):.2f}, landing on the single nearest name {mean_metric("v216", "open", "nearest_acc"):.0%} of the time.

`v27` lands on the single nearest name {mean_metric("v27", "open", "nearest_acc"):.0%} of the time against {blind_nearest("v27", "open", evals):.0%} for the constant, and {shape_27["shell"]} of the {shape_27["n"]} held-out guesses sit one grid step from the true mix (0.53), the rest at two steps. So the geometry did come through the tokenizer change on both grids, and what `v27` lost is the last step, picking the exactly right name — but this grid seems to be too coarse.
...
KL_TAUS = np.geomspace(0.02, 1.2, 40)


def kl_curves(g: str, es: str, arrays: dict, evals: dict, taus: np.ndarray) -> tuple[np.ndarray, np.ndarray]:
...
@memo
def distance_kl_plot(arrays: dict, evals: dict, taus: np.ndarray) -> str:
    @themed(
        name="distance-kl",
        alt_text="""
    ...
    def plot() -> plt.Figure:
        ...
        for ax, (es, title) in zip(axes, DIST_PANELS, strict=True):
            for g in GRID_NAMES:
                kl_m, kl_u = kl_curves(g, es, arrays, evals, taus)
                ax.plot(taus, kl_m, color=shades[g], lw=1.6, label=g)
                ax.plot(taus, kl_u, color=shades[g], lw=1.0, ls="--", alpha=0.7)
...
distance_kl_plot(arrays, evals, KL_TAUS)
...
@memo
def residual_probes_plot(metrics: dict) -> str:
    @themed(
    ...
    def plot() -> plt.Figure:
        ...
        for ax, g in zip(axes, GRID_NAMES, strict=True):
            ...
            n = len(next(iter(transfer.values())))
            ax.set_xticks(range(n), ["emb", *map(str, range(1, n))])
            ax.grid(alpha=0.3)
...
residual_probes_plot(metrics)

# %%

op_v216 = cell_of(metrics, "v216", 0)["probe_r2"]["operand_rgb"]
res_v216 = cell_of(metrics, "v216", 0)["transfer_r2"]

rf"""
The mix is computed in value space at the pre-answer position on both grids, but it shows up later. At word level it was decodable from depth 1–2 on. Here at `v216` it is essentially absent until the final block: R² {res_v216["fit"][2]:.2f} at depth 2, {res_v216["fit"][3]:.2f} at depth 3, then {res_v216["fit"][4]:.2f} at depth 4. The earlier layers are reading: the operand probe climbs {" → ".join(f"{v:.2f}" for v in op_v216)} across depths, so turning a four-letter name back into a value is itself a multi-layer computation. The final-block mix transfers with almost no loss to held-out and open prompts ({res_v216["named_holdout"][4]:.2f} and {res_v216["open"][4]:.2f}), so it is a real value-space computation.
...
hex_margins = load_sibling(HEX_MARGINS_REF, kind="npz")


@memo
def answer_schedule_plot(arrays: dict, metrics: dict, hex_margins: dict | None) -> str:
    @themed(
    ...
    def plot() -> plt.Figure:
        ...
        if hex_margins is not None and "control-s0/schedule/r2" in hex_margins:
            panels.append(("hex (ex-2.1.2)", hex_margins["control-s0/schedule/r2"][:, -1], list(range(-4, 4)), True))
...
answer_schedule_plot(arrays, metrics, hex_margins)
...
def completion_rows(grid: str, es: str, evals: dict, arrays: dict, k: int = 6) -> str:
    exs = evals[grid][es]
...
"\n\n".join(
    figure_html(
        f'<table class="report-table">{COMPLETION_HEAD}{completion_rows(g, es, evals, arrays, COMPLETION_K)}</table>',
        caption=f"`{g}` · {es.replace('_', ' ')}: the {COMPLETION_K} widest misses (seed 0).",
    )
    for g, es in [("v27", "named_holdout"), ("v216", "named_holdout"), ("v216", "open")]
)

# %%

miss27 = miss_shape("v27", arrays, evals)
n27, k27, null27 = miss27["n"], miss27["operand"], miss27["null_operand"]
se27 = (n27 * null27 * (1 - null27)) ** 0.5
evidence27 = v27_evidence(evals)

rf"""
In `v216`, the worst held-out misses are neighbor colors, and its worst open-pair choices hover a step from the floor. The `v27` misses often return an operand ({k27} of {n27} predictions, {k27 / n27:.0%}), which looks like an echo until you count the alternatives. Closure puts each operand one grid level from the mix, so a guesser that picks uniformly from the one-step shell of the mix already returns an operand {null27:.0%} of the time; against that reference the observed rate is about {(k27 / n27 - null27) / (se27 / n27):.1f} standard errors away, on {n27} predictions. So there is no operand echo here, only a small vocabulary.
...
It turns out the `v27` grid is too coarse to answer the question. Its closed-pair universe is {evidence27["seen"] + evidence27["hold"]} equations in total, split into {evidence27["seen"]} for training and {evidence27["hold"]} held out. Of the training pairs, {evidence27["seen"] - evidence27["informative"]} are `a + a = a`, leaving {evidence27["informative"]} equations that say anything about how two names combine. There are {evidence27["lonely"]} names that never appear in a mix at all, and one held-out pair is built entirely from them: both operands and the true answer occur in training only as `a + a = a`. No amount of skill reaches that answer from this corpus.

The grading side is equally thin for v27. A guesser confined to the one-step shell of the true mix scores {evidence27["shell_null"]:.2f} exact on these ten pairs, and always answering the center of the training answers scores {evidence27["blind_exact"]:.2f}, so the word-level 0.27 of Ex-2.1.3 and the 0.00 of this experiment both sit inside the range a model with no naming ability produces.
