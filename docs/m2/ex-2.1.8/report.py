# title: Ex 2.1.8: Repulsion tuning
...
Then in ex-2.1.7 we added a repulsive *anti-subspace* term that pushed everything away from the _red_ axis $\hat v_{{\text{{red}}}}$. It helped, but not enough: the mean *red*-alignment over all colors fell from $\bar\alpha$ = 0.53 to {ex.EX217_REFERENCE["end50-hold03"]["alpha"]:.2f}, and the target was &lt; {ex.MEAN_ALIGN_GATE:g}.
...
| $\lambda_\text{{a}}$ | anchor weight, fixed at {ex.SCORING_LAMBDA:g} throughout | |
...
| contained | of a condition: $\bar\alpha \le {ex.MEAN_ALIGN_GATE:g}$, so the drift is held near the control rather than just weakened | |
| $\rho$ | *rank* grading: the Spearman correlation between the per-color $\alpha_c$ and `redness`. Asks whether the response puts the colors in the same order redness does, ignoring how large it is | ≥ {ex.GRADE_RHO_GATE:g} |
| $R^2$ | *proportionality* grading: the squared Pearson correlation between $\alpha_c$ and `sim¹·⁵`, each color's cosine similarity to *red* raised to 1.5. Asks whether the response has that shape, ignoring its scale | ≥ {ex.GRADE_R2_GATE:g} |
| retention | per run, the final $m_{{\text{{op1}}}}$ divided by its running peak (scored once the peak reaches {ex.H4_FLOOR:g}); reported as the minimum across seeds | ↑ |
...
# The data load and the analysis functions come first, ahead of Method and
# Hypotheses in the rendered order, because the Findings summary right below
# needs them: Marimo can compute a cell out of the order it displays it in,
# but a literate script cannot, so this is the earliest point in the
# rendered document where they can be computed.
res = load_results()
if res is None:
...
# The preregistered primary condition, and the best-graded condition in the
# grid: both are read more than once below, so each is resolved here.
PRIMARY_COND = ex.PRIMARY
BEST_R2_COND = max(ex.FACTORIAL_NAMES, key=lambda c: grading(c)[1])
...
**H2 (an operating point exists) — fails, on grading.** `{PRIMARY_COND}`, the preregistered primary condition, meets containment ($\bar\alpha = {alpha_bar(PRIMARY_COND).mean():.3f}$, gate {ex.MEAN_ALIGN_GATE:g}), margin ({m_op1(PRIMARY_COND).mean():.3f}, gate {ex.MARGIN_GATE:g}) and retention ({retention(PRIMARY_COND):.2f}, gate {ex.H4_RETENTION:g}). No condition in the grid reaches the grading gate of {ex.GRADE_R2_GATE:g} on either track; the best R² is {grading(BEST_R2_COND)[1]:.2f}. Neither named partial applies.
...
**Containment is now newly reachable.** `{PRIMARY_COND}` is the first condition in M2 to hold $\bar\alpha$ near the control while keeping the margin. The best in ex-2.1.7 was {ex.EX217_REFERENCE["end90-hold03"]["alpha"]:.2f} and its gate went unmet everywhere.
"""
...
rf"""
The anchor weight stays at $\lambda_\text{{a}} = {ex.SCORING_LAMBDA:g}$. The ceiling arm in ex-2.1.7 showed selectivity falling at ten times this weight, with no task cost even there. So the previous scoring rung is still the right place to tune the repulsion against.
"""
...
- **A.** Anneal endpoint
    ...
    Levels: {and_join(f"{e:g}" for e in ex.ANTI_ANNEAL_ENDS)}.
...
- **B.** Hold ratio
    ...
    The level it settles at. Levels: {and_join(f"{h:.2f}" for h in ex.ANTI_HOLD_RATIOS)}.
...
giving {len(ex.CONDITIONS)} conditions and {len(ex.CONDITIONS) * len(ex.SEEDS)} runs in total.
"""
...
dose_ref = ex.anti_dose(next(c for c in ex.CONDITIONS if c["name"] == ex.DOSE_ARM_REFERENCE))
dose_rows = "\n".join(
    f"| `{c['name']}` | {c['anti_anneal_end']:.0f} | {c['anti_hold_ratio']:.2f} | "
    f"{c['anti_peak_ratio']:.3f} | {ex.anti_dose(c):.5f} | {ex.anti_dose(c) / dose_ref:.2f} |"
...
def anti_mu(c: dict, e: np.ndarray) -> np.ndarray:
...
@memo
def schedules_figure(conditions: list[dict], e: np.ndarray, lr: np.ndarray, alt_text: str, caption: str) -> str:
    """The anti-subspace schedules under test, and the dose they deliver. Memoized on the schedule inputs and the text."""

    @themed(name="schedules", alt_text=alt_text, caption=caption)
    def plot() -> plt.Figure:
        grey = light_dark("#666", "#999")
        hold = {0.03: light_dark("#1f6fb4", "#5fa8dd"), 0.30: light_dark("#b4531f", "#dd8f5f")}
        dashes = {50.0: "-", 70.0: (0, (1, 1.6)), 90.0: (0, (4, 1.6))}
        ...
        arm_color = light_dark("#7b3fa0", "#b98ce0")
        ...
        for c in conditions:
            if c["name"] not in ex.FACTORIAL_NAMES:
                continue
            dash = dashes[c["anti_anneal_end"]]
            axs[0].plot(e, anti_mu(c, e), color=hold[c["anti_hold_ratio"]], ls=dash, lw=1.4)
        arm = by_name[ex.DOSE_ARM]
        axs[0].plot(e, anti_mu(arm, e), color=arm_color, ls="-.", lw=1)
        axs[0].plot(e, ex.anchor_weight(e), color=grey, lw=0.5, alpha=0.45, zorder=-1)
        axs[0].set_yscale("log")
        axs[0].set_ylim(1e-3, 1)
        axs[0].minorticks_off()
        axs[0].set_ylabel("weight", fontsize="x-small")
        axs[0].set_title("the factorial", fontsize="small", color=grey)
        ...
        def key(label: str, ls="none", color=grey, lw=1.4) -> plt.Line2D:
            return plt.Line2D([], [], label=label, ls=ls, color=color, lw=lw)

        axs[0].legend(
            handles=[
                key("anneal end"),
                *(key(f"{e_:g}", ls=d) for e_, d in dashes.items()),
                key("hold ratio"),
                *(key(f"{h:.2f}", ls="-", color=col) for h, col in hold.items()),
                key("dose arm", ls="-.", color=arm_color, lw=1),
            ],
            loc="upper right",
            ncols=2,
            fontsize=6.5,
        ...
        ax, ref = axs[1], by_name[ex.DOSE_ARM_REFERENCE]
        d_arm, d_ref = anti_mu(arm, e) * lr * 1e3, anti_mu(ref, e) * lr * 1e3
        hatch = dict(facecolor="none", lw=0, hatch_linewidth=0.5)
        ax.fill_between(e, d_ref, d_arm, where=d_arm < d_ref, hatch="///", edgecolor=grey, **hatch)
        ax.fill_between(e, d_ref, d_arm, where=d_arm > d_ref, hatch="\\\\\\", edgecolor=arm_color, **hatch)
        ...
        ax.plot(e, anti_mu(by_name["end90-hold03"], e) * lr * 1e3, color=grey, lw=0.5, alpha=0.45, ls=dashes[90.0])
        ax.plot(e, ex.anchor_weight(e) * lr * 1e3, color=grey, lw=0.5, alpha=0.45, zorder=-1)
        ax.plot(e, d_ref, color=grey, lw=1.0, ls=dashes[50.0])
        ax.plot(e, d_arm, color=arm_color, lw=1, ls="-.")
        ax.set_ylim(0, None)
        ax.set_ylabel("weight × lr / $10^{-3}$", fontsize="x-small")
        ax.set_title("the dose arm", fontsize="small", color=grey)
...
schedules_figure(ex.CONDITIONS, E, LR, SCHEDULES_ALT, SCHEDULES_CAPTION)
...
seeds in its Arms section. This report uses the **{ex.RETENTION_STAT}**, and every
...
exact match within {ex.TASK_GATE:g} (absolute) of the seed mean of the in-experiment
...
(a) containment, $\bar\alpha \le {ex.MEAN_ALIGN_GATE:g}$;
(b) margin, $m_{{\text{{op1}}}} \ge {ex.MARGIN_GATE:g}$;
(c) grading, Spearman $\rho \ge {ex.GRADE_RHO_GATE:g}$ against `redness` or Pearson
$R^2 \ge {ex.GRADE_R2_GATE:g}$ against `sim¹·⁵`;
(d) retention, every run whose running maximum of $m_{{\text{{op1}}}}$ reaches
{ex.H4_FLOOR:g} ending at $\ge {ex.H4_RETENTION:g}\times$ that maximum.
Partial, in decreasing strength: (a) is met at the looser {ex.MEAN_ALIGN_PARTIAL:g} (the
halving level from ex-2.1.7) while (b)–(d) all hold; or (a) is met at
{ex.MEAN_ALIGN_GATE:g} but the margin falls between the ex-2.1.7 value of {ex.EX217_REFERENCE["end50-hold03"]["m_op1"]:.2f} and the
gate.
...
at 0.30, is at least {ex.ALPHA_EFFECT_GATE:g}. That says the floor does something at all.
(b) The interaction is at least {ex.ALPHA_INTERACTION_GATE:g}: the end50 − end90 difference
...
minus the three at endpoint 90. If it clears {ex.ALPHA_EFFECT_GATE:g} in both rows while the
interaction stays under {ex.ALPHA_INTERACTION_GATE:g}, that is the timing account: the repulsion
has to be present while the pull is strong, and no floor stands in for it.
**2.** If `{ex.PRIMARY}` alone contains the drift while (a) fails, then each
...
[^h3-fail-2]: Suppose the `hold03` row repeats its ex-2.1.7 span of {ex.EX217_REFERENCE["end50-hold03"]["alpha"]:.2f}
    to {ex.EX217_REFERENCE["end90-hold03"]["alpha"]:.2f}. Then (a) can only fail if the mean of the 0.30 row is nearly
    as high, and with `{ex.PRIMARY}` contained that in turn requires a
...
[^effect]: Start from the ex-2.1.6 control spread of about 0.02 for
    ...
    noise is near $0.02\sqrt{{2}}/\sqrt{{9}} \approx 0.009$, and {ex.ALPHA_EFFECT_GATE:g} is
    about five times that. The interaction is a difference of two
    differences of three-run condition means, with noise near
    $0.02 \cdot 2/\sqrt{{3}} \approx 0.023$, so {ex.ALPHA_INTERACTION_GATE:g} is about four times
    ...
    {ex.ALPHA_EFFECT_GATE:g} is about three times that where named miss 1 reads it.
    What the level account expects for the interaction is about 0.20,
    twice the gate: the full {ex.EX217_REFERENCE["end50-hold03"]["alpha"]:.2f} to {ex.EX217_REFERENCE["end90-hold03"]["alpha"]:.2f} span in the 0.03 row and none
...
repro_rows = "".join(
    ...
    f"<td class='num'>±{alpha_tol(c):.3f}</td></tr>"
...
the last column.
"""
figure_html(repro_table, caption=repro_caption, class_="report-figure")

# %%
repro_dm = max(abs(m_op1(c).mean() - r["m_op1"]) for c, r in ex.EX217_REFERENCE.items())
repro_da = max(abs(alpha_bar(c).mean() - r["alpha"]) for c, r in ex.EX217_REFERENCE.items())

rf"""
Both conditions reproduce. The margins land {repro_dm:.3f} from the published values against a floor of {ex.NOISE_SEED_MEAN:g}, and ᾱ within {repro_da:.3f}, inside the tolerance in either row. Grading agrees as well: `end90-hold03` scores R² = {grading("end90-hold03")[1]:.2f} here, the value ex-2.1.7 published for the same condition.
...
# %%
worst_cond = max(CONDS, key=lambda c: abs(acc(c).mean() - CONTROL_ACC))
worst_hi_cond = max(ROW[HOLD_HI], key=lambda c: abs(acc(c).mean() - CONTROL_ACC))

rf"""
**H1 holds.** The largest gap from the control is {abs(acc(worst_cond).mean() - CONTROL_ACC):.4f}, at `{worst_cond}`, against a gate of {ex.TASK_GATE:g}. Every condition solves the task.

The preregistration named the `hold{HOLD_HI * 100:02.0f}` row as the place a task cost would show up, since it sustains a repulsive weight no earlier experiment held. It does not: the worst condition in that row is `{worst_hi_cond}` at {abs(acc(worst_hi_cond).mean() - CONTROL_ACC):.4f}, the same size as the gaps elsewhere. Holding the repulsion an order of magnitude above the M1 floor for the rest of training costs nothing the task loss can see.
"""
...
def h2_row(c: str) -> str:
    ...
    return (
        ...
        f"<td class='num'>{mark(alpha_bar(c).mean(), ex.MEAN_ALIGN_GATE, up=False)}"
        f" <span class='range'>±{alpha_bar(c).std(ddof=1):.3f}</span></td>"
        f"<td class='num'>{mark(m_op1(c).mean(), ex.MARGIN_GATE, up=True)}"
        f" <span class='range'>±{m_op1(c).std(ddof=1):.3f}</span></td>"
        f"<td class='num'>{mark(rho, ex.GRADE_RHO_GATE, up=True, fmt='{:.2f}')}</td>"
        f"<td class='num'>{mark(r2, ex.GRADE_R2_GATE, up=True, fmt='{:.2f}')}</td>"
        f"<td class='num'>{mark(retention(c), ex.H4_RETENTION, up=True, fmt='{:.2f}')}</td>"
...
<table class="report-table">
  ...
  <tbody>{"".join(h2_row(c) for c in ex.FACTORIAL_NAMES)}</tbody>
</table>
"""
h2_caption = f"""
...
"""
figure_html(h2_table, caption=h2_caption, class_="report-figure")

# %%
best_rho_cond = max(ex.FACTORIAL_NAMES, key=lambda c: grading(c)[0])
contained_conds = [c for c in ex.FACTORIAL_NAMES if alpha_bar(c).mean() <= ex.MEAN_ALIGN_GATE]

rf"""
**H2 fails, on grading.** The condition the preregistration named, `{PRIMARY_COND}`, meets three of the four criteria: it contains the drift at ᾱ = {alpha_bar(PRIMARY_COND).mean():.3f} against a gate of {ex.MEAN_ALIGN_GATE:g}, holds the margin at {m_op1(PRIMARY_COND).mean():.3f} against {ex.MARGIN_GATE:g}, and retains {retention(PRIMARY_COND):.2f}× its peak against {ex.H4_RETENTION:g}. But its response is graded at ρ = {grading(PRIMARY_COND)[0]:.2f} and R² = {grading(PRIMARY_COND)[1]:.2f}, and the gate is {ex.GRADE_R2_GATE:g} on either track.

Every condition misses on grading. The best R² in the grid is {grading(BEST_R2_COND)[1]:.2f} (`{BEST_R2_COND}`) and the best ρ is {grading(best_rho_cond)[0]:.2f} (`{best_rho_cond}`), so no condition comes within {ex.GRADE_R2_GATE - grading(BEST_R2_COND)[1]:.2f} of passing. Neither named partial applies, since both assume grading holds and vary the containment or margin around it.
...
# Seed means once, up front: @themed calls the plot function twice.
traj_mean = {c: {k: np.mean([traj(c, s, k) for s in ex.SEEDS], axis=0) for k in TRAJ_KEYS} for c in ex.FACTORIAL_NAMES}
traj_mean["lam0"] = {k: np.mean([traj("lam0", s, k) for s in ex.SEEDS], axis=0) for k in TRAJ_KEYS}
traj_epochs = np.mean([traj(ex.PRIMARY, s, "epoch") for s in ex.SEEDS], axis=0)
...
SCHED_E = np.linspace(0.0, float(ex.SCHEDULER.epochs), 601)
sched_anchor = ex.anchor_weight(SCHED_E)
sched_anti = {c: anti_mu(BY_NAME[c], SCHED_E) for c in ex.FACTORIAL_NAMES}
...
TRAJ_PANELS = [*ROW[HOLD_LO], *ROW[HOLD_HI]]
TRAJ_LEFT = (ROW[HOLD_LO][0], ROW[HOLD_HI][0])
...
def trajectories_figure(
    ...
    """Training dynamics across the grid, and the schedule beneath each. Memoized on the trajectory and schedule arrays and the text."""
    ...
    def plot() -> plt.Figure:
        ...
        term = {"anchor": light_dark("#c33", "#e66"), "anti": light_dark("#36c", "#7af")}
        ...
        hold = {HOLD_LO: light_dark("#1f6fb4", "#5fa8dd"), HOLD_HI: light_dark("#b4531f", "#dd8f5f")}
        dash = (0, (6, 1))
        # One y-scale everywhere, so a line's height means the same in every panel.
        top = max(max(np.max(mean[c][k]) for k in TRAJ_KEYS) for c in ex.FACTORIAL_NAMES)
        for cond in TRAJ_PANELS:
            ax, ink = axd[cond], hold[BY_NAME[cond]["anti_hold_ratio"]]
            ...
            ax.plot(0, ex.MEAN_ALIGN_GATE, marker=9, ms=3, color=gate, clip_on=False, zorder=6,
                    transform=ax.get_yaxis_transform())  # fmt: skip
            ax.plot(1, ex.H4_FLOOR, marker=8, ms=3, color=gate, clip_on=False, zorder=6,
                    transform=ax.get_yaxis_transform())  # fmt: skip
            ax.plot(epochs, mean["lam0"]["alpha_op1"], color=grey, lw=1.1, zorder=3)
            ax.plot(epochs, mean["lam0"]["m_op1"], color=grey, lw=1.1, ls=dash, zorder=3)
            ax.plot(epochs, mean[cond]["m_op1"], color=ink, lw=1.4, ls=dash, zorder=4)
            ax.plot(epochs, mean[cond]["alpha_op1"], color=ink, lw=1.7, zorder=5)
            ...
            ax.set_ylabel("cosine", fontsize="x-small") if cond in TRAJ_LEFT else None

        import matplotlib.patheffects as pe

        halo = [pe.withStroke(linewidth=2.5, foreground=light_dark("#ffffff", "#000000"))]
        axd[TRAJ_LEFT[0]].annotate(
            "control", (58, mean["lam0"]["alpha_op1"][-1]), fontsize=7.5, color=grey, va="center", ha="center",
            path_effects=halo,
        )  # fmt: skip
        ...
        for cond in TRAJ_PANELS:
            ax = axd[f"w-{cond}"]
            ax.plot(sched_e, anchor, color=term["anchor"], lw=1.4)
            ax.plot(sched_e, anti[cond], color=term["anti"], lw=1.4)
            ax.set_yscale("log")
            ax.set_xlim(0, ex.SCHEDULER.epochs)
            ax.set_ylim(2e-4, 4)
            ...
            ax.set_ylabel("weight", fontsize="x-small") if cond in TRAJ_LEFT else None
        ...
        for x, y, txt, k in ((64, 0.13, r"$\lambda_\mathrm{a}$", "anchor"), (30, 0.5, r"$\lambda_{\bar{\mathrm{s}}}$", "anti")):  # fmt: skip
            axd[f"w-{ROW[HOLD_LO][0]}"].annotate(txt, (x, y), fontsize=9, color=term[k], va="bottom")
        return fig

    return plot()


trajectories_figure(traj_mean, traj_epochs, SCHED_E, sched_anchor, sched_anti, TRAJ_ALT, TRAJ_CAPTION)
...
CROSS_E = np.linspace(0.0, float(ex.SCHEDULER.epochs), 4001)
CROSS_A = ex.anchor_weight(CROSS_E)


def crossing(cond: str) -> float:
    """First epoch at which the repulsion falls below the pull."""
    mu = anti_mu(BY_NAME[cond], CROSS_E)
    return float(CROSS_E[mu < CROSS_A][0])


ctrl_epoch = np.mean([traj("lam0", s, "epoch") for s in ex.SEEDS], axis=0)
ctrl_alpha = np.mean([traj("lam0", s, "alpha_op1") for s in ex.SEEDS], axis=0)


def departure(cond: str) -> float:
    ...
    near = np.flatnonzero(a <= ctrl_alpha + 0.05)
    return float(ctrl_epoch[near[-1]]) if len(near) else float("nan")


crossing_rows = "".join(
    f"<tr><th><code>{c}</code></th>"
    f"<td class='num'>{crossing(c):.0f}</td><td class='num'>{departure(c):.0f}</td>"
    f"<td class='num'>{departure(c) - crossing(c):+.0f}</td>"
    f"<td class='num'>{ex.SCHEDULER.epochs - departure(c):.0f}</td>"
...
"""
figure_html(crossing_table, caption=crossing_caption, class_="report-figure")
...
def factorial_cells(fn, fmt: str = "{:.3f}") -> str:
    return "".join(
...
factorial_head = "".join(f"<th class='num'>end {e:.0f}</th>" for e in ex.ANTI_ANNEAL_ENDS)
factorial_table = f"""
<table class="report-table">
  <thead><tr><th>ᾱ ↓</th>{factorial_head}<th class="num">row mean</th></tr></thead>
  <tbody>{factorial_cells(lambda c: float(alpha_bar(c).mean()))}</tbody>
  <thead><tr><th>m<sub>op1</sub> ↑</th>{factorial_head}<th class="num">row mean</th></tr></thead>
  <tbody>{factorial_cells(lambda c: float(m_op1(c).mean()))}</tbody>
</table>
"""
factorial_caption = """
...
dose_arm_order = [ex.DOSE_ARM_REFERENCE, "end90-hold03", ex.DOSE_ARM]
dose_arm_ref = ex.anti_dose(BY_NAME[ex.DOSE_ARM_REFERENCE])
dose_arm_rows = "".join(
    f"<tr><th><code>{c}</code></th>"
    f"<td class='num'>{BY_NAME[c]['anti_anneal_end']:.0f}</td>"
    f"<td class='num'>{BY_NAME[c]['anti_peak_ratio']:.3f}</td>"
    f"<td class='num'>{ex.anti_dose(BY_NAME[c]) / dose_arm_ref:.2f}</td>"
...
"""
figure_html(dose_arm_table, caption=dose_arm_caption, class_="report-figure")

# %%
dose_arm_ref_alpha, dose_arm_late_alpha, dose_arm_alpha = (
    float(alpha_bar(c).mean()) for c in (ex.DOSE_ARM_REFERENCE, "end90-hold03", ex.DOSE_ARM)
)

rf"""
The arm lands at ᾱ = {dose_arm_alpha:.3f}, between `{ex.DOSE_ARM_REFERENCE}` ({dose_arm_ref_alpha:.3f}) and `end90-hold03` ({dose_arm_late_alpha:.3f}), and about {(dose_arm_ref_alpha - dose_arm_alpha) / (dose_arm_ref_alpha - dose_arm_late_alpha) * 100:.0f}% of the way across. So most of what the endpoint buys is timing, and some of it is dose.
...
axis_r2 = {c: float(np.mean([cells[f"{c}-s{s}"]["axis_r2"] for s in ex.SEEDS])) for c in CONDS}
leak_r2 = {c: np.mean([cells[f"{c}-s{s}"]["leak_r2"] for s in ex.SEEDS], axis=0) for c in CONDS}
cube_spread = {c: np.mean([geometry[f"{c}-s{s}"]["spread"] for s in ex.SEEDS], axis=0) for c in CONDS}
redness_floor = ex.redness_rgb_floor()
secondary_rows = "".join(
    f"<tr><th><code>{c}</code></th>"
    f"<td class='num'>{axis_r2[c]:.3f}</td>"
    f"<td class='num'>{leak_r2[c][0]:.3f}</td><td class='num'>{leak_r2[c][-1]:.3f}</td>"
    f"<td class='num'>{leak_r2[c][-1] - redness_floor:+.3f}</td>"
    f"<td class='num'>{cube_spread[c][-1]:.3f}</td></tr>"
    for c in CONDS
)
secondary_table = f"""
<table class="report-table">
...
"""
secondary_caption = f"""
Where redness is readable at op1, and what the color cloud looks like there. On-axis R² is redness read from the anchor coordinate alone, averaged over layers. The two off-axis columns are the ridge probe on the other 63 directions, at the token embedding and at the last layer, and the next column is the last-layer value against the RGB floor of {redness_floor:.3f}. Cube spread is the mean squared distance of the 216 op1 states from their centroid at the last layer: how much extent the cloud keeps. All are seed means, and none is scored.
"""
figure_html(secondary_table, caption=secondary_caption, class_="report-figure")

# %%
axis_hi_cond = max(ANCHORED, key=lambda c: axis_r2[c])

rf"""
The anchor coordinate carries redness in every anchored condition, from R² = {min(axis_r2[c] for c in ANCHORED):.2f} to {axis_r2[axis_hi_cond]:.2f} (`{axis_hi_cond}`), against {axis_r2["lam0"]:.2f} in the control. The spread across the grid is narrow, and it does not order the conditions the way ᾱ does, so this is not a second containment measurement.

Off the axis, every condition sits within {max(abs(v[-1] - redness_floor) for v in leak_r2.values()):.2f} of the RGB floor at the last layer, and the anchored conditions all sit slightly above it ({min(leak_r2[c][-1] for c in ANCHORED):.2f}–{max(leak_r2[c][-1] for c in ANCHORED):.2f}) where the control sits just below ({leak_r2["lam0"][-1]:.2f}). Above the floor is reachable because the residual stream encodes color nonlinearly and redness is quadratic in RGB, so a linear probe does better there than on raw channels. Nothing here reads as a second copy of the concept living off the axis, and nothing reads as the cube being lost: spread at the last layer runs {min(cube_spread[c][-1] for c in CONDS):.3f}–{max(cube_spread[c][-1] for c in CONDS):.3f}, with the control at {cube_spread["lam0"][-1]:.3f} in the middle of that range.
...
{ex.STEP_RHO_CEILING:.2f} and R² {ex.STEP_R2_CEILING:.2f}, both under the {ex.GRADE_R2_GATE:g} gates, so no step passes.
"""

above_step_ceiling = [c for c in ex.FACTORIAL_NAMES if grading(c)[1] > ex.STEP_R2_CEILING]
best_rho_val = max(grading(c)[0] for c in ex.FACTORIAL_NAMES)
best_r2_val = max(grading(c)[1] for c in ex.FACTORIAL_NAMES)

rf"""
Considering those ceilings, the two tracks say opposite things.

- **ρ** ranks colors by `redness`. Every condition scores below what a step would: the best ρ in the grid is {best_rho_val:.2f} against a step ceiling of {ex.STEP_RHO_CEILING:.2f}.
- **R²** fits values against `sim¹·⁵` (proportionality). {len(above_step_ceiling)} of the six score above the ceiling, up to R² = {best_r2_val:.2f} against {ex.STEP_R2_CEILING:.2f}.

Neither actually says whether the response is a grade or a step, so we visualize it below.
"""

grade_resp = {c: alpha_map(c)[:, :, 0].mean(axis=0) for c in [*ex.FACTORIAL_NAMES, "lam0"]}
redness_order = np.argsort(ex.REDNESS)


def windowed(y: np.ndarray, k: int = 25) -> np.ndarray:
    """Mean of *y* over a sliding window of the redness ordering."""
    pad = np.pad(y[redness_order], (k // 2, k // 2), mode="edge")
    return np.convolve(pad, np.ones(k) / k, mode="valid")


def fitted_target(y: np.ndarray) -> np.ndarray:
...
grade_ctrl = windowed(grade_resp["lam0"])
GRADE_GRID = [ROW[HOLD_LO], ROW[HOLD_HI]]
...
def grading_figure(
    ...
    """Per-color alignment against redness, across the grid. Memoized on the per-condition responses and the text."""

    @themed(name="grading", alt_text=alt_text, caption=caption)
    def plot() -> plt.Figure:
        ...
        for row, conds in zip(axes, GRADE_GRID, strict=True):
            for ax, cond in zip(row, conds, strict=True):
                a, (rho, r2) = resp[cond], grading(cond)
                ax.axhline(0, color=light_dark("#bbb", "#555"), lw=0.8, zorder=0)
                ax.plot(
                    redness[redness_order], ctrl, color=light_dark("#999", "#777"), lw=3, alpha=0.5, zorder=1,
                ...
                ax.plot(redness[redness_order], windowed(fitted_target(a)), color=light_dark("#888", "#888"),
                        lw=1.0, ls=(0, (4, 2)), zorder=2)  # fmt: skip
                ax.scatter(redness, a, c=ex.GRID_RGB, s=18, lw=0.4, zorder=3,
                           edgecolors=light_dark("#00000033", "#ffffff55"))  # fmt: skip
                ax.plot(redness[redness_order], windowed(a), color=light_dark("#222", "#eee"), lw=1.6, zorder=4)
                ax.set_title(
...
grading_figure(grade_resp, grade_ctrl, ex.REDNESS, ex.SIM_TARGET, GRADING_ALT, GRADING_CAPTION)

# %%
low_redness_n = int((ex.REDNESS < 0.4).sum())
...
But most of the color cube lives in that flat part. {low_redness_n} of the {ex.REDNESS.size} colors
fall below redness 0.4, and inside a flat band their alignments carry no
ordering. A rank statistic over all {ex.REDNESS.size} colors is therefore dominated by them.
"""

knee_levels = (0.2, 0.3, 0.4, 0.5)
knee_resp = {c: alpha_map(c)[:, :, 0].mean(axis=0) for c in ex.FACTORIAL_NAMES}
knee_rows = "".join(
    f"<tr><th><code>{c}</code></th>"
    f"<td class='num'>{ex.spearman(knee_resp[c], ex.REDNESS):.3f}</td>"
    + "".join(
        f"<td class='num'>{ex.spearman(knee_resp[c][ex.REDNESS >= k], ex.REDNESS[ex.REDNESS >= k]):.3f}</td>"
        for k in knee_levels
    )
    + "</tr>"
    for c in ex.FACTORIAL_NAMES
)
knee_head = "".join(
    f"<th class='num'>≥ {k:g} <span class='range'>({(ex.REDNESS >= k).sum()})</span></th>" for k in knee_levels
)
knee_table = f"""
<table class="report-table">
  <thead>
    <tr><th>condition</th><th class="num">all 216</th>{knee_head}</tr></thead>
  <tbody>{knee_rows}</tbody>
</table>
"""
knee_caption = f"""
...
"""
figure_html(knee_table, caption=knee_caption, class_="report-figure")

# %%
above_knee = ex.REDNESS >= 0.4
rho_above_knee = [
    float(ex.spearman(a[above_knee], ex.REDNESS[above_knee]))
    for a in (alpha_map(c)[:, :, 0].mean(axis=0) for c in ex.FACTORIAL_NAMES)
]

rf"""
Above the knee the ordering is good in every condition — ρ runs {min(rho_above_knee):.2f} to {max(rho_above_knee):.2f} on the {above_knee.sum()} colors with redness ≥ 0.4, against a gate of {ex.GRADE_RHO_GATE:g} — and it climbs monotonically as the flat colors are excluded. So the graded part of the response is graded, but the full-set statistic is dominated by the flat part.
"""
...
block as {low_redness_n} failures of ordering, however clean the rise above the knee
is.
...
"""
figure_html(ceiling_table, caption=ceiling_caption, class_="report-figure")

# %%
ceiling_best_r2 = max(grading(c)[1] for c in ex.FACTORIAL_NAMES)
sim_redness_corr = float(np.corrcoef(ex.SIM_TARGET, ex.REDNESS)[0, 1])
ceiling_open, ceiling_mid, ceiling_knee, ceiling_deep = (ceiling(k) for k in (0.0, 0.3, 0.4, 0.5))
ceiling_half = ceiling(0.2, scale=0.5)

rf"""
The two tracks come apart. Even with no containment at all, the best ρ a perfect `sim¹·⁵` response reaches is {ceiling_open[0]:.2f}, barely over the gate, because `sim¹·⁵` is not a monotone function of `redness` (they correlate at {sim_redness_corr:.2f}, not 1). With the {low_redness_n} colors below `redness` 0.4 contained, the ceiling falls to {ceiling_knee[0]:.2f}, under the observed values. So past a light knee the ρ track penalizes containment, and at the containment this grid achieves it cannot reach {ex.GRADE_RHO_GATE:g} however well the response is graded.

R² behaves the other way. Its target is near zero for the colors containment collapses, so containment moves the response toward the target rather than away: the ceiling is {ceiling_knee[1]:.2f} at a knee of 0.4 and {ceiling_deep[1]:.2f} even at 0.5. That track is reachable, and the best condition in this grid gets {ceiling_best_r2:.2f} of it.[^m1r2]

Within that family — `sim¹·⁵` at full amplitude — the ρ gate and the containment H2 also asks for cannot both be met. ᾱ crosses under {ex.MEAN_ALIGN_GATE:g} somewhere between a knee of 0.3 and 0.4, and by there ρ has fallen from {ceiling_mid[0]:.2f} to {ceiling_knee[0]:.2f}.

That family is narrow, though. ρ ranks colors, so it is blind to the amplitude the ᾱ gate measures: scale the above-knee response to half of `sim¹·⁵` at a knee of 0.2, and both gates clear at once, at ρ = {ceiling_half[0]:.2f} and ᾱ = {ceiling_half[2]:.3f}. What the two gates jointly exclude is a response that is both steeply graded and full-amplitude on the reds, which is narrower than the gates being incompatible. Either way the ρ gate is doing something the preregistration did not say it was doing, and a future design should settle the amplitude question before reusing it.

The R² shortfall is in the response itself: against an achievable {ceiling_knee[1]:.2f}, {ceiling_best_r2:.2f} is a real gap. Setting a realistic R² gate for future work would take more than this one grid.
...
discussion_alt = "end90-hold03"

rf"""
## Discussion

The operating point to carry forward is `{PRIMARY_COND}`: ᾱ = {alpha_bar(PRIMARY_COND).mean():.3f}, margin {m_op1(PRIMARY_COND).mean():.2f}, retention {retention(PRIMARY_COND):.2f}, task-clean. `{discussion_alt}` grades slightly better, but ᾱ decides it: ᾱ predicts how much of the color cube an edit along the anchor axis would drag with it, and the two conditions differ by {alpha_bar(discussion_alt).mean() / alpha_bar(PRIMARY_COND).mean():.1f}× on it, against a {grading(discussion_alt)[1] - grading(PRIMARY_COND)[1]:.2f} difference on the reachable grading track. Anything that pulls without a position oracle, and any intervention experiment, wants the smaller spread of side-effects.
