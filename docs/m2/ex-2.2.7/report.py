...
app = marimo.App(
    app_title="Ex 2.2.7: a pilot of the syntax embeddings",
    css_file="../../report.css",
    auto_download=["html"],
)
...
def _(res: Results):
    ...
    _red = {c: res.red_mean(c).mean() for c in ROW_ARMS}
    ...
    - **Where the component works.** On the stored `{ex.REFERENCE}` checkpoints, stripping it from the readout side costs the `=` prediction on the red lines ({_eq("recipe-short", "clean"):.2f} to {_eq("recipe-short", "output"):.2f}; {_eq("t00", "output"):.2f} on `t00`) and nothing else. Stripping it from the embedding side costs the answer instead ({_ans("recipe-short", "clean", "red"):.2f} to {_ans("recipe-short", "input", "red"):.2f} on the red lines, {_ans("recipe-short", "input", "nonred"):.2f} on the non-red). But turning the embeddings by the same angle in a random direction costs at least as much ({_ans("recipe-short", "input-control", "red"):.2f}), so on that side the blocks seem to have learned where the `=` embedding sits. The pilot arms repeat the pattern. On `untied`, stripping the readout side costs the `=` prediction ({_eq("untied", "output"):.2f}) and stripping the embedding side costs little ({_ans("untied", "input", "red"):.2f} on the answer). On `syntax-off-axis` and `untied-line`, no strip changes anything ([figures](#where-the-component-works)).
    - **The embedding-component table.** The `=` embedding is at {_row["recipe-short"]:.2f} on `{ex.REFERENCE}` and {_row["t00"]:.2f} on `t00`; the op words are at {_ops["recipe-short"]:.2f} and {_ops["t00"]:.2f}. Anchoring the blocks only leaves the `=` embedding at {_row["blocks-only"]:.2f}. Untying the readout brings the `=` embedding down to {_row["untied"]:.2f}, and puts {_head["untied"]:.2f} on the readout vector. Under the whole-line labeller the `⏎` embedding takes the axis as well ({_nl["blocks-only-line"]:.2f} on `blocks-only-line`, {_head["untied-line"]:.2f} on the readout of `untied-line`). `syntax-off-axis` holds every syntax embedding at zero by construction. The red color embeddings themselves sit at {_red["recipe-short"]:.2f} on the reference, {_red["rows-clean"]:.2f} on `syntax-off-axis` and {_red["untied"]:.2f} on `untied`, but at {_red["blocks-only"]:.2f} on `blocks-only` and {_red["blocks-only-line"]:.2f} on `blocks-only-line`, where nothing pulls the embedding ([figure](#the-embedding-component-table)).
...
def _(res: Results):
    ...
    for _c in STORED + [a.name for a in ex.ARMS]:
        for _s in ex.STRIPS:
            _rows.append(
                [f"<code>{show(_c)}</code>" if _s == "clean" else "", STRIP_LABEL[_s].replace("\n", " ")]
    ...
    def _plot() -> plt.Figure:
        ...
        for i, c in enumerate(ROW_ARMS):
            ...
            for k, table in enumerate(tables):
                ...
                for word, marker, ms in ((EQ, "s", 4.5), (NL, r"^", 4.5)):
                ...
                v = res.red_mean(c, table)
                ax.plot(i + off, v.mean(), "v", ms=4, color=grey, mfc=grey if table == "rows" else "none")
        ax.set_xticks(_x, [show(a) for a in ROW_ARMS], fontsize=7, rotation=25, ha="right")
...
def _(res: Results):
    ...
    for _c in ROW_ARMS:
        _rows.append(
            ...
            + [span2(res.color_abs(_c), ".3f"), span2(res.red_mean(_c), ".2f")]
        )
        if not res.tied(_c):
            _rows.append(
                [
                ...
                + [span2(res.color_abs(_c, "rows_readout"), ".3f"), span2(res.red_mean(_c, "rows_readout"), ".2f")]
            )
    _head = ["arm", "table", "=", "⏎"] + [f"<code>{w}</code>" for w in OPS] + ["colors, mean |·|", "red colors, mean"]
    _caption = "The embedding-component table as numbers: the signed axis component of each syntax embedding, per arm and table, seed mean with half the seed range; the last two columns are the mean absolute component over the color embeddings and the mean signed component over the red color embeddings."
