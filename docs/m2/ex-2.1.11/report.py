# title: Ex 2.1.11: Ablations and a survey for the D2.1 operating point
...
def delta_cell_(text: str, resolved: bool) -> str:
...
def delta_table(summary: dict, arms: list[str], ref: str = "ref") -> str:
    ...
    for s in ex.DECISION_STATS:
        ...
        cells = "".join(
            delta_cell_(f"{d['delta']:+.4f}{MARK[d['verdict']]}", d["verdict"] != "within band") for d in stats
        )
        rows.append(
    ...
    rows.append(
        ...
        + "".join(delta_cell_(f"{v[a]['task_cost']:.4f}", not v[a]["task_ok"]) for a in arms)
        + "</tr>"
    )
    rows.append(
        f"<tr><td>latched runs ↓</td><td class='num'>{len(summary[ref]['latched'])}</td><td class='num'>—</td>"
        + "".join(delta_cell_(str(len(v[a]["latched"])), bool(v[a]["latched"])) for a in arms)
        + "</tr>"
    )
...
m10_ = prior["m10"]
primary_ = [c for c in m10_["cells"] if c["condition"] == "either-t100"]
probes_ = prior["probes"]
g1_, g2_ = group_weights(probes_["r1"], probes_["r2"])


def retention_(cell: dict) -> float:
    tr = np.asarray(cell["traj"]["m_line"], dtype=float)
    return float(tr[-1] / np.maximum.accumulate(tr).max())


def contrast_(label: str) -> float:
    w = prior["a10"][f"{label}/w_line"]  # (L1, N, SPAN)
    return float(np.einsum("lnt,n->lt", w, g2_)[1:, 2].mean() - np.einsum("lnt,n->lt", w, g1_)[1:, 2].mean())


def r2_(label: str) -> float:
...
per_run: dict[str, np.ndarray] = {
    "m_line": np.array([c["m_line"] for c in primary_]),
    "alpha_op1": np.array([c["alpha_mean_op1"] for c in primary_]),
    "retention": np.array([retention_(c) for c in primary_]),
    "r2_sim": np.array([r2_(c["label"]) for c in primary_]),
    "contrast": np.array([contrast_(c["label"]) for c in primary_]),
    "holdout_em": np.array([c["sets"]["named_holdout"]["accuracy"] for c in primary_]),
}
...
m10_ = prior["m10"]
conds_ = ["op1-labels", "either-t100", "either-t500", "either-t2500", "slot-oracle"]
pts_: dict[str, tuple[np.ndarray, np.ndarray]] = {}
for c_ in conds_:
    cells_ = [c for c in m10_["cells"] if c["condition"] == c_]
    m_ = np.array([c["m_line"] for c in cells_])
    r_ = np.array(
        [
...
def seed_means_(metrics: dict, stat: str) -> dict[str, float]:
...
def corr_(stat: str, sources: list[dict]) -> tuple[float, int]:
    m = [v for s in sources for v in seed_means_(s, stat).values()]
    a = [v for s in sources for v in seed_means_(s, "alpha_mean_op1").values()]
    return float(np.corrcoef(m, a)[0, 1]), len(m)


r_span_, n_span_ = corr_("m_span", [prior["m9"], prior["m10"]])
r_op1_, n_op1_ = corr_("m_op1", [prior["m8"], prior["m9"], prior["m10"]])

rf"""
{trade_html_}

Margin–containment correlation across the stored anchored operating points: $r$ = {r_span_:+.2f} over the {n_span_} ex-2.1.9/ex-2.1.10 points (m_span basis), and $r$ = {r_op1_:+.2f} over all {n_op1_} points including the ex-2.1.8 grid (m_op1 basis, the statistic all three share). No trade on either reading; both improve together along the design line. For scale, the primary's nine runs put per-run grading at {per_run["r2_sim"].mean():.2f} ± {per_run["r2_sim"].std(ddof=1):.3f} against {pts_["either-t2500"][1].mean():.2f} for the softest arm.
"""
...
m10_ = prior["m10"]
labels_ = [c["label"] for c in m10_["cells"] if c["condition"] == "either-t100"]
alpha_ = np.mean([prior["a10"][f"{la}/alpha_lines"] for la in labels_], axis=0)  # (L1, N, T), seed mean
w_color_ = prior["probes"]["weights"]
n_part_ = alpha_.shape[1] // len(w_color_)
x_ = 1.0 - alpha_[:, :, : ex.SPAN]
taus_ = np.geomspace(0.01, 1.0, 60)
...
lead_ = np.array(
    [
        np.einsum(
            "lct,c->lt",
            softmin_weights_np(x_, t).reshape(x_.shape[0], len(w_color_), n_part_, ex.SPAN).mean(axis=2),
            w_color_,
        ).max(axis=-1)
        for t in taus_
    ]
...
def tau_range_plot() -> plt.Figure:
    ...
    for li in range(lead_.shape[1]):
        ax.plot(taus_, lead_[:, li], color=slice_colors[li], lw=1.4, label=f"slice {li}")
...
e_ = np.linspace(0.0, float(ex.EPOCHS), 2001)
lr_ = ex.learning_rate(e_)
flat_ = np.ones_like(e_)
anchor_sched_ = ex.anchor_weight(e_)
anchor_flat_ = ex.SCORING_LAMBDA * flat_
anti_sched_ = ex.anti_weight(e_)
anti_hold_ = ex.ANTI_HOLD_RATIO * ex.SCORING_LAMBDA * flat_
...
def doses_plot() -> plt.Figure:
    grey = light_dark("#666", "#999")
    sched_c = light_dark("#1f6fb4", "#5fa8dd")
    flat_c = light_dark("#b4531f", "#dd8f5f")
    flat2_c = light_dark("#2a9d8f", "#5fc9bd")
    fig, axs_ = plt.subplots(2, 2, figsize=(7.2, 4.4), layout="constrained", sharex="col", sharey="row")
    (ax_aw, ax_sw), (ax_ad, ax_sd) = cast(AxesGrid, axs_)
    for ax, mult in ((ax_aw, flat_), (ax_ad, lr_)):
        ax.plot(e_, anchor_sched_ * mult, color=sched_c, lw=1.4, label="schedule")
        ax.plot(e_, anchor_flat_ * mult, color=flat_c, lw=1.4, ls=(0, (4, 2)), label="flat-anchor")
    for ax, mult in ((ax_sw, flat_), (ax_sd, lr_)):
        ax.plot(e_, anti_sched_ * mult, color=sched_c, lw=1.4, label="schedule")
        ax.plot(e_, anti_hold_ * mult, color=flat2_c, lw=1.4, ls=(0, (4, 2)), label="anti-hold")
        ax.plot(e_, anti_peak_ * mult, color=flat_c, lw=1.4, ls=(0, (1, 2)), label="anti-peak")
...
latch_rows_ = []
for c_ in m9_["cells"]:
    if c_["condition"] in ("lam0",) or c_.get("tau") is None:
        continue
    pi_ = np.asarray(c_["pi"])  # (L1, SPAN)
    deep_ = pi_[1:].mean(axis=0)
    syntax_ = float(max(deep_[1], deep_[3]))
    latch_rows_.append((c_["label"], deep_, syntax_))
...
latch_body_ = "".join(
    ...
    for la, deep, syn in latch_rows_
)
latch_table_ = f"""
<table class="report-table">
...
def invariants_(c: dict) -> tuple[float, float, float, float]:
    epochs: int = c["epochs"]
...
def fmt_(v: float, spec: str) -> str:
    return "—" if np.isnan(v) else format(v, spec)


cond_rows_ = []
for c_ in ex.ABLATION_CONDITIONS:
    ad_, ac_, sd_, sc_ = invariants_(c_)
    what_ = {
    ...
    cond_rows_.append(
        f"<tr><td><code>{c_['name']}</code></td><td>{what_}</td>"
        f"<td class='num'>{c_['epochs']}</td>"
        f"<td class='num'>{fmt_(ad_, '.4f')}</td><td class='num'>{fmt_(ac_, '.1f')}</td>"
        f"<td class='num'>{fmt_(sd_, '.4f')}</td><td class='num'>{fmt_(sc_, '.1f')}</td></tr>"
    )
cond_table_ = f"""
<table class="report-table">
...
def trial_table_(space: dict, sched: bool) -> str:
    trials = ex.sobol_trials(space)
    head = "".join(f"<th class='num'>{d}</th>" for d in space)
    extra = "<th class='num'>rel dose</th><th class='num'>centroid</th>" if sched else "<th class='num'>rel dose</th>"
    rows = []
    for t in trials:
...
trials_a_ = trial_table_({**ex.SPACE_COMMON, **ex.SPACE_ANTI_SCHED}, sched=True)
trials_b_ = trial_table_({**ex.SPACE_COMMON, **ex.SPACE_ANTI_FLAT}, sched=False)

figure_html(
    f"<details><summary>Branch A trial list (schedule retained)</summary>{trials_a_}</details>"
    f"<details><summary>Branch B trial list (flat anti)</summary>{trials_b_}</details>",
    caption=rf"""
...
figure_html(
    f"""
    <table class="report-table">
    <thead><tr><th>condition</th><th class="num">seeds</th>{ab_table_head_}
    <th class="num">holdout EM ↑</th><th class="num">latched ↓</th></tr></thead>
    <tbody>{ab_table_rows_}</tbody>
    </table>
    """,
    caption="""
...
def flat_anchor_plot() -> plt.Figure:
    grey = light_dark("#666", "#999")
    colors = {
    ...
    for name, runs in traj_flat_.items():
    ...
    for name in ("ref", "flat-anchor"):
        for i, p in enumerate(pi_by_cond_.get(name, [])):
            ax_p.scatter(
...
figure_html(
    ...
    caption=f"""
        **The two flat brackets against `ref`.** Read as the table above. The arms bracket the dose of the schedule rather than matching it: `anti-hold` delivers {dose_ref_["anti-hold"] / dose_ref_["ref"]:.2f}× and `anti-peak` {dose_ref_["anti-peak"] / dose_ref_["ref"]:.2f}× of what the schedule delivers. So an arm that holds every statistic while delivering a different dose tells us the dose was not the active ingredient.
    """,
    class_="report-figure",
)
...
flat_both_read_ = ab_decision["flat_both_read"]

figure_html(
    delta_table(ab_summary, ["flat-both"]),
    caption=f"""
        **The composition against `ref`.** Read as the tables above. Rule 3 {"reads this arm, because rules 1 and 2 both simplify" if flat_both_read_ is not None else "does not read this arm: rules 1 and 2 did not both simplify, so there is no composition to adopt"}; the numbers are shown either way, since a survey that publishes only the arms it acted on is not a complete map.
    """,
    class_="report-figure",
)
...
traj_short_keys_ = ("m_line", "val_loss")
...
def short_plot() -> plt.Figure:
    ...
    fig, axs_ = plt.subplots(1, 2, figsize=(7.2, 2.9), layout="constrained", sharex=True)
    ax_m, ax_v = cast(tuple, axs_)
    for ax, key in ((ax_m, "m_line"), (ax_v, "val_loss")):
        for name, runs in traj_short_[key].items():
            for i, (e, y) in enumerate(runs):
...
sched_dose_ = ex.anchor_dose(lambda e: ex.anti_weight(e))
lin_dose_ = ex.anchor_dose(lambda e: ex.anti_weight(e, shape="linear"))

figure_html(
    delta_table(ab_summary, ["linear"]),
    caption=f"""
        **`linear` against `ref`.** Read as the tables above. Straightening the ramps moves the anchor dose by under a percent, since a smoothstep and a straight line both cover half their window. It moves the anti-subspace dose by {abs(lin_dose_ - sched_dose_) / sched_dose_:.0%}, because that term interpolates over 90 epochs, and the two shapes differ most over the early part of that window, where the learning rate is highest.
    """,
    class_="report-figure",
)
...
def anti_invariants_(c: dict):
    e = c["epochs"]
    shape = ex.condition_shapes(c)[1]
    w = lambda e_, shape=shape, e0=e, c=c: ex.anti_weight(  # noqa: E731
        e_, lam=c["lam"], epochs=e0, shape=shape, hold_ratio=c.get("anti_ratio", ex.ANTI_HOLD_RATIO)
    )
    return ex.anchor_dose(w, epochs=e), ex.dose_centroid(w, epochs=e)


ref_dose_, _ = anti_invariants_(next(c for c in ex.ABLATION_CONDITIONS if c["name"] == "ref"))
amendment_what_ = {
...
amendment_rows_ = "".join(
    f"<tr><td><code>{c['name']}</code></td><td>{amendment_what_[c['name']]}</td>"
    f"<td class='num'>{c['epochs']}</td>"
    f"<td class='num'>{'—' if c['lam'] == 0 else format(anti_invariants_(c)[0] / ref_dose_, '.2f')}</td>"
    f"<td class='num'>{'—' if c['lam'] == 0 else format(anti_invariants_(c)[1] / c['epochs'], '.2f')}</td></tr>"
...
@themed(
    name="amendment-flow",
    alt_text=f"""
        Flowchart of amendment rules 6 to 8. A start box, round 1: flat anchor, scheduled anti, 50 epochs, feeds two lanes. Left lane: rule 6 compares anti-mid to ref. Resolved worse leads to branch A confirmed: scheduled anti, 4 dimensions, {n_a_} trials. Within band leads to rule 7, which compares anti-mid-flat to flat-anchor. There, within band leads to flat anchor with flat anti at {ex.ANTI_MID_RATIO:g}, 3 dimensions, {n_b_} trials; resolved worse leads to scheduled anchor with flat anti at {ex.ANTI_MID_RATIO:g}, also 3 dimensions, {n_b_} trials. Right lane: rule 8 compares short25 to ref, task-gated against short25-lam0. Both passing leads to 25 epochs; either failing leads to 50 epochs under rule 4.
    """,
    caption="""
...
def amendment_flow_plot() -> plt.Figure:
    ...
    box(12, 44, f"branch A confirmed:\nscheduled anti\n4 dims · {n_a_} trials", out_fc, out_ec)
...
anti_mid_flat_html_ = figure_html(
...
two_by_two_runs_ = {
    ...
    for k, cond in two_by_two_corners_.items()
}
...
def short25_plot() -> plt.Figure:
    ...
    ax_m, ax_v = cast(tuple, axs_)
    for ax, key in ((ax_m, "m_line"), (ax_v, "val_loss")):
        for name, runs in traj25_[key].items():
            for i, (e, y) in enumerate(runs):
                ax.plot(e / e.max(), y, color=colors[name], lw=1.0, alpha=0.8, label=name if i == 0 else None)
        ax.set_xlabel("fraction of training", fontsize="x-small")
        ax.set_ylabel(labels[key], fontsize="x-small")
...
survey_space_ = ex.survey_space(ab_decision)
survey_rows_ = "".join(
    f"<tr><td><code>{d}</code></td><td class='num'>{lo:g}</td><td class='num'>{hi:g}</td><td>{scale}</td></tr>"
    for d, (lo, hi, scale) in survey_space_.items()
)
...
sv_trials: dict[int, dict] = {t["trial"]: t for t in sv_["trials"]}
sv_cells: dict[str, dict] = {c["label"]: c for c in sv_["cells"]}
sv_promoted: list[int] = sv_["promoted"]
...
sv_hue = {t: HUES[zlib.crc32(str(t).encode()) % len(HUES)] for t in sv_scored}
...
def sv_swatch(t: int | None) -> str:
    ...
    light, dark = sv_hue[t]
    open_ = "" if sv_scored[t]["feasible"] else " sw-open"
    return f'<span class="sw sw-dot sw-themed{open_}" style="--sw-light: {light}; --sw-dark: {dark};" aria-hidden="true"></span>'
...
sv_epochs_ = ab_decision["epochs"]


def sv_centroid_frac_(tr: dict) -> float:
    w = lambda e: ex.anti_weight(  # noqa: E731
        e, lam=1.0, peak_ratio=tr["anti_peak_ratio"], anneal_end_frac=tr["anti_anneal_end_frac"], epochs=sv_epochs_
    )
    return ex.dose_centroid(w, epochs=sv_epochs_) / sv_epochs_


sv_coords = {
    t: {"dose": ex.anti_rel_dose(tr, epochs=sv_epochs_), "centroid": sv_centroid_frac_(tr)}
    for t, tr in sv_trials.items()
}
sv_control_em = ab_summary[ex.CONTROL_OF[sv_epochs_]]["holdout_em"]
sv_r2_floor = ab_summary["ref"]["r2_sim"] - ex.GRADE_R2_DROP

sv_infeasible_ = {t: s for t, s in sv_scored.items() if not s["feasible"]}
sv_fail_counts_ = {
    k: sum(1 for s in sv_infeasible_.values() if not s["checks"][k])
    for k in ("latch", "containment", "grading", "task", "contrast")
}
sv_veto_tau_ = max(sv_trials[t]["tau"] for t, s in sv_infeasible_.items() if not s["checks"]["latch"])
sv_task_lam_ = min(sv_trials[t]["lam"] for t, s in sv_infeasible_.items() if not s["checks"]["task"])

f"""
All {len(sv_trials)} trials ran; {len(sv_scored) - len(sv_infeasible_)} are feasible on the final reading and {len(sv_infeasible_)} are not. No constraint is decorative: the latch veto removes {sv_fail_counts_["latch"]} trials, containment {sv_fail_counts_["containment"]}, grading {sv_fail_counts_["grading"]}, the task gate {sv_fail_counts_["task"]}, and the contrast floor {sv_fail_counts_["contrast"]} (a trial can fail several).

The failures organize by region rather than scattering. The latch veto owns the cold edge: every vetoed trial has τ ≤ {sv_veto_tau_:.2f}. Grading and containment fail mostly where the pull is weak (λ_a ≲ 0.1), where the response is too control-like to show graded structure. The task gate binds only where the pull is strong: every task failure has λ_a ≥ {sv_task_lam_:.2f}.
...
def sv_cond_mean_(t: int) -> float | None:
    vals = [c["r2_cond"] for s in range(sv_scored[t]["n_seeds"]) if "r2_cond" in (c := sv_cells[f"t{t:02d}-s{s}"])]
    return float(np.mean(vals)) if vals else None


def sv_row_(t: int) -> str:
    tr, s = sv_trials[t], sv_scored[t]
    fails = ", ".join(k for k, ok in s["checks"].items() if not ok) or "—"
    name = f"t{t:02d}"
    if t in sv_promoted:
        name = f"<b>{name}</b>"
    cond = sv_cond_mean_(t)
    return (
        f"<tr><td>{sv_swatch(t)} {name}</td>"
        + "".join(
            f"<td class='num'>{tr[k]:.3f}</td>" for k in ("lam", "tau", "anti_peak_ratio", "anti_anneal_end_frac")
        )
        + f"<td class='num'>{sv_coords[t]['dose']:.2f}</td><td class='num'>{sv_coords[t]['centroid']:.2f}</td>"
        + f"<td class='num'>{s['n_seeds']}</td>"
        + "".join(f"<td class='num'>{s[k]:+.4f}</td>" for k in ("m_line", "alpha_op1", "retention", "r2_sim"))
        + f"<td class='num'>{'—' if cond is None else format(cond, '+.4f')}</td>"
        + f"<td class='num'>{s['contrast']:+.4f}</td><td class='num'>{s['holdout_em']:.4f}</td>"
        + f"<td class='num'>{s['latch_pi']:.2f}</td><td>{fails}</td></tr>"
    )


sv_rows_ = [sv_row_(t) for t in sorted(sv_trials)]
sv_head_stats_ = "".join(
    f"<th class='num'>{h}</th>"
    for h in (
        "m_line ↑",
...
sv_table_html_ = figure_html(
    ...
    <th class="num">seeds</th>{sv_head_stats_}<th>fails</th></tr></thead>
    <tbody>{"".join(sv_rows_)}</tbody>
    </table>
    """,
    caption="""
...
def sv_x_(t: int, key: str) -> float:
    return sv_trials[t][key] if key in sv_trials[t] else sv_coords[t][key]


sv_feas_ = [t for t, s in sv_scored.items() if s["feasible"]]
...
def marginals_plot() -> plt.Figure:
    ...
    trial_color_ = {t: light_dark(*h) for t, h in sv_hue.items()}
    ticks_ = {"lam": [0.02, 0.05, 0.1, 0.2, 0.5, 1.0], "tau": [0.05, 0.1, 0.2, 0.3], "dose": [0.3, 0.5, 1.0]}
    fig, axs = plt.subplots(
        len(sv_ROWS_), len(sv_COLS_), figsize=(8.4, 10.2), layout="constrained", sharex="col", sharey="row"
    )
    for i, (stat, sname, gates) in enumerate(sv_ROWS_):
        for j, (key, xname, scale) in enumerate(sv_COLS_):
            ax = axs[i][j]
            ax.scatter(
                [sv_x_(t, key) for t in sv_infeas_],
                [sv_scored[t][stat] for t in sv_infeas_],
                s=11,
                facecolors="none",
                edgecolors=[to_rgba(trial_color_[t], alpha=0.45) for t in sv_infeas_],
                lw=0.8,
            )
            ax.scatter(
                [sv_x_(t, key) for t in sv_feas_],
                [sv_scored[t][stat] for t in sv_feas_],
                s=12,
                color=[trial_color_[t] for t in sv_feas_],
                lw=0,
            )
            ax.scatter(
                [sv_x_(0, key)], [sv_scored[0][stat]], s=52, facecolors="none", edgecolors=trial_color_[0], lw=1.4
            )
            for g in gates:
                ax.axhline(g, color=c_gate, ls=(0, (4, 3)), lw=0.7)
            if scale == "log":
                ax.set_xscale("log")
                ax.set_xticks(ticks_[key])
...
sv_has_cond_ = any("r2_cond" in c for c in sv_cells.values())
sv_ref_cond_ = float(np.mean([ex.r2_sim_cond(ab_arrays[f"ref-s{s}/alpha"][:, :, 0].mean(axis=0)) for s in range(3)]))


def sv_cond_mean2_(t: int) -> float:
    return float(np.mean([sv_cells[f"t{t:02d}-s{s}"]["r2_cond"] for s in range(sv_scored[t]["n_seeds"])]))


sv_plane_pts_ = {
    t: (s["m_line"], s["r2_sim"], sv_cond_mean2_(t) if sv_has_cond_ else np.nan, s["feasible"])
...
def plane_plot() -> plt.Figure:
    from matplotlib.colors import to_rgba

    c_gate = light_dark("#666", "#999")
    c_ref = light_dark("#7b3fa0", "#b98ce0")
    trial_color_ = {t: light_dark(*h) for t, h in sv_hue.items()}
    ncols = 2 if sv_has_cond_ else 1
    fig, axs_ = plt.subplots(
        1, ncols, figsize=(7.2 if sv_has_cond_ else 4.2, 3.0), layout="constrained", sharex=True, sharey=True
    )
    axs = np.atleast_1d(axs_)
    for k, ax in enumerate(axs):
        yi = 1 if k == 0 else 2
        for t, (m, r, rc, feas) in sv_plane_pts_.items():
            y, c = (m, r, rc)[yi], trial_color_[t]
            if feas:
        ...
        ref_y_ = ab_summary["ref"]["r2_sim"] if yi == 1 else sv_ref_cond_
        ax.scatter([ab_summary["ref"]["m_line"]], [ref_y_], marker="x", s=44, color=c_ref, lw=1.6)
...
def lam_marginal_plot() -> plt.Figure:
    ...
    seen_ = set()
    for t, s in sv_scored.items():
        x, m, c = sv_trials[t]["lam"], s["m_line"], trial_color_[t]
        if s["feasible"]:
            ax.scatter([x], [m], s=16, color=c, lw=0)
            seen_.add("feasible")
        else:
            mk, mname = sv_MARKS_[sv_first_fail_(s)]
            if mk == "x":
            ...
            seen_.add(mname)
    ax.scatter(
        [sv_trials[0]["lam"]], [sv_scored[0]["m_line"]], s=64, facecolors="none", edgecolors=trial_color_[0], lw=1.4
    )
    keys_ = [("o", "feasible", True)] + [(mk, name, mk == "x") for mk, name in sv_MARKS_.values()]
    ax.legend(
        handles=[
            Line2D(
                [],
...
t0_, s0_ = sv_trials[0], sv_scored[0]
ref_ = ab_summary["ref"]


def pm_(stat: str, s: dict) -> str:
    return f"{s[stat]:.4f} ± {ex.NOISE_RUN[stat] / np.sqrt(s['n']):.4f}"


point_stats_ = ["m_line", "alpha_op1", "retention", "r2_sim", "contrast", "holdout_em"]
point_rows_ = (
    "<tr><td>proposed (<code>t00</code>)</td>"
    + f"<td class='num'>{t0_['lam']:.3f}</td><td class='num'>{t0_['tau']:.3f}</td>"
    + f"<td class='num'>{t0_['anti_peak_ratio']:.3f}</td><td class='num'>{t0_['anti_anneal_end_frac']:.3f}</td>"
    + f"<td class='num'>{sv_coords[0]['dose']:.2f}</td>"
    + "".join(f"<td class='num'>{pm_(s, {**s0_, 'n': s0_['n_seeds']})}</td>" for s in point_stats_)
    + "</tr>"
    ...
    + "".join(f"<td class='num'>{pm_(s, ref_)}</td>" for s in point_stats_)
    + "</tr>"
)
...
feas_ranked_ = sorted((t for t, s in sv_scored.items() if s["feasible"]), key=lambda t: -sv_scored[t]["m_line"])
plateau_band_ = ex.equiv_band("m_line", 5, 1)
plateau_top_ = [t for t in feas_ranked_ if sv_scored[t]["m_line"] >= sv_scored[0]["m_line"] - plateau_band_]
plateau_rng_ = lambda key: (  # noqa: E731
    min(sv_trials[t].get(key) or sv_coords[t][key] for t in plateau_top_),
    max(sv_trials[t].get(key) or sv_coords[t][key] for t in plateau_top_),
)
plateau_lam_, plateau_tau_, plateau_pk_, plateau_dose_ = (
    plateau_rng_("lam"), plateau_rng_("tau"), plateau_rng_("anti_peak_ratio"), plateau_rng_("dose"),
)  # fmt: skip
plateau_r2_others_ = [sv_scored[t]["r2_sim"] for t in plateau_top_[1:]]
...
The proposal is on a plateau rather than a peak. {len(plateau_top_)} trials land within one m_line band ({plateau_band_:.3f}, five seeds against one) of the proposal, spanning λ_a {plateau_lam_[0]:.2f}–{plateau_lam_[1]:.2f}, τ {plateau_tau_[0]:.2f}–{plateau_tau_[1]:.2f}, peak ratio {plateau_pk_[0]:.1f}–{plateau_pk_[1]:.1f} and relative dose {plateau_dose_[0]:.2f}–{plateau_dose_[1]:.2f}.

The runners-up among them hold {min(plateau_r2_others_) - sv_scored[0]["r2_sim"]:.2f}–{max(plateau_r2_others_) - sv_scored[0]["r2_sim"]:.2f} more grading at nearly the same margin (the plane figure above). So what the search found is the plateau together with the constraint that bounds it, and the proposal is one end of that plateau.
"""

promotion_rows_ = "".join(
...
veto_ = {t: s for t, s in sv_scored.items() if not s["checks"]["latch"]}
veto_weak_ = {t for t in veto_ if sv_trials[t]["lam"] <= 0.08}
veto_strong_ = sorted(set(veto_) - veto_weak_, key=lambda t: sv_trials[t]["lam"])

rf"""
### Where the vetoes actually landed

The vetoes from the survey sort by τ rather than by λ_a. All {len(veto_)} vetoed trials sit at τ ≤ {max(sv_trials[t]["tau"] for t in veto_):.2f}, the cold end of the box. {len(veto_weak_)} of them are the weak-pull corner the paragraph above anticipated (λ_a ≤ {max(sv_trials[t]["lam"] for t in veto_weak_):.2f}; one, `t29` at π = {sv_scored[29]["latch_pi"]:.2f}, also fails grading and contrast, which is the control-like signature).

The other {len(veto_strong_)} have healthy pulls, λ_a {sv_trials[veto_strong_[0]]["lam"]:.2f}–{sv_trials[veto_strong_[-1]]["lam"]:.2f}. Their π values, {min(veto_[t]["latch_pi"] for t in veto_strong_):.2f} to {max(veto_[t]["latch_pi"] for t in veto_strong_):.2f}, fall inside the range ex-2.1.9's calibration found empty, with latched runs at ≥ 0.85 and healthy runs at ≤ 0.03. Cold pooling concentrates the softmin weight, so a modest alignment with a syntax role reads as a large π there.
...
alts_ = sorted((t for t in sv_promoted if sv_scored[t]["feasible"]), key=lambda t: -sv_scored[t]["m_line"])
naming_band_ = {s: ex.equiv_band(s, 5, 5) for s in ("m_line", "r2_sim", "contrast")}
naming_spread_ = {s: max(sv_scored[t][s] for t in alts_) - min(sv_scored[t][s] for t in alts_) for s in naming_band_}

rf"""
### Naming the step-back points

The objective cannot rank the {len(alts_)} promoted trials that are still feasible on five-seed means. Their m_line values span only {naming_spread_["m_line"]:.3f}, or {naming_spread_["m_line"] / naming_band_["m_line"]:.1f} resolution bands at five seeds a side.

The same trials span {naming_spread_["r2_sim"]:.3f} of grading ({naming_spread_["r2_sim"] / naming_band_["r2_sim"]:.1f} bands) and {naming_spread_["contrast"]:.3f} of contrast ({naming_spread_["contrast"] / naming_band_["contrast"]:.0f} bands). So the frozen argmax ordered them by differences near its own noise floor, and the places where they differ measurably are the statistics it treats as constraints.
"""


def alt_pm_(t: int, stat: str) -> str:
...
alt_rows_ = "".join(
    ...
    + "".join(f"<td class='num'>{alt_pm_(t, s)}</td>" for s in ("m_line", "r2_sim", "contrast"))
...
knee_ = max(alts_, key=lambda t: sv_scored[t]["r2_sim"])
knee_band_ = {s: ex.equiv_band(s, 5, 5) for s in ("m_line", "r2_sim", "contrast")}
(twin_,) = [t for t in alts_[1:] if sv_scored[0]["m_line"] - sv_scored[t]["m_line"] < knee_band_["m_line"]]


def sig_(t: int) -> float:
...
- `t{twin_:02d}` is within one band of `t00` on m_line and on grading, so the search cannot tell the two apart on either. It holds {sv_scored[twin_]["contrast"] - sv_scored[0]["contrast"]:.2f} more contrast ({(sv_scored[twin_]["contrast"] - sv_scored[0]["contrast"]) / knee_band_["contrast"]:.0f} bands), and its grading margin is {sig_(twin_):.1f} per-run σ, against {sig_(0):.1f} for `t00`.
- `t{knee_:02d}` is the knee. Giving up {sv_scored[0]["m_line"] - sv_scored[knee_]["m_line"]:.3f} of m_line ({(sv_scored[0]["m_line"] - sv_scored[knee_]["m_line"]) / knee_band_["m_line"]:.1f} bands) brings grading back to the level of the reference recipe: {sv_scored[knee_]["r2_sim"]:.3f} against {ab_summary["ref"]["r2_sim"]:.3f} for `ref`, a floor margin of {sig_(knee_):.1f} σ. It also holds {sv_scored[knee_]["contrast"] - sv_scored[0]["contrast"]:.2f} more contrast than `t00`.

The margin matters because two of the six promoted trials crossed the task gate on their five-seed means. A grading pass of {sig_(0):.1f} σ per run faces the same draw again at the fresh seeds in D2.2.

None of this reopens the frozen rule, and `t00` stands as the proposal from the survey. The recommendation goes instead to the preregistration for D2.2, which is not yet frozen: adopt `t00`, `t{knee_:02d}`, and `t{twin_:02d}` as candidate points, re-measure all three at fresh seeds, and freeze there a selection rule that weighs grading and contrast alongside the margin.
