# title: Ex 2.1.5: disjoint vocabularies and more named colors
...
st = stats["n140-h216"]
palette_pts = np.array(list(st["palette"].values()), dtype=float) / 255
hex_ops_pts = (
    np.array([mv.lift((int(h[1], 16), int(h[2], 16), int(h[3], 16))) for h in st["hex_ops"]], dtype=float) / 255
)


@memo
def palettes_plot(pal: np.ndarray, ops: np.ndarray) -> str:
    @themed(
        name="palettes",
        alt_text="""
    ...
    def plot() -> plt.Figure:
        ...
        for ax, pts, title in [(axes[0], pal, "names (140)"), (axes[1], ops, "hex operands (216)")]:
            sv.plot_rgb_cube(ax, pts)
            ax.set_title(title)
        return fig
...
palettes_plot(palette_pts, hex_ops_pts)
...
grid8 = np.array([mv.lift((r, g, b)) for r in range(16) for g in range(16) for b in range(16)], dtype=float)


def spacing(pts: np.ndarray) -> tuple[float, float, float]:
    dd = np.linalg.norm(pts[:, None] - pts[None], axis=2)
    np.fill_diagonal(dd, np.inf)
    nn = dd.min(1)
    gap = np.linalg.norm(grid8[:, None] - pts[None], axis=2).min(1).max()
    return float(nn.min()), float(np.median(nn)), float(gap)


ops8 = np.array([mv.lift((int(h[1], 16), int(h[2], 16), int(h[3], 16))) for h in st["hex_ops"]], dtype=float)
hex_spacing, names_spacing = spacing(ops8), spacing(np.array(list(st["palette"].values()), dtype=float))
draws = np.array(
    [spacing(grid8[np.random.default_rng(1000 + i).permutation(len(grid8))[: len(ops8)]]) for i in range(24)]
)
typical_spacing = np.median(draws, axis=0)
better_gap = float((draws[:, 2] > hex_spacing[2]).mean())
...
def hex_dense_agreement(target: str) -> tuple[float, float]:
    a = np.mean([arrays[f"center-s{s}/probes/hex/{target}/r2_strict"] for s in SEEDS], axis=0)
    b = np.mean([arrays[f"hex-dense-s{s}/probes/hex/{target}/r2_strict"] for s in SEEDS], axis=0)
    return float(np.corrcoef(a.ravel(), b.ravel())[0, 1]), float(np.abs(a - b).mean())


hex_dense_corrs = {t: hex_dense_agreement(t) for t in ("op1", "op2", "mix")}
corr_lo, corr_hi = min(c for c, _ in hex_dense_corrs.values()), max(c for c, _ in hex_dense_corrs.values())
dmax = max(d for _, d in hex_dense_corrs.values())

rf"""
/// details | Is the clumping in the hex panel a problem?
It is conspicuous next to the names, and it is exactly what the two sampling rules predict: farthest-point selection cannot leave a clump, and uniform sampling clumps at every scale. Nearest-neighbor distances in 8-bit units (closest pair, then median) are {hex_spacing[0]:.0f} and {hex_spacing[1]:.0f} for the hex operands, against {names_spacing[0]:.0f} and {names_spacing[1]:.0f} for the names and {typical_spacing[0]:.0f} and {typical_spacing[1]:.0f} for a typical fresh draw of {len(ops8)} grid points. So the local spacing of the subset matches the median draw; it is not an unlucky one. Its coverage is if anything better than average: the farthest any of the 4,096 grid points sits from an operand is {hex_spacing[2]:.0f}, against {typical_spacing[2]:.0f} for the typical draw, and {better_gap * 100:.0f}% of draws leave a wider hole.

Whether that biases the *geometry* is answered by the sweep. The *hex-dense* condition draws 2,048 operands from the same grid — a tenth of the spacing, and no visible clumping — and its hex probe map is the same map: the two conditions' depth × landmark maps correlate {corr_lo:.2f} to {corr_hi:.2f} across the three targets, with a mean absolute difference of at most {dmax:.2f} in $R^2$. The two conditions also answer held-out hex equations equally well (see the density arms under H1). Nothing about the layout of the hex form appears to be an artifact of the 216-point draw, so the same should hold for what transfers out of it.
///
"""


def corpus_stats_row(key: str) -> str:
    s = stats[key]
    vals = [
        f"{s['n_lines']['named']:,}",
        f"{s['n_lines']['hex']:,}",
        ...
        f"{s['n_holdout']['named']:,}",
        f"{s['n_holdout']['hex']:,}",
        f"{max(v['max'] for v in s['line_length'].values())}",
        f"{s['answer_perplexity']:.0f}",
        f"{s['answers_reachable']}",
    ]
    return f"<tr><td>{key}</td>" + "".join(f'<td class="num">{v}</td>' for v in vals) + "</tr>"
...
corpus_thead = (
    f"<tr><th>{corpus_cols[0]}</th>" + "".join(f'<th class="num">{h}</th>' for h in corpus_cols[1:]) + "</tr>"
)

figure_html(
    '<table class="report-table">' + corpus_thead + "".join(corpus_stats_row(k) for k in sorted(stats)) + "</table>",
    caption="""
...
params_rows = "".join(
    f"<tr><td>{a}</td>"
    + f'<td class="num">{ARMS[a]["width"]}</td>'
    + f'<td class="num">{ARMS[a]["depth"]}</td>'
    + f'<td class="num">{cells[f"{a}-s0"]["n_params"]:,}</td>'
    + "</tr>"
    for a in ARMS
)
...
figure_html('<table class="report-table">' + params_thead + params_rows + "</table>")
...
@memo
def loss_curves_plot(cells: dict) -> str:
    @themed(
        name="loss-curves",
        alt_text="""
    ...
    def plot() -> plt.Figure:
        fig, axes = plt.subplots(2, 4, figsize=(9.0, 4.4), sharex=True, sharey=True)
        c = light_dark("#1a5f8a", "#6ab0d4")
        for ax, arm in zip(axes.flat, ARMS, strict=True):
            for s in SEEDS:
                cell = cells[f"{arm}-s{s}"]
                ax.plot([v for v in cell["train_loss"] if v is not None], color=c, lw=0.8, alpha=0.35)
                ax.plot([v for v in cell["val_loss"] if v is not None], color=c, lw=1.0)
...
def accuracy_row(set_name: str) -> str:
    nulls = stats["n140-h216"]["nulls"]
    vals = [
        f"{seed_mean('center', set_name, 'accuracy'):.3f}",
        f"{seed_mean('center', set_name, 'guess_dist'):.3f}",
        f"{seed_mean('center', set_name, 'floor_dist'):.3f}",
        f"{nulls[set_name]['blind']['acc']:.3f}",
        f"{nulls[set_name]['knn']['k2']['acc']:.3f}",
    ]
    return f"<tr><td>{set_name.replace('_', ' ')}</td>" + "".join(f'<td class="num">{v}</td>' for v in vals) + "</tr>"


accuracy_cols = ["eval set", "exact", "guess dist", "floor dist", "blind null", "2-NN null"]
accuracy_thead = (
    f"<tr><th>{accuracy_cols[0]}</th>" + "".join(f'<th class="num">{h}</th>' for h in accuracy_cols[1:]) + "</tr>"
)

figure_html(
    '<table class="report-table">'
    + accuracy_thead
    + "".join(accuracy_row(s) for s in ("named_seen", "named_holdout", "hex_seen", "hex_holdout"))
    + "</table>",
    caption="""
...
@memo
def miss_ranks_plot(arrays: dict) -> str:
    @themed(
        name="miss-ranks",
        alt_text="""
    ...
    def plot() -> plt.Figure:
        ...
        cap = 6
        for ax, set_name, title in [
            (axes[0], "named_holdout", "named held-out"),
            (axes[1], "hex_holdout", "hex held-out"),
        ]:
            r = np.concatenate([arrays[f"center-s{s}/evals/{set_name}/rank"] for s in SEEDS])
            r = r[r >= 0]
            counts = np.bincount(np.minimum(r, cap), minlength=cap + 1) / len(r)
            ax.bar(range(cap + 1), counts, color=light_dark("#1a5f8a", "#6ab0d4"))
            ax.set_xticks(range(cap + 1), [*map(str, range(cap)), f"{cap}+"])
            ax.set_title(title)
            ax.set_xlabel("guess rank (0 = correct)")
...
miss_ranks_plot(arrays)

# %%

corpus_of = {"center": "n140-h216", "hex-dense": "n140-h2048", "palette-250": "n250-h216"}
density_thead = (
    "<tr><th>condition</th>"
...
density_rows = "".join(
    f"<tr><td>{a}</td>"
    + f'<td class="num">{seed_mean(a, "named_holdout", "accuracy"):.3f}</td>'
    + f'<td class="num">{seed_mean(a, "hex_holdout", "accuracy"):.3f}</td>'
    + f'<td class="num">{stats[k]["answer_perplexity"]:.0f}</td>'
    + "</tr>"
    for a, k in corpus_of.items()
)

figure_html(
    '<table class="report-table">' + density_thead + density_rows + "</table>",
    caption="""
...
precision_rng = np.random.default_rng(0)


def precision_behavior(arm: str, corpus: str, set_name: str) -> dict:
    pal = np.array(list(stats[corpus]["palette"].values()), dtype=float) / 255

    def stack(key: str) -> np.ndarray:
        return np.concatenate([arrays[f"{arm}-s{s}/evals/{set_name}/{key}"] for s in SEEDS]).astype(float) / 255

    mix, true, guess = stack("mix_value"), stack("true_value"), stack("guess_value")
    true_idx = np.linalg.norm(pal[None] - true[:, None], axis=2).argmin(1)
    hit = np.all(np.abs(guess - true) < 1e-9, axis=1)
    return {
        "pal": pal,
        "mix": mix,
        "true_idx": true_idx,
        "hit": hit,
        "margin": bl.snap_margin(np.linalg.norm(pal[None] - mix[:, None], axis=2)),
    }
...
def fit_sigma(arm: str, corpus: str, set_name: str) -> dict:
    ...
    b = precision_behavior(arm, corpus, set_name)
    b["sigma"] = bl.fit_precision(b["pal"], b["mix"], b["true_idx"], b["hit"].mean(), precision_rng, 24, 11)
    return b


precision = {}
for arm, corpus in (("center", "n140-h216"), ("palette-250", "n250-h216")):
    for set_name in ("named_seen", "named_holdout"):
        b = fit_sigma(arm, corpus, set_name)
        b["pred"] = bl.precision_limited_acc(b["pal"], b["mix"], b["true_idx"], b["sigma"], precision_rng, 120)
        precision[arm, set_name] = b
...
center_p, palette250_p = precision["center", "named_holdout"], precision["palette-250", "named_holdout"]
standardized = float(
    bl.precision_limited_acc(
        palette250_p["pal"],
        palette250_p["mix"],
        palette250_p["true_idx"],
        center_p["sigma"],
        np.random.default_rng(1),
        160,
    ).mean()
)
per_pair = {k: stats[k]["n_lines"]["named"] / stats[k]["n_seen_distinct"]["named"] for k in ("n140-h216", "n250-h216")}
...
First, answering correctly became harder. The median gap between the nearest and second-nearest name to the exact mix falls from {np.median(center_p["margin"]):.3f} to {np.median(palette250_p["margin"]):.3f} of the unit cube, so more prompts ask the model to place the mix inside a tighter boundary. A reference guesser with the precision of the *center* condition, answering the 250-name prompts, predicts {standardized:.3f}. The observed score is {palette250_p["hit"].mean():.3f}, so roughly half of the drop from {center_p["hit"].mean():.3f} can be attributed to the tighter precision demand.

Second, each training pair got less supervision. The same ≈ 50,000 named lines now spread over {stats["n250-h216"]["n_seen_distinct"]["named"]:,} distinct training pairs instead of {stats["n140-h216"]["n_seen_distinct"]["named"]:,}, which is {per_pair["n250-h216"]:.1f} lines per pair against {per_pair["n140-h216"]:.1f}. One sign of this is that the gap between seen and held-out accuracy nearly closes at 250 names ({precision["palette-250", "named_seen"]["hit"].mean():.3f} against {palette250_p["hit"].mean():.3f}), where the center condition runs {precision["center", "named_seen"]["hit"].mean():.3f} against {center_p["hit"].mean():.3f}. Fitted precision agrees: on *seen* pairs it is {precision["palette-250", "named_seen"]["sigma"]:.3f} at 250 names, the same as what the center condition manages on pairs it has never seen ({center_p["sigma"]:.3f}). At 140 names a training pair recurs often enough to be remembered; at 250 it barely does.

Neither reading needs a capacity limit, and the residual gap after standardizing ({palette250_p["hit"].mean():.3f} against {standardized:.3f}) is small enough that three seeds cannot separate a genuine precision cost from the seed spread.
...
@memo
def probe_channels_plot(arrays: dict) -> str:
    @themed(
        name="probe-channels",
        alt_text="""
    ...
    def plot() -> plt.Figure:
        ...
        # exploratory section.
        stacks = {
            (f, t): np.stack([arrays[f"center-s{s}/probes/{f}/{t}/r2_strict_ch"] for s in SEEDS])
            for f in vp.FORMS
            for t in vp.TARGETS
        }
        return vp.probe_trace_grid(stacks, form_axis="row", width=10.5, panel_height=0.4, ylabel="probe R² per depth")

    return plot()


probe_channels_plot(arrays)
...
lm_index = {n: i for i, n in enumerate(LANDMARKS)}


def h2_site_value(form: str, target: str, key: str, landmark: str, ch: int | None, depth: int) -> float:
    m = np.mean([arrays[f"center-s{s}/probes/{form}/{target}/{key}"] for s in SEEDS], axis=0)
    cell_ = m[depth, lm_index[landmark]]
    return float(cell_[ch] if ch is not None else np.mean(cell_))


h2_sites_thead = (
    "<tr><th>site</th>"
    + "".join(f'<th class="num">{h}</th>' for h in ("per-equation", "strict", "difference"))
    + "</tr>"
)
h2_sites_rows = ""
for f_, t_, lmk_, ch_, desc_ in h2_sites:
    d_ = 0 if "embedding" in desc_ else 4
    e_ = h2_site_value(f_, t_, "r2_ch", lmk_, ch_, d_)
    s_ = h2_site_value(f_, t_, "r2_strict_ch", lmk_, ch_, d_)
    h2_sites_rows += (
        f"<tr><td>{desc_}</td>"
        + f'<td class="num">{e_:+.3f}</td><td class="num">{s_:+.3f}</td>'
        + f'<td class="num">{s_ - e_:+.3f}</td></tr>'
    )

figure_html(
    '<table class="report-table">' + h2_sites_thead + h2_sites_rows + "</table>",
    caption="""
...
all_cross = np.concatenate(
    [
        arrays[f"center-s{s}/cross/{d}/{t}/r2"].ravel()
        for s in SEEDS
        for d in ("hex2name", "name2hex")
        for t in vp.TARGETS
    ]
)
pre_idx = LANDMARKS.index("pre")
named_pre = float(np.mean([arrays[f"center-s{s}/probes/named/mix/r2_strict"][4, pre_idx] for s in SEEDS]))
named_pre_eq = float(np.mean([arrays[f"center-s{s}/probes/named/mix/r2"][4, pre_idx] for s in SEEDS]))
carried_pre = float(np.mean([arrays[f"center-s{s}/cross/hex2name/mix/r2"][4, pre_idx] for s in SEEDS]))

rf"""
Zero is a stronger statement than it looks. $\rho$ clips negative cross-form $R^2$ to zero, so in principle a floor reading could hide a score just below break-even. Nothing here is close: all {len(all_cross):,} cross-form cells in the scan are negative, and the best of them is ${all_cross.max():.3f}$. Take the pre-answer position in the last layer as an example. The mix probe fitted on the named form reads {named_pre:.2f} there; the hex probe applied to the same activations reads ${carried_pre:.0f}$. That is far worse than answering with the training mean, which suggests the decoder is pointing in an unrelated direction.
...
The result is the same either way, since the numerator is what fails and it is unchanged: the preregistered ratios are zero too. The estimators do differ in which cells clear the guard on $\rho$, and both are reported below. At the pre-answer site the named form reads {named_pre:.2f} strict against {named_pre_eq:.2f} per-equation, so the H3 headline site is one of the places they agree.
///
"""

cross_applied = {"named": "hex2name", "hex": "name2hex"}


@memo
def cross_form_transfer_plot(arrays: dict) -> str:
    @themed(
    ...
    def plot() -> plt.Figure:
        within = {
            (f, t): np.stack([arrays[f"center-s{s}/probes/{f}/{t}/r2_strict"] for s in SEEDS])
            for f in vp.FORMS
            for t in vp.TARGETS
        }
        cross_ = {
            (f, t): np.stack([arrays[f"center-s{s}/cross/{cross_applied[f]}/{t}/r2"] for s in SEEDS])
            for f in vp.FORMS
            for t in vp.TARGETS
        }
        return vp.transfer_trace_grid(within, cross_, width=10.5, panel_height=0.4, ylabel="probe R² per depth")

    return plot()


cross_form_transfer_plot(arrays)
...
stored_site = cells["center-s0"]["alignment"]["primary_site"]


def alignment_mean(path: str, depth: int, landmark: str) -> float:
    return float(np.mean([arrays[f"center-s{s}/{path}"][depth, lm_index[landmark]] for s in SEEDS]))


def rho_at(direction: str, into: str, depth: int, landmark: str, key: str = "r2_strict") -> str:
    ...
    cross_ = np.mean([arrays[f"center-s{s}/cross/{direction}/mix/r2"] for s in SEEDS], axis=0)
    within_ = np.mean([arrays[f"center-s{s}/probes/{into}/mix/{key}"] for s in SEEDS], axis=0)
    v = gm.rho(cross_, within_)[depth, lm_index[landmark]]
    return "—" if np.isnan(v) else f"{v:.2f}"


def alignment_row(label: str, depth: int, landmark: str) -> str:
    angles = np.mean([arrays[f"center-s{s}/angles/mix"][depth, lm_index[landmark]] for s in SEEDS], axis=0)
    vals = [
        f"{alignment_mean('probes/named/mix/r2_strict', depth, landmark):.2f}",
        f"{alignment_mean('probes/hex/mix/r2_strict', depth, landmark):.2f}",
        f"{alignment_mean('cross/hex2name/mix/r2', depth, landmark):+.2f}",
        rho_at("hex2name", "named", depth, landmark),
        f"{alignment_mean('cross/name2hex/mix/r2', depth, landmark):+.2f}",
        rho_at("name2hex", "hex", depth, landmark),
        " · ".join(f"{a:.0f}°" for a in angles),
    ]
    return f"<tr><td>{label}</td>" + "".join(f'<td class="num">{v}</td>' for v in vals) + "</tr>"


alignment_cols = ("within named", "within hex", "cross → named", "ρ → named", "cross → hex", "ρ → hex", "angles")
alignment_thead = "<tr><th>site (mix)</th>" + "".join(f'<th class="num">{h}</th>' for h in alignment_cols) + "</tr>"
alignment_rows = alignment_row(
    f"depth {stored_site['depth']}, {stored_site['landmark']} (preregistered)",
    stored_site["depth"],
    stored_site["landmark"],
) + alignment_row("depth 4, pre-answer (clean ground)", 4, "pre")
closest_angle = float(np.mean([arrays[f"center-s{s}/angles/mix"][4, lm_index["pre"]] for s in SEEDS], axis=0).min())

figure_html(
    '<table class="report-table">' + alignment_thead + alignment_rows + "</table>",
    caption=rf"""
        The mix probe at two sites, seed-averaged; within-form readings use the strict holdout. The preregistered per-equation ratios give the same reading: {rho_at("hex2name", "named", stored_site["depth"], stored_site["landmark"], "r2")} and {rho_at("name2hex", "hex", stored_site["depth"], stored_site["landmark"], "r2")} at the preregistered site, {rho_at("hex2name", "named", 4, "pre", "r2")} and {rho_at("name2hex", "hex", 4, "pre", "r2")} at the pre-answer site.
    """,
)

rf"""
Neither site was picked using $\rho$. The preregistered site maximizes the within-form $R^2$ of the weaker form, so the two probes are compared where both have something to carry. The pre-answer site is where the geometry of the named form is strongest and where the answer text cannot contaminate either form. The two sites disagree about where hex is legible: at the pre-answer site, the within-form hex reading falls under the guard on $\rho$, so the ratio into hex reports nothing there rather than zero. They agree about everything that bears on H3. The pre-answer site is also where the two mix decoders come closest to sharing a direction, and even there the smallest of the three principal angles is {closest_angle:.0f}°.
...
angle_rng = np.random.default_rng(0)


def first_angle(wa: np.ndarray, wb: np.ndarray) -> float:
    return float(gm.principal_angles(wa, wb)[0])
...
def random_angle_null(width: int, n: int = 2000) -> tuple[float, float]:
    ...
    """
    pairs = angle_rng.normal(size=(n, 2, width, 3))
    a = gm.principal_angles(pairs[:, 0], pairs[:, 1])[:, 0]
...
def angles_at(arm: str, target: str = "mix") -> dict[str, float]:
    ...
    """
    d, w = ARMS[arm]["depth"], ARMS[arm]["width"]
    pre_ = LANDMARKS.index("pre")
    wt = {(f, s): arrays[f"{arm}-s{s}/probes/{f}/{target}/weights"][d, pre_] for f in vp.FORMS for s in SEEDS}
    mean_, sd_ = random_angle_null(w)
    return {
        "cross_form": float(np.mean([first_angle(wt["named", s], wt["hex", s]) for s in SEEDS])),
        "seed_control": float(
            np.mean([first_angle(wt["named", a], wt["named", b]) for a, b in ((0, 1), (0, 2), (1, 2))])
        ),
        "random": mean_,
        "random_sd": sd_,
    }
...
def rho_summary(arm: str) -> dict:
    ...
    """
    wn, wh = maps(arm, "probes/named/mix/r2"), maps(arm, "probes/hex/mix/r2")
    site = np.unravel_index(np.minimum(wn, wh).argmax(), wn.shape)
    at, best, guarded, total = {}, -np.inf, 0, 0
    for dir_, into in (("hex2name", "named"), ("name2hex", "hex")):
        for t in vp.TARGETS:
            r = gm.rho(maps(arm, f"cross/{dir_}/{t}/r2"), maps(arm, f"probes/{into}/{t}/r2_strict"))
            guarded += int((~np.isnan(r)).sum())
            total += r.size
            if not np.isnan(r).all():
                best = max(best, float(np.nanmax(r)))
            if t == "mix":
                at[into] = float(r[site])
    return {"site": site, "at": at, "best": best, "guarded": guarded, "total": total}


rho_all = {a: rho_summary(a) for a in ARMS}
rho_best = max(r["best"] for r in rho_all.values())
rho_guarded = sum(r["guarded"] for r in rho_all.values())
rho_total = sum(r["total"] for r in rho_all.values())

rf"""
## Alignment under compression (H4)

H4 predicted that narrowing the residual stream would push the two forms together: $\rho$ and subspace overlap rising over d64 → d32 → d16, with d16-L8 the most aligned condition, but that didn't happen. $\rho$ is {rho_best:.2f} at every one of the {rho_guarded:,} cells where the guard defines it across the whole sweep ({rho_total:,} cells before gating), in both directions and for all three targets.
...
widths_by_shape = {(ARMS[a]["width"], ARMS[a]["depth"]): a for a in ARMS if not ARMS[a]["bridge"]}
widths_l4 = {w: widths_by_shape[w, 4] for w in (16, 32, 64)}
widths_l8 = {w: widths_by_shape[w, 8] for w in (16, 32, 64) if (w, 8) in widths_by_shape}


def width_acc(arm: str, set_name: str) -> float:
    return float(np.mean([cells[f"{arm}-s{s}"]["sets"][set_name]["accuracy"] for s in SEEDS]))


def width_within(arm: str) -> float:
    return float(maps(arm, "probes/named/mix/r2_strict")[ARMS[arm]["depth"], LANDMARKS.index("pre")])


def compression_plot(l4: dict, l8: dict) -> str:
    @themed(
        name="compression",
        alt_text="""
    ...
    def plot() -> plt.Figure:
        ...
        named_c = light_dark("#1a5f8a", "#6ab0d4")
        hex_c = light_dark("#6a4f8a", "#b79ad6")
        amber, ref = vp.transfer_color(), light_dark("#d1495b", "#ff6b7d")

        def series(ax: plt.Axes, fn, color, label) -> None:
            ax.plot(widths, [fn(l4[w]) for w in widths], "o-", color=color, lw=1.4, ms=5, label=label)
            ax.plot(list(l8), [fn(l8[w]) for w in l8], "s:", mfc="none", color=color, lw=0.8, ms=6)

        axes[0].set_title("accuracy")
        series(axes[0], lambda a: width_acc(a, "hex_holdout"), hex_c, "hex")
        series(axes[0], lambda a: width_acc(a, "named_holdout"), named_c, "named")
        axes[0].set(ylabel="held-out exact match", ylim=(0, 1.05))
        axes[0].legend(fontsize=7, loc="center left", framealpha=0.6)

        axes[1].set_title("probe")
        series(axes[1], width_within, named_c, "named")
        axes[1].set(ylabel="named mix $R^2$, pre-answer", ylim=(0, 1.05))

        axes[2].set_title("rotation")
        ang = {w: angles_at(l4[w]) for w in widths}
        mean_ = np.array([ang[w]["random"] for w in widths])
        sd_ = np.array([ang[w]["random_sd"] for w in widths])
        axes[2].fill_between(widths, mean_ - sd_, mean_ + sd_, color=vp.mean_color(), alpha=0.2, lw=0)
        axes[2].plot(widths, mean_, color=vp.mean_color(), lw=1, alpha=0.7, label="random 3-planes")
        series(axes[2], lambda a: angles_at(a)["seed_control"], ref, "seed control")
        series(axes[2], lambda a: angles_at(a)["cross_form"], amber, "named vs hex")
        axes[2].set(ylabel="first principal angle", ylim=(0, 95))
        axes[2].yaxis.set_major_formatter(lambda v, p: f"{v:.0f}°")
        axes[2].legend(fontsize=7, loc="lower right", framealpha=0.6)
        ...
        for ax in axes:
            ...
            ax.set(xlabel="residual width", xticks=widths)
...
compression_plot(widths_l4, widths_l8)

# %%

width_order = [a for a in ARMS if not ARMS[a]["bridge"] and ARMS[a]["names"] == 140 and ARMS[a]["hex"] == 216]
width_pre = LANDMARKS.index("pre")


def width_row(arm: str) -> str:
    r, g = rho_summary(arm), angles_at(arm)
    ...
    vals = [
        f"{ARMS[arm]['width']}",
        f"{ARMS[arm]['depth']}",
        f"{np.mean([cells[f'{arm}-s{s}']['sets']['named_holdout']['accuracy'] for s in SEEDS]):.3f}",
        f"{maps(arm, 'probes/named/mix/r2_strict')[ARMS[arm]['depth'], width_pre]:.2f}",
        fmt(r["at"]["named"]),
        fmt(r["at"]["hex"]),
        f"{r['best']:.2f}",
        f"{g['cross_form']:.0f}°",
    ...
    return f"<tr><td>{arm}</td>" + "".join(f'<td class="num">{v}</td>' for v in vals) + "</tr>"
...
width_thead = "<tr><th>condition</th>" + "".join(f'<th class="num">{h}</th>' for h in width_cols) + "</tr>"

figure_html(
    '<table class="report-table">' + width_thead + "".join(width_row(a) for a in width_order) + "</table>",
    caption="""
...
sig_widths = {a: fit_sigma(a, "n140-h216", "named_holdout")["sigma"] for a in ("center", "d32", "d16", "d16-L8")}
angles_widths = {a: angles_at(a) for a in ("center", "d32", "d16")}

rf"""
The angle does fall as the stream narrows, from {angles_widths["center"]["cross_form"]:.0f}° at d64 to {angles_widths["d32"]["cross_form"]:.0f}° at d32 and {angles_widths["d16"]["cross_form"]:.0f}° at d16. That *looks* like the trend H4 predicted, but it's not: two 3-dimensional subspaces drawn at random come just as close once the space they sit in is small enough. The same three widths give {angles_widths["center"]["random"]:.0f}°, {angles_widths["d32"]["random"]:.0f}° and {angles_widths["d16"]["random"]:.0f}° for a random pair. Every observed angle sits inside one standard deviation of its null, and the seed control (the *named* probes of two seeds, decoding the same quantity in unrelated bases) tracks the same curve. So the cross-form angle at every width is what two unrelated decoders would give.

Under compression the two forms don't merge; one of them gives way first. Hex keeps its held-out accuracy at {seed_mean("d32", "hex_holdout", "accuracy"):.2f} at d32 and loses little even at d16 ({seed_mean("d16", "hex_holdout", "accuracy"):.2f}), while named falls from {seed_mean("center", "named_holdout", "accuracy"):.2f} to {seed_mean("d32", "named_holdout", "accuracy"):.2f} to {seed_mean("d16", "named_holdout", "accuracy"):.2f}. In terms of the resolution account from H1, the fitted precision $\sigma$[^sigma] coarsens from {sig_widths["center"]:.3f} of the unit cube at d64 to {sig_widths["d32"]:.3f} at d32 and {sig_widths["d16"]:.3f} at d16: roughly an eightfold loss of color resolution over two halvings of width. Hex needs far less resolution, because the runner-up to a hex answer is a whole grid step away, so a coarse read still lands on the right digit.
...
assert cross is not None, "the bridge condition's cross-form eval hasn't been written yet"
bridge_n = cross["n_pairs"]
bridge_acc = {
    s: float(np.mean([c["sets"][s]["accuracy"] for c in cross["cells"]])) for s in ("cross_seen", "cross_unseen")
}
bridge_share = stats["n140-h216-bridge"]["n_lines"]["cross"] / sum(stats["n140-h216-bridge"]["n_lines"].values())
...
That would be an easy result to dismiss if the bridge had simply failed to learn anything, so let's check whether the supervision worked. Cross lines are {bridge_share * 100:.0f}% of the bridge corpus: {bridge_n["n_lines"]:,} of them, drawn over {bridge_n["seen"]:,} distinct (name, hex) pairs out of {bridge_n["seen"] + bridge_n["unseen"]:,} possible, so a pair recurs about {bridge_n["n_lines"] / bridge_n["seen"]:.1f} times and two thirds of the cross space is never seen at all. The sweep doesn't grade them, because the form exists to supervise alignment rather than to be scored, so we decoded them afterwards from the published checkpoints.[^crosseval]

The model does learn to answer bridge-form equations. Exact match is {bridge_acc["cross_seen"]:.2f} on cross pairs that appeared in training and {bridge_acc["cross_unseen"]:.2f} on pairs that did not, with no malformed completions; at 1.2 lines per pair there is not much for the model to memorize, and the two numbers say it didn't. Given the unseen prompt `pale lime + #fc7 = `, all three seeds return `#dd7`, which is the right answer. So the value of a color name does reach the hex answer path, for names and hex codes the model has never seen together.
...
def bridge_row(arm: str) -> str:
    r, g = rho_summary(arm), angles_at(arm)

    def fmt(v: float) -> str:
        return "—" if np.isnan(v) else f"{v:.2f}"

    bridged = ARMS[arm]["bridge"]
    vals = [
        f"{seed_mean(arm, 'named_holdout', 'accuracy'):.3f}",
        f"{seed_mean(arm, 'hex_holdout', 'accuracy'):.3f}",
        f"{bridge_acc['cross_unseen']:.3f}" if bridged else "—",
        f"{maps(arm, 'probes/named/mix/r2_strict')[4, width_pre]:.2f}",
        fmt(r["at"]["named"]),
        fmt(r["at"]["hex"]),
        f"{r['best']:.2f}",
        f"{g['cross_form']:.0f}°",
...
bridge_thead = "<tr><th>condition</th>" + "".join(f'<th class="num">{h}</th>' for h in bridge_cols) + "</tr>"

figure_html(
    '<table class="report-table">' + bridge_thead + bridge_row("center") + bridge_row("bridge") + "</table>",
    caption="""
...
@memo
def bridge_transfer_plot(arrays: dict) -> str:
    @themed(
    ...
    def plot() -> plt.Figure:
        within = {
            (f, t): np.stack([arrays[f"bridge-s{s}/probes/{f}/{t}/r2_strict"] for s in SEEDS])
            for f in vp.FORMS
            for t in vp.TARGETS
        }
        cross_ = {
            (f, t): np.stack([arrays[f"bridge-s{s}/cross/{cross_applied[f]}/{t}/r2"] for s in SEEDS])
            for f in vp.FORMS
            for t in vp.TARGETS
        }
        return vp.transfer_trace_grid(within, cross_, width=10.5, panel_height=0.4, ylabel="probe R² per depth")
...
bridge_transfer_plot(arrays)

# %%

bridge_unseen = float(np.mean([c["sets"]["cross_unseen"]["accuracy"] for c in cross["cells"]]))
bridge_guess = float(np.mean([c["sets"]["cross_unseen"]["guess_dist"] for c in cross["cells"]]))
bridge_floor = float(np.mean([c["sets"]["cross_unseen"]["floor_dist"] for c in cross["cells"]]))
bridge_angles = angles_at("bridge")
bridge_lines = stats["n140-h216-bridge"]["n_lines"]
bridge_share2 = bridge_lines["cross"] / sum(bridge_lines.values())

rf"""
So the bridge condition converts between the two vocabularies and keeps their geometries apart. Behaviorally the forms are joined: an unseen name and an unseen hex code mix to the right answer {bridge_unseen * 100:.0f}% of the time, and the answers it emits sit {bridge_guess:.3f} from the exact mix on average, against a quantization floor of {bridge_floor:.3f}. Geometrically nothing moved: $\rho$ stays at zero across the whole scan, and the first principal angle between the two mix decoders is {bridge_angles["cross_form"]:.0f}° against a random-subspace null of {bridge_angles["random"]:.0f}° and a seed control of {bridge_angles["seed_control"]:.0f}°.
...
Cross lines are {bridge_share2 * 100:.0f}% of the corpus here, and the weight was set before we knew what the disjoint corpus would do. A larger share, or cross equations that answer in *both* forms rather than only in hex, would be a different experiment; this one shows that {bridge_share2 * 100:.0f}% is enough to teach the conversion and not enough to merge the geometries. Whether those two thresholds are far apart is open. It is also possible that no amount of bridging merges them under this grammar, since the answer form is determined by the prompt form, and keeping two decoders is a perfectly good way to satisfy that.
///
"""

sig_depth = {a: fit_sigma(a, "n140-h216", "named_holdout")["sigma"] for a in ("center", "L8", "d16", "d16-L8")}
...
At d64 the extra layers buy nothing. Held-out named accuracy goes from {seed_mean("center", "named_holdout", "accuracy"):.3f} at L4 to {seed_mean("L8", "named_holdout", "accuracy"):.3f} at L8, and accuracy on trained pairs from {seed_mean("center", "named_seen", "accuracy"):.3f} to {seed_mean("L8", "named_seen", "accuracy"):.3f}. Three seeds cannot resolve differences that small. Fitted precision agrees: $\sigma$ comes out at {sig_depth["center"]:.3f} of the unit cube at L4 and {sig_depth["L8"]:.3f} at L8, the same to three decimals, so the model reads colors to the same resolution either way. That fits the H1 account, where the named ceiling at d64 is the spacing of the palette rather than anything the model ran out of. Depth cannot buy resolution the task isn't short of.

At d16 it buys a great deal. Held-out named accuracy goes from {seed_mean("d16", "named_holdout", "accuracy"):.3f} to {seed_mean("d16-L8", "named_holdout", "accuracy"):.3f}, and $\sigma$ from {sig_depth["d16"]:.3f} to {sig_depth["d16-L8"]:.3f}. The same eight layers that do nothing at width 64 recover about half the color resolution lost by narrowing to width 16. So depth substitutes for width here, up to the point where the resolution ceiling of the task itself takes over.
...
@memo
def depth_profiles_plot(arrays: dict) -> str:
    @themed(
        name="depth-profiles",
        alt_text="""
    ...
    def plot() -> plt.Figure:
        ...
        for ax, lmk, title in [(axes[0], "eq", "at the ="), (axes[1], "pre", "at the pre-answer space")]:
            i = LANDMARKS.index(lmk)
            for arm, label, cl, cd, ls in depth_cellset:
                v = np.stack([arrays[f"{arm}-s{s}/probes/named/mix/r2_strict"][:, i] for s in SEEDS])
                x = np.arange(v.shape[1]) / (v.shape[1] - 1)
                c = light_dark(cl, cd)
                ax.fill_between(x, v.min(0), v.max(0), color=c, alpha=0.15, lw=0)
                ax.plot(x, v.mean(0), ls, color=c, lw=1.4, label=label)
            ax.set(title=title, xlabel="relative depth", ylim=(-0.1, 1.05))
            ax.grid(alpha=0.3)
        axes[0].set_ylabel("named mix $R^2$")
        axes[0].legend(fontsize=7, loc="upper left", framealpha=0.6)
        return fig

    return plot()


depth_profiles_plot(arrays)

# %%

depth_pre, depth_eq = LANDMARKS.index("pre"), LANDMARKS.index("eq")


def depth_profile(arm: str, seed: int) -> np.ndarray:
    return arrays[f"{arm}-s{seed}/probes/named/mix/r2_strict"][:, depth_pre]


def layers_at_ceiling(arm: str, thresh: float = 0.9) -> float:
    """Mean number of layers, per seed, where the pre-answer mix clears *thresh*."""
    return float(np.mean([(depth_profile(arm, s) >= thresh).sum() for s in SEEDS]))


def crossings(arm: str, thresh: float = 0.9) -> str:
    """The layer at which the pre-answer mix of each seed first clears *thresh*."""
    return ", ".join(str(int(np.argmax(depth_profile(arm, s) >= thresh))) for s in SEEDS)


center_depth_map, l8_depth_map = maps("center", "probes/named/mix/r2_strict"), maps("L8", "probes/named/mix/r2_strict")

rf"""
The mix does crystallize before the last layer at L8, which is what the clause asked for. It reads {l8_depth_map[6, depth_pre]:.2f} at layer 6, {l8_depth_map[7, depth_pre]:.2f} at layer 7 and {l8_depth_map[8, depth_pre]:.2f} at layer 8, so the final layer adds nothing and the concept exists for two layers rather than one. Counting layers over 0.9, L4 averages {layers_at_ceiling("center"):.1f} of 4 across seeds and L8 {layers_at_ceiling("L8"):.1f} of 8.

Read as a fraction of the network, though, the mix sits *later* at L8: about {layers_at_ceiling("center") / 4:.0%} of the layers at L4 falls to {layers_at_ceiling("L8") / 8:.0%} at L8. The extra layers went in front of the computation rather than after it, and the mature mix stayed where it was, up against the answer. That is the counter-expectation H6 flagged. It is also the reading we'd trust, since it is the one that would generalize to a deeper model.

Depth does move the mix earlier in the sequence. At the `=` sign the named mix reads {center_depth_map[4, depth_eq]:.2f} at L4 and {l8_depth_map[7, depth_eq]:.2f} at L8, so one character before the site H2 measures, the deeper model has already assembled most of the answer. A head start on the token axis is a plausible thing for four extra layers to buy, though we haven't tested that reading.
...
crossed_w = {f: np.stack([arrays[f"center-s{s}/probes/{f}/mix/weights"] for s in SEEDS]) for f in ("named", "hex")}
crossed_strict = {
    f: np.mean([arrays[f"center-s{s}/probes/{f}/mix/r2_strict"] for s in SEEDS], axis=0) for f in ("named", "hex")
}
crossed_n_depth = crossed_w["named"].shape[1]


def crossed_first_angle(dn: int, ln: int, dh: int, lh: int) -> float:
    return float(
        np.mean([gm.principal_angles(crossed_w["named"][s, dn, ln], crossed_w["hex"][s, dh, lh])[0] for s in range(3)])
    )


crossed_pre = lm_index["pre"]
crossed_angles = np.array(
    [
        [crossed_first_angle(dn, crossed_pre, dh, crossed_pre) for dh in range(crossed_n_depth)]
        for dn in range(crossed_n_depth)
    ]
)
crossed_off_embedding = crossed_angles[1:, 1:]  # depth 0 is the token embedding; see below
...
crossed_sites = {f: np.argwhere(m >= 0.3) for f, m in crossed_strict.items()}
crossed_best = min(
    (crossed_first_angle(dn, ln, dh, lh) for dn, ln in crossed_sites["named"] for dh, lh in crossed_sites["hex"]),
    default=float("nan"),
)
crossed_hex_pre = crossed_strict["hex"][:, crossed_pre]

rf"""
### The forms compute at different depths, and $\rho$ compares in place (post hoc)

$\rho$ as preregistered compares the same cell in both forms: same depth, same landmark. That assumes the two forms put the mix in the same place, and H2 showed they do not. Named color mixes are assembled at the pre-answer position in the last layer. The best hex mix reading is mid-answer, a layer earlier, and its pre-answer column never clears {crossed_hex_pre.max():.2f} at any depth ({", ".join(f"{v:.2f}" for v in crossed_hex_pre)} from the embedding up). So the same-cell comparison may score the mature named mix against a hex representation that hasn't formed at that position, and a depth-crossed $\rho$ could in principle be higher.

We can check half of this from the stored data. The transfer half cannot be answered: applying the hex depth-{crossed_n_depth - 3} probe to the named depth-{crossed_n_depth - 1} activations needs the activations, and only the fitted probes and their scores are stored. The subspace half can, from the probe weights alone. The first principal angle between the named mix probe at *any* depth and the hex mix probe at *any* depth, both at the pre-answer position, stays between {crossed_off_embedding.min():.0f}° and {crossed_off_embedding.max():.0f}° across all {crossed_off_embedding.size} pairs. Widening the search to every depth × landmark pair where the strict within-form readings of both forms clear 0.3 (a low bar, and one that admits the contaminated answer positions), the closest the two subspaces come is {crossed_best:.0f}°.
...
snap_edges = np.array([0, 0.01, 0.02, 0.03, 0.045, 0.065, 0.09, 0.2])


@memo
def snap_margin_plot(precision: dict, panels: list, edges: np.ndarray) -> str:
    @themed(
        name="snap-margin",
        alt_text="""
    ...
    def plot() -> plt.Figure:
        ...
        obs_c = light_dark("#1a5f8a", "#6ab0d4")
        ref_c = light_dark("#d1495b", "#ff6b7d")
        for ax, (arm, set_name, title) in zip(axes, panels, strict=True):
            b = precision[arm, set_name]
            mg, hit, pred = b["margin"], b["hit"], b["pred"]
            mid = np.sqrt(edges[:-1] * np.maximum(edges[1:], 1e-6))  # log-ish bin centre
            rate, err, ref, x = [], [], [], []
            for lo, hi, m in zip(edges[:-1], edges[1:], mid, strict=True):
                sel = (mg >= lo) & (mg < hi)
                if sel.sum() < 12:
                    continue
                rate.append(hit[sel].mean())
                err.append(np.sqrt(rate[-1] * (1 - rate[-1]) / sel.sum()))
                ref.append(pred[sel].mean())
                x.append(m)
            ...
            share = np.histogram(mg, edges)[0] / len(mg)
            ax.fill_between(
                edges,
                0,
                np.append(share, share[-1]) / 1.5,
                step="post",
                color=obs_c,
                alpha=light_dark(0.14, 0.25),
                lw=0,
            )
            ax.plot(x, ref, color=ref_c, lw=1.4, label="resolution-limited")
            ax.errorbar(x, rate, yerr=err, fmt="o", ms=4, color=obs_c, lw=1, label="observed")
            ax.set(xscale="log", xlim=(edges[0] + 0.006, edges[-1]), ylim=(0, 1.05))
            ax.set_title(title, fontsize=8)
            ax.set_xlabel("snap margin")
            ax.grid(alpha=0.3)
            ax.text(0.95, 0.06, f"σ = {b['sigma']:.3f}", transform=ax.transAxes, ha="right", fontsize=8, color=ref_c)
...
snap_margin_plot(precision, snap_panels, snap_edges)
...
sigma_named = precision["center", "named_holdout"]["sigma"]
grid_rng = np.random.default_rng(2)
ref_grids = {"v216 (ex-2.1.3/2.1.4)": (0, 3, 6, 9, 12, 15), "v4096 (ex-2.1.3)": tuple(range(16))}
grid_preds = {}
for grid_name, levels in ref_grids.items():
    ...
    pts = np.array([(r, g, b) for r in levels for g in levels for b in levels])
    idx = np.random.default_rng(3).integers(len(pts), size=(8192, 2))
    mix = (pts[idx[:, 0]] + pts[idx[:, 1]] + 1) // 2
    mix = mix[np.isin(mix, np.array(levels)).all(1)][:256] / 15
    pal = pts / 15
    ti = np.linalg.norm(pal[None] - mix[:, None], axis=2).argmin(1)
    grid_preds[grid_name] = float(bl.precision_limited_acc(pal, mix, ti, sigma_named, grid_rng, 120).mean())
...
target_sd = float(
    np.sqrt(
        np.var(
            np.concatenate([arrays[f"center-s{s}/evals/named_seen/mix_value"] for s in SEEDS]).astype(float) / 255,
            axis=0,
        ).mean()
    )
)
probe_pre_r2 = float(
    np.mean([arrays[f"center-s{s}/probes/named/mix/r2_strict"][4, LANDMARKS.index("pre")] for s in SEEDS])
)
probe_sd = target_sd * np.sqrt(max(0.0, 1 - probe_pre_r2))
...
**The earlier rungs are consistent with the same precision.** On the regular grids of ex-2.1.3, where a mix lands on a vocabulary point and the margin is the grid step, $\sigma = {sigma_named:.3f}$ predicts {grid_preds["v216 (ex-2.1.3/2.1.4)"]:.2f} at `v216`, which is about what ex-2.1.3 (≈ 1.0, single-token names) and ex-2.1.4 (0.91, spelled names) actually scored. So the drop from ≈ 1.0 there to 0.67 here needs no change in how well colors are represented; the coarser vocabulary forgives more. The exception is `v4096`, where the same $\sigma$ predicts {grid_preds["v4096 (ex-2.1.3)"]:.2f} against the observed 0.65 in ex-2.1.3: that model read colors roughly 1.5× more finely than this one, plausible for a single-vocabulary corpus with single-token answers.

**The behavior is more precise than the probe.** At the pre-answer position in the last layer the strict mix probe reads $R^2 = {probe_pre_r2:.3f}$. With a per-channel target standard deviation of {target_sd:.3f}, that puts the probe residual near {probe_sd:.3f}, roughly twice the {sigma_named:.3f} the behavior implies. The two are different estimands (the error of a linear decoder versus an implied end-to-end resolution), so only the ordering is worth reading: the linear probe recovers a coarser version of the value than the model itself uses. For anchor design this means a probe $R^2$ of 0.94 at a site is a floor on what is there, and anchor supervision is itself a linear readout with the same limitation.
