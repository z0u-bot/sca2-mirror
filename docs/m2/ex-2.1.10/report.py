# title: Ex 2.1.10: A label that doesn't point
...
grid_levels = nc.GRIDS[ex.GRID]
palette_map = nc.grid_palette(grid_levels)
palette_colors = list(palette_map.values())
on_grid = set(grid_levels)
line_pairs = [(c, b) for c in palette_colors for b in palette_colors if all(v in on_grid for v in mix(c, b))]
R1 = redness(np.asarray([p[0] for p in line_pairs], dtype=float) / (N_LEVELS - 1))
R2 = redness(np.asarray([p[1] for p in line_pairs], dtype=float) / (N_LEVELS - 1))
...
def retention(cond: str) -> float | None:
    ...
    """
    r = [t[-1] / t.max() for s in range(N_SEEDS[cond]) if (t := traj(cond, s, "m_line")).max() >= ex.RETENTION_FLOOR]
...
findings_ctrl = acc("lam0").mean()
findings_lead = pi_group_mean(ex.PRIMARY)[0, 0, 0]  # G1's op1 weight at the embedding; G2's op2 weight equals it
findings_frac = (m_line(ex.PRIMARY).mean() - m_line("lam0").mean()) / (
    m_line(ex.ORACLE_ARM).mean() - m_line("lam0").mean()
)
findings_ret = retention(ex.PRIMARY)
assert findings_ret is not None

rf"""
**H1 (task cost) — holds.** The largest `named_holdout` exact-match gap from the control across the five anchored conditions is {max(abs(acc(c).mean() - findings_ctrl) for c in [ex.REFERENCE_ARM, *ex.EITHER_NAMES, ex.ORACLE_ARM]):.4f}, against a gate of {ex.TASK_GATE:g}.

**H2 (the pull selects the labelled operand) — holds.** On the primary (`{ex.PRIMARY}`): (a) at the embedding the leading role in each group is its own operand, at weight {findings_lead:.2f} (gate ≥ {ex.LEAD_GATE:g}; uniform is 0.25), in all nine runs individually; (b) the deep-slice between-group op2 contrast is {contrast(ex.PRIMARY).mean():+.2f} (gate ≥ {ex.CONTRAST_GATE:g}).

**H3 (the operating point survives) — holds.** On `{ex.PRIMARY}`: containment $\bar\alpha$ at op1 = {a_op1(ex.PRIMARY).mean():.3f} (gate ≤ {ex.MEAN_ALIGN_GATE:g}); retention {findings_ret:.2f} (gate ≥ {ex.RETENTION_GATE:g}, minimum across nine seeds); grading $r^2$ = {grading_r2(ex.PRIMARY):.2f} against {grading_r2(ex.REFERENCE_ARM):.2f} for the reference arm, a drop of {grading_r2(ex.REFERENCE_ARM) - grading_r2(ex.PRIMARY):.3f} (gate ≤ {ex.GRADE_R2_DROP:g}).

**H4 (selectivity without the pointer) — holds.** Control-subtracted m_line of the primary is {findings_frac:.0%} of the value for the slot oracle (gate ≥ {ex.ORACLE_FRAC_GATE:.0%}).
...
{ex.EX219_REFERENCE["pool-t100"]["lead_emb"]:.2f} of its weight on op1 at the embedding without being told. But the
...
Each operand of a line draws independently at redness⁸ × {ex.PER_SLOT_RATE:g}, and
the line is labeled when either draws. The per-slot rate is half of the
op1 labeller's {ex.RED_RATE:g}, which keeps the expected labeled-line rate where
...
(op1-keyed at {ex.RED_RATE:g}), reproducing ex-2.1.9's `pool-t100`
...
n_runs = sum(c["seeds"] for c in ex.CONDITIONS)
...
and we can measure the failure rate better with more seeds. {n_runs} runs in
...
cal_levels = nc.GRIDS[ex.GRID]
cal_distinct = nc.closed_pairs(cal_levels)
cal_colors = list(nc.grid_palette(cal_levels).values())
cal_ordered = np.asarray(
    [*cal_distinct, *[(b, a) for a, b in cal_distinct], *[(c, c) for c in cal_colors]], dtype=float
)
cal_r1 = redness(cal_ordered[:, 0] / (N_LEVELS - 1))
cal_r2 = redness(cal_ordered[:, 1] / (N_LEVELS - 1))

p_old = float(np.mean(cal_r1**8 * ex.RED_RATE))
p_new = float(np.mean(ex.line_label_p(cal_r1, cal_r2)))
g1_mass = ex.p_slot(cal_r1) * (1 - ex.p_slot(cal_r2))
g2_mass = (1 - ex.p_slot(cal_r1)) * ex.p_slot(cal_r2)
both_mass = ex.p_slot(cal_r1) * ex.p_slot(cal_r2)
label_total = float(np.mean(g1_mass + g2_mass + both_mass))
label_shares = [float(np.mean(x)) / label_total for x in (g1_mass, g2_mass, both_mass)]
...
ALPHA219 = {c: ex219_alpha(c) for c in ["lam0", "pool-t100"]}
...
tau_grid = np.geomspace(0.003, 8.0, 70)
...
def tau_ladder_plot() -> plt.Figure:
    ...
    for ax, (cond, title) in zip(axs, [("lam0", "control profile"), ("pool-t100", "pooled-pull profile")], strict=True):
        for li in range(5):
            ax.plot(tau_grid, tau_lead[cond][:, li], color=slice_colors[li], lw=1.4, label=f"slice {li}")
        for t in ex.TAUS:
...
def seed_mean_219(stat, cond: str) -> float:
...
m9 = seed_mean_219(lambda a: ex.pooled_margin(a, ex.LABEL_W), "pool-t100")
a9 = seed_mean_219(ex.alpha_span, "pool-t100")
ao9 = seed_mean_219(lambda a: float(a[:, :, 0].mean()), "pool-t100")

rf"""
The ex-2.1.9 arrays give the reproduction targets for the `op1-labels` arm, as means of per-run statistics: m_span = {m9:.2f}, ᾱ_span = {a9:.2f}, ᾱ at op1 = {ao9:.2f}. A miss there means the new labelling code path moved the numbers, and gets settled before any either-slot condition is read.
"""
...
[^data-volume]: The volume stays small: 5 slices × 5832 lines × ~6 positions
    ...
    all {n_runs} runs. That is an order of magnitude over ex-2.1.9's
...
Throughout, "the primary" is `{ex.PRIMARY}` (fixed in advance, marked in the
...
operand (op1 for G1, op2 for G2) with weight $\ge {ex.LEAD_GATE:g}$ in both
(uniform is 0.25);
**(b)** across the four post-attention slices, the between-group contrast in
op2 weight, $\mathrm{{mean}}_\ell\,[\bar\pi_{{G2}}(\ell, \text{{op2}}) -
\bar\pi_{{G1}}(\ell, \text{{op2}})]$, is $\ge {ex.CONTRAST_GATE:g}$.
Partial: (a) holds and (b) lands in $[{ex.CONTRAST_PARTIAL:g}, {ex.CONTRAST_GATE:g})$.
...
*uniform* — the primary reaches {ex.LEAD_GATE:g} in neither group at any slice
...
**(a)** containment, $\bar\alpha \le {ex.MEAN_ALIGN_GATE:g}$ at op1 (also the contamination
...
`sim¹·⁵` no more than {ex.GRADE_R2_DROP:g} below the `op1-labels` arm's (higher is
fine).
Partial: (a) and (b) hold, (c) misses.

**H4: Selectivity holds without the pointer.** Control-subtracted
m_line of the primary reaches $\ge {ex.ORACLE_FRAC_GATE:g}$ of the control-subtracted
m_line of `slot-oracle`. Partial: $[{ex.ORACLE_FRAC_PARTIAL:g}, {ex.ORACLE_FRAC_GATE:g})$.
...
h1_rows = "".join(
    ...
    f"<td class='num'>{acc(c).mean() - h1_ctrl:+.4f}</td>"
    f"<td>{'clean' if abs(acc(c).mean() - h1_ctrl) <= ex.TASK_GATE else 'not clean'}</td></tr>"
    for c in CONDS
)
h1_table = f"""<table class="report-table">
  <thead><tr>
...
h1_worst = max((c for c in CONDS if c != "lam0"), key=lambda c: abs(acc(c).mean() - h1_ctrl))

rf"""
**H1 holds.** The largest gap from the control is {abs(acc(h1_worst).mean() - h1_ctrl):.4f}, at `{h1_worst}`, against a gate of {ex.TASK_GATE:g}, so H2–H4 are all scored. Labels that no longer say which operand drew change nothing the task loss can see, as with every anchored condition on this testbed so far.
"""
...
repro_stats = {"m_span": m_span, "alpha_span": a_span, "m_op1": m_op1, "alpha_op1": a_op1}
repro_names = {"m_span": "m<sub>span</sub>", "alpha_span": "ᾱ<sub>span</sub>", "m_op1": "m<sub>op1</sub>",
               "alpha_op1": "ᾱ (op1)"}  # fmt: skip


def repro_row(cond: str, ref_key: str) -> str:
    ref = ex.EX219_REFERENCE[ref_key]
    tds = ""
    for k, fn in repro_stats.items():
        v = fn(cond)
        tds += (
...
repro_head = "".join(
    f"<th class='num'>{repro_names[k]}</th><th class='num'>ex-2.1.9</th><th class='num'>Δ</th>" for k in repro_stats
)
repro_table = f"""<table class="report-table">
  <thead><tr><th>condition</th>{repro_head}</tr></thead>
  <tbody>{repro_row("op1-labels", "pool-t100")}{repro_row("lam0", "lam0")}</tbody>
</table>"""
...
rf"""
The reproduction is as close as the rounding in the store allows: every carried statistic lands within ±0.001 of its target, and the embedding lead weight ({pi_group_mean("op1-labels")[0, 0, 0]:.2f}) matches the {ex.EX219_REFERENCE["pool-t100"]["lead_emb"]:g} of ex-2.1.9. The op1-keyed draw consumes the rng as the old sampler did, so this is the same training trajectory replayed, and the either-slot conditions can be read against `op1-labels` as a like-for-like baseline.
...
GROUP_ALT = {
    "either-t100": """
        Ten small panels in two columns labeled G1, op1 drew, and G2, op2 drew: five residual slices per column with the embedding at the bottom, each spanning the roles op1, plus, op2 and equals. In the G1 column every slice shows a tall block at op1 — 0.77 at the embedding, near 0.9 at depth — with a small step at equals only at the embedding. The G2 column is the mirror image: the block sits at op2 at every slice, 0.77 at the embedding rising to 0.95 at depth. Nine per-seed hairlines hug the mean everywhere. A dashed readability line in the G1 column is high at op1 on every slice; at the embedding it falls to near zero at the other three roles, while at depth it dips at plus and rises again at equals.
    """,
    "either-t500": """
...
def profile_fig(cond: str, title: str) -> str:
    ...
    @themed(
        ...
        caption=rf"""
            **{title}** — τ = {dict(zip(ex.EITHER_NAMES, ex.TAUS, strict=True))[cond]:g}; deep-slice op2 contrast {con.mean():+.2f}
            <span class='range'>±{con.std(ddof=1):.2f}</span>.
        """,
    )
    def plot() -> plt.Figure:
        ...
        for col, gname in enumerate(("G1: op1 drew", "G2: op2 drew")):
            for row in range(5):
                ...
                ax = axes[row][col]
                smooth_step_area(ax, span_x, pg[col, sl], ramp=0.5, color=ink, alpha=light_dark(0.22, 0.28))
                smooth_step(ax, span_x, pg[col, sl], ramp=0.5, color=ink, lw=1.5)
                for sp in seeds:
                    smooth_step(ax, span_x, sp[col, sl], ramp=0.5, color=hair, lw=0.5)
                if col == 0:
            ...
            emb_ax = axes[4][col]
            emb_ax.plot(col, ex.LEAD_GATE, marker=9 if col == 0 else 8, ms=3, color=gate,
                        clip_on=False, zorder=6, transform=emb_ax.get_yaxis_transform())  # fmt: skip
            val = float(pg[col, 0, own])
            emb_ax.annotate(f"{val:.2f}", (own + 0.35 if col == 0 else own, min(val + 0.14, 0.92)),
                             ha="left" if col == 0 else "center", fontsize=7,
                             color=light_dark("#444", "#bbb"))  # fmt: skip
            axes[4][col].set_xticks(span_x, ex.ROLES)
...
GROUP_TITLES = {"either-t100": "The primary", "either-t500": "Softer", "either-t2500": "Near the span mean"}
group_body = "".join(profile_fig(c, GROUP_TITLES[c]) for c in ex.EITHER_NAMES)
group_caption = rf"""
**Per-group weight profiles under the either-slot labeller.** Within each sub-figure: columns are the label groups (G1 weights probe lines by P(only op1 drew), G2 by P(only op2 drew)); rows are residual slices with the embedding at the bottom; the shaded smooth-step is the seed-mean softmin weight $\bar\pi_G(\ell, t)$ over the span roles, with per-seed hairlines (nine on the primary). The dashed line behind the G1 columns is the un-anchored control's readability profile $R^2(\ell, t)$ for op1's redness. At the embedding, the caret on each column's outer axis marks the H2(a) gate level ({ex.LEAD_GATE:g}; uniform is 0.25), and the number above each group's own operand is the seed-mean weight there. Each sub-caption quotes the H2(b) deep-slice op2 contrast (gate ≥ {ex.CONTRAST_GATE:g}, scored on the primary only).
"""

figure_html(
    group_body,
    caption=group_caption,
    aria_label="Per-group softmin weight profiles, one sub-figure per either-slot condition",
)

rf"""
**H2 holds, and the profiles mirror each other.** (a) At the embedding the leading role in each group is its own operand, at weight {pi_group_mean(ex.PRIMARY)[0, 0, 0]:.2f} (gate ≥ {ex.LEAD_GATE:g}), in all nine runs individually. The two groups take equal values there by construction, since at the embedding the state of a token cannot depend on its line.[^emb-sym]

(b) The between-group op2 contrast over the post-attention slices is {contrast(ex.PRIMARY).mean():+.2f}
<span class='range'>±{contrast(ex.PRIMARY).std(ddof=1):.2f}</span> against the
{ex.CONTRAST_GATE:g} gate. The op2 weight of G2 at the last slice is
{pi_group_mean(ex.PRIMARY)[1, 4, 2]:.2f}, so on the lines where op2 was the evidence the pull
...
The unscored arms behave as the τ ladder predicted. At τ = 0.5 the embedding is nearly flat (lead {pi_group_mean("either-t500")[0, 0, 0]:.2f}) and the deep contrast reaches only {contrast("either-t500").mean():+.2f}; at τ = 2.5 the profile is the span mean in all but name (contrast {contrast("either-t2500").mean():+.2f}). A pull that cannot concentrate cannot localize, however good the labels are.

The op1-keyed reference arm has a *measured* contrast of {contrast("op1-labels").mean():+.2f}, worth holding onto for M3. The shared lexicon aligns red op2 tokens for free, so a softmin read at τ = 0.1 finds them even though the pull never targeted op2. But that free contrast decays with depth: the op2 weight of G2 falls from {pi_group_mean("op1-labels")[1, 1, 2]:.2f} at slice 1 to {pi_group_mean("op1-labels")[1, 4, 2]:.2f} at slice 4, where the primary holds ({pi_group_mean(ex.PRIMARY)[1, 1, 2]:.2f} → {pi_group_mean(ex.PRIMARY)[1, 4, 2]:.2f}). Training on the wider labels is what keeps the localization alive where the states are contextual.
"""
...
TRAJ_KEYS = ("alpha_op1", "m_line")
# Seed means once, in the cell: @themed calls the plot function twice.
traj_mean = {
    c: {k: np.mean([traj(c, s, k) for s in range(N_SEEDS[c])], axis=0) for k in TRAJ_KEYS} for c in [*ANCHORED, "lam0"]
}
traj_epochs = np.mean([traj(ex.PRIMARY, s, "epoch") for s in range(N_SEEDS[ex.PRIMARY])], axis=0)
traj_retention = {c: retention(c) for c in ANCHORED}
...
sched_epochs = np.linspace(0.0, float(ex.SCHEDULER.epochs), 601)
sched_anchor = ex.anchor_weight(sched_epochs)
sched_anti = ex.anti_subspace_weight(sched_epochs)
TRAJ_GRID = [["op1-labels", "either-t100", "either-t500"], ["either-t2500", "slot-oracle", "sched"]]
...
def trajectories_plot() -> plt.Figure:
    ...
    dash = (0, (6, 1))
    top = max(np.max(traj_mean[c]["m_line"]) for c in ANCHORED)
    for cond in ANCHORED:
        ...
        ax.plot(1, ex.RETENTION_FLOOR, marker=8, ms=3, mfc="none", mec=gate, mew=0.8,
        ...
        ax.plot(traj_epochs, traj_mean["lam0"]["m_line"], color=grey, lw=1.1, ls=dash, zorder=3)
        for other in ANCHORED:
            ...
            if other != cond:
                ax.plot(traj_epochs, traj_mean[other]["m_line"], color=ghost, lw=0.8, zorder=2)
        ax.plot(traj_epochs, traj_mean[cond]["m_line"], color=ink, lw=1.4, ls=dash, zorder=4)
        ax.plot(traj_epochs, traj_mean[cond]["alpha_op1"], color=ink, lw=1.7, zorder=5)
        r = traj_retention[cond]
        assert r is not None
        ax.annotate(f"{r:.2f}", (0.97, 0.6), xycoords="axes fraction", ha="right", fontsize=8,
                    color=light_dark("#444", "#bbb"))  # fmt: skip
        ax.set_title("either-t100 (primary)" if cond == ex.PRIMARY else cond, fontsize=9.5, color=ink,
                     fontweight="bold" if cond == ex.PRIMARY else "normal")  # fmt: skip
        ax.set_xlim(0, ex.SCHEDULER.epochs)
        ax.set_ylim(-0.08, top * 1.08)
        ax.tick_params(labelbottom=cond in TRAJ_GRID[1], labelleft=cond in TRAJ_LEFT, labelsize=8)
        ax.set_ylabel("cosine", fontsize="x-small") if cond in TRAJ_LEFT else None
        ax.set_xlabel("epoch", fontsize="x-small") if cond in TRAJ_GRID[1] else None
    ax = axd["sched"]
    ax.plot(sched_epochs, sched_anchor, color=term["anchor"], lw=1.4)
    ax.plot(sched_epochs, sched_anti, color=term["anti"], lw=1.4)
    ax.set_yscale("log")
    ax.set_xlim(0, ex.SCHEDULER.epochs)
    ax.set_ylim(2e-4, 4)
    ...
    ax.set_title("weights (all anchored)", fontsize=9.5, color=grey)
    for xx, yy, txt, k in ((64, 0.13, r"$\lambda_\mathrm{a}$", "anchor"), (16, 0.5, r"$\lambda_{\bar{\mathrm{s}}}$", "anti")):  # fmt: skip
        ax.annotate(txt, (xx, yy), fontsize=9, color=term[k], va="bottom")
    fig.legend(
...
h3_ret = retention(ex.PRIMARY)
assert h3_ret is not None
h3_worst = min(r for c in ANCHORED if (r := retention(c)) is not None)

rf"""
**H3(a) and (b) hold.** On the primary, $\bar\alpha$ at op1 ends at {a_op1(ex.PRIMARY).mean():.3f} <span class='range'>±{a_op1(ex.PRIMARY).std(ddof=1):.3f}</span> against the {ex.MEAN_ALIGN_GATE:g} gate. That is *below* the {a_op1(ex.REFERENCE_ARM).mean():.3f} of the op1-keyed reference, so the contamination a global latch would leave (non-red op1 states dragged onto the axis on op2-triggered lines) is absent, which agrees with the H2 profiles. Op1 now hosts less of the drift, perhaps because half the pull budget now goes to op2.

Retention is {h3_ret:.2f} against {ex.RETENTION_GATE:g}, taken as the minimum over nine seeds, a stricter read than for the three-seed conditions; the worst retention anywhere in the sweep is {h3_worst:.2f}. The five anchored panels above are near copies, so localizing per line neither delays the margin nor loosens its hold.
"""

GRADE_RESP = {c: alpha_map(c)[:, :, 0].mean(axis=0) for c in CONDS}
...
GRADE_CTRL = level_mean(GRADE_RESP["lam0"])
...
def grading_plot() -> plt.Figure:
    ...
    for row, conds in zip(axes, GRADE_GRID, strict=True):
        for ax, cond in zip(row, conds, strict=True):
            ...
            ax.set_title(
                "either-t100 (primary)" if cond == ex.PRIMARY else ("control" if cond == "lam0" else cond),
                fontsize=9.5,
            )
            if cond != "lam0":
                ax.annotate(
...
h4_ctrl = float(m_line("lam0").mean())
# Control-subtracted per-run values, computed once — @themed draws twice.
h4_delta = {c: m_line(c) - h4_ctrl for c in ANCHORED}
...
def h4_dots_plot() -> plt.Figure:
    ...
    ax.set_title(r"m_line − control", fontsize="small", color=grey)
    ax.set_yticks(range(len(rows)), ["slot-oracle", "either-t2500", "either-t500", "either-t100 (primary)",
                                      "op1-labels"], fontsize=8)  # fmt: skip
    ax.set_xlim(0, 0.52)
    ax.tick_params(axis="x", labelsize=8)
    ax.axvline(h4_oracle * ex.ORACLE_FRAC_GATE, color=gate, lw=0.9, ls=(0, (4, 3)))
    ax.axvline(h4_oracle * ex.ORACLE_FRAC_PARTIAL, color=gate, lw=0.9, ls=(0, (1, 2)))
    return fig
...
h4_oracle_ctrl = float(m_line(ex.ORACLE_ARM).mean()) - h4_ctrl


def h4_row(c: str) -> str:
    frac = f"{(m_line(c).mean() - h4_ctrl) / h4_oracle_ctrl:.0%}" if c != ex.ORACLE_ARM else "—"
    ...
    return (
        ...
        f"<td class='num'>{m_line(c).mean() - h4_ctrl:.3f}</td>"
...
h4_table = f"""<table class="report-table">
  <thead><tr>
...
figure_html(
    h4_table,
    caption=rf"""
        The anchored conditions, seed means (± seed standard deviation on the scored statistic). Only the primary (so marked) is gated; "oracle frac" is control-subtracted m<sub>line</sub> as a fraction of the slot oracle's {h4_oracle_ctrl:.3f}. m<sub>span</sub> and m<sub>op1</sub> are carried for continuity with ex-2.1.9 (both key their line weights on op1 alone), ᾱ<sub>span</sub> and ᾱ (op1) are the drift statistics, and r² repeats the grading track scored under H3.
    """,
    class_="report-figure",
)
...
h4_p_frac = (m_line(ex.PRIMARY).mean() - h4_ctrl) / (m_line(ex.ORACLE_ARM).mean() - h4_ctrl)
h4_ref_frac = (m_line(ex.REFERENCE_ARM).mean() - h4_ctrl) / (m_line(ex.ORACLE_ARM).mean() - h4_ctrl)

rf"""
**H4 holds.** The primary delivers {h4_p_frac:.0%} of the control-subtracted m_line of the oracle, against the {ex.ORACLE_FRAC_GATE:.0%} gate. That is level with the oracle, within seed noise (spreads of {m_line(ex.PRIMARY).std(ddof=1):.3f} and {m_line(ex.ORACLE_ARM).std(ddof=1):.3f} on conditions sharing the same label draws). The latch account predicted roughly the op1-triggered half plus the lexicon give-away, and it is ruled out twice over: by this fraction, and by the H2 profiles.
...
The op1-keyed arm reaches only {h4_ref_frac:.0%} of the oracle on the same statistic, because its pull never serves the op2-triggered half of the label mass. Widening the labels is what closed the gap.

The soft-τ arms *exceed* everything on m_line ({m_line("either-t500").mean():.2f} at τ = 0.5) while failing every other instrument: contrast near zero (H2), grading down by ~0.2 (H3(c)), and span drift ᾱ_span at {a_span("either-t500").mean():.2f} against {a_span(ex.PRIMARY).mean():.2f} for the primary. A diffuse pull aligns the whole span of labeled-ish lines, and a margin that takes the best role per slice rewards that spread. The number is real, but what it measures there is a smeared, poorly graded alignment rather than a localized one. The exploratory section returns to this trade.
"""
...
role_alpha = np.mean(
    [traj(ex.PRIMARY, s, "alpha_roles") for s in range(N_SEEDS[ex.PRIMARY])], axis=0
)  # (K, slices, roles)
role_epochs = np.mean([traj(ex.PRIMARY, s, "epoch") for s in range(N_SEEDS[ex.PRIMARY])], axis=0)
role_sched_epochs = np.linspace(0.0, float(ex.SCHEDULER.epochs), 601)
...
def role_drift_plot() -> plt.Figure:
    ...
    for ax, sl, title in zip(axes, (0, 4), ("embedding slice", "last slice"), strict=True):
        ax.fill_between(role_sched_epochs, role_anti / role_anti.max() * 0.45, color=grey, alpha=0.10, lw=0, zorder=0)
        for t in range(ex.SPAN):
            ax.plot(role_epochs, role_alpha[:, sl, t], color=role_ink[t], lw=1.4, label=ex.ROLES[t])
...
line_p = np.asarray(ex.line_label_p(R1, R2))
LW_LINE = line_p / line_p.sum()
PB_BOTH = np.asarray(ex.p_slot(R1)) * np.asarray(ex.p_slot(R2))
BOTH_W = PB_BOTH / PB_BOTH.sum()

role_at = lambda e: role_alpha[int(np.searchsorted(role_epochs, e))]  # noqa: E731

lead_mass: list[float] = []
split_mass: list[float] = []
both4: list[np.ndarray] = []
for s in range(N_SEEDS[ex.PRIMARY]):
    w4 = arrays[f"{ex.PRIMARY}-s{s}/w_line"][4]  # (lines, roles)
    lead = w4.max(axis=1)
    lead_mass.append(float(np.einsum("n,n->", LW_LINE, (lead > 0.9).astype(float))))
    ops = w4[:, [0, 2]]
    split_mass.append(float(np.einsum("n,n->", BOTH_W, (np.abs(ops[:, 0] - ops[:, 1]) < 0.2).astype(float))))
    both4.append(np.einsum("n,nt->t", BOTH_W, w4))
both_emb = np.mean(
    [np.einsum("n,nt->t", BOTH_W, arrays[f"{ex.PRIMARY}-s{s}/w_line"][0]) for s in range(N_SEEDS[ex.PRIMARY])], axis=0
)
both_deep = np.mean(both4, axis=0)

rf"""
**When the syntax embeddings drift (queued by ex-2.1.9).** Ex-2.1.9 could see that the `+` embedding ended near 0.30 alignment, but not when. The per-role trajectory shows the drift occurs late. At epoch 30 the two syntax embeddings sit at {role_at(30)[0, 1]:.2f} (`+`) and {role_at(30)[0, 3]:.2f} (`=`). The drift accrues through epochs 30–90 as the repulsion anneals toward the hold ratio, reaching {role_at(90)[0, 1]:.2f} and {role_at(90)[0, 3]:.2f}, and the end-of-training anneal of the anchor recovers none of it. So the syntax drift is bought against the *weakening* repulsion, and in this experiment `=` is the larger of the two; `=` sits outside the labelled operands but inside the pulled span.

At depth the picture inverts. The operand roles carry the residual drift ({role_at(100)[4, 0]:.2f} at op1, {role_at(100)[4, 2]:.2f} at op2) and the syntax roles carry none, and that is where the containment gate of H3(a) reads and passes.

**Is the deep choice still one-hot? (per-line weight entropy.)** Yes, per line. On the label-weighted probe mass, the leading role at the last slice holds over 0.9 of the weight of a line on {np.mean(lead_mass):.0%} of the mass (seed range {min(lead_mass):.0%}–{max(lead_mass):.0%}). The un-labelled bulk stays near uniform, since a flat alignment profile gives the softmin nothing to concentrate on. The winner-take-all behavior of ex-2.1.9 survives the wider labels in the form H2 requires: committed within a line, varying across lines.

**The both-red lines.** Where both operands are red, either answer is defensible, and the pool declines to choose: at the embedding the both-draw-weighted profile splits {both_emb[0]:.2f}/{both_emb[2]:.2f} between the operands (forced by the shared lexicon), and at the last slice it still spreads: op1 {both_deep[0]:.2f}, op2 {both_deep[2]:.2f}, `=` {both_deep[3]:.2f}, with {np.mean(split_mass):.0%} of the both-draw mass within 0.2 of an even operand split. These lines are {PB_BOTH.sum() / line_p.sum():.1%} of the label mass, so nothing above turns on them. They preview the M3 case where a document holds the concept in several places at once.
...
GRADE_T = ex.SIM_TARGET
GRADE_RL = np.round(ex.REDNESS, 6)
GRADE_IX = [np.where(GRADE_RL == lv)[0] for lv in np.unique(GRADE_RL)]
GRADE_QUOTED = [*ex.EITHER_NAMES, ex.REFERENCE_ARM, ex.ORACLE_ARM]


def corr_r2(a: np.ndarray, b: np.ndarray) -> float:
    return float(np.corrcoef(a, b)[0, 1] ** 2)


def slide2(y: np.ndarray) -> np.ndarray:
    """Mean over a sliding window of two adjacent redness levels."""
    return np.convolve(np.array([y[ix].mean() for ix in GRADE_IX]), [0.5, 0.5], mode="valid")


GRADE_QRESP = {c: alpha_map(c)[:, :, 0].mean(axis=0) for c in GRADE_QUOTED}
GRADE_SLIDE = {c: corr_r2(slide2(y), slide2(GRADE_T)) for c, y in GRADE_QRESP.items()}


def within_share(c: str) -> float:
    """The within-redness-level share of the residual variance about the affine sim¹·⁵ fit."""
    y = GRADE_QRESP[c]
    res = y - np.polyval(np.polyfit(GRADE_T, y, 1), GRADE_T)
    win = sum(float(((res[ix] - res[ix].mean()) ** 2).sum()) for ix in GRADE_IX)
    return win / float(((res - res.mean()) ** 2).sum())


def within_r2(c: str) -> float:
    """How much of the reference's within-level variation the response tracks."""
    y = GRADE_QRESP[c]
    da = np.concatenate([y[ix] - y[ix].mean() for ix in GRADE_IX if len(ix) > 1])
    dt = np.concatenate([GRADE_T[ix] - GRADE_T[ix].mean() for ix in GRADE_IX if len(ix) > 1])
    return corr_r2(da, dt)


def seed_noise_share(c: str) -> float:
    """Seed noise in the seed-mean response, as a share of its residual variance."""
    per = np.stack([arrays[f"{c}-s{s}/alpha"][:, :, 0].mean(axis=0) for s in range(N_SEEDS[c])])
    res = GRADE_QRESP[c] - np.polyval(np.polyfit(GRADE_T, GRADE_QRESP[c], 1), GRADE_T)
    return float(per.var(axis=0, ddof=1).mean() / len(per) / res.var())


# Per-slice shape of the primary's response, for the flattening question.
PRIMARY_SLICE_RESP = alpha_map(ex.PRIMARY)[:, :, 0]  # (slices, colors), seed mean
REDNESS_ORDER = np.argsort(-ex.REDNESS)


def top_steepness(y: np.ndarray) -> float:
    """Top-of-curve steepness: the three reddest colors' mean over the next nine's."""
    return float(y[REDNESS_ORDER[:3]].mean() / y[REDNESS_ORDER[3:12]].mean())


rf"""
**Does the per-color $r^2$ score the soft arms down only because the reference zig-zags? (queued by the review round.)** No — what separates them is per-color scatter, though the conditional-mean reading does fit them best: smoothing both curves over a sliding window of two redness levels and correlating puts the soft arms ahead, at $r^2$ {GRADE_SLIDE["either-t500"]:.2f} and {GRADE_SLIDE["either-t2500"]:.2f} against {GRADE_SLIDE[ex.PRIMARY]:.2f} for the primary and {GRADE_SLIDE[ex.ORACLE_ARM]:.2f} for the oracle, matching the conditional-mean lines in the grading figure. The zig-zag itself is unrelated to the frozen statistic, which is computed per color with no binning, and no condition recovers it: every response tracks the reference's within-level variation at $r^2$ ≤ {max(within_r2(c) for c in GRADE_QUOTED):.2f}.

What separates the soft arms is scatter, not the zig-zag: within-level scatter carries {within_share("either-t500"):.0%} and {within_share("either-t2500"):.0%} of their residual variance about the affine `sim¹·⁵` fit, against {within_share(ex.PRIMARY):.0%} for the primary. Seed noise accounts for only {seed_noise_share("either-t500"):.0%} of that residual, so the deviations are stable properties of the pull at that τ, reproducing across seeds.
...
**Why every curve flattens near the pole (queued by the review round).** `sim¹·⁵` steepens toward redness 1, but the responses compress instead. The primary's per-slice profiles show where: at the embedding the response is *steeper* than the reference (three reddest colors {top_steepness(PRIMARY_SLICE_RESP[0]):.1f}× above the next nine, against {top_steepness(GRADE_T):.2f}× for `sim¹·⁵`); in mid-stack the top of the scale runs out of cosine (pure red reaches ≈ {float(PRIMARY_SLICE_RESP[1:4, REDNESS_ORDER[0]].mean()):.2f}, next tier only {top_steepness(PRIMARY_SLICE_RESP[2]):.2f}× below it where the reference calls for {top_steepness(GRADE_T):.2f}×); and at the last slice the red state rotates partly away ({float(PRIMARY_SLICE_RESP[4, REDNESS_ORDER[0]]):.2f}) as it comes to encode the mix.

Averaged over layers — steep start, saturated middle, rotated end — that is a curve with a compressed top ({top_steepness(PRIMARY_SLICE_RESP.mean(axis=0)):.2f}× overall): the flattening is the readout saturating. Near the pole there is no headroom, so colors pulled often enough land at the same ceiling, and whatever keeps them distinguishable for the task lives in directions orthogonal to the axis — which in $d = 64$ costs almost no cosine. That is the shape to expect of a strongly anchored top of the scale, and the graded part of the response lives below it.
"""
