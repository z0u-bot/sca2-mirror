# title: Ex 2.1.9: Softmin sequence pooling
...
# The main results are loaded here, ahead of every prose section, so the Findings summary
# just below can quote the same computed numbers the rest of the report reads later.
res = load_results()
if res is None:
...
P = ex.PRIMARY
ctrl = acc("lam0").mean()
gain = m_span(P).mean() - m_span(ex.MEAN_ARM).mean()
headroom = m_span(ex.ORACLE_ARM).mean() - m_span(ex.MEAN_ARM).mean()
drop = a_span(ex.MEAN_ARM).mean() - a_span(P).mean()
ret = retention(P)
assert ret is not None

rf"""
**H1 (task cost) — holds.** The largest `named_holdout` exact-match gap from the control across all five anchored conditions is {max(abs(acc(c).mean() - ctrl) for c in ["span-mean", *ex.POOLED_NAMES, ex.ORACLE_ARM]):.4f}, against a gate of {ex.TASK_GATE:g}.

**H2 (pooling increases selectivity) — unmet.** On the primary condition (`{P}`):
(a) the margin gain over the span mean is {gain:+.3f} against a gate of {ex.POOL_GAIN_GATE:g} (partial {ex.POOL_GAIN_PARTIAL:g}), and
(b) the syntax-role drift falls by {drop:.3f} against a required {ex.SYNTAX_DRIFT_GATE:g} — the predicted direction on both, short of both gates, and neither partial applies. One piece of context, unpacked in the selectivity section: the op1 oracle itself beats the span mean by only {headroom:.3f} on $m_\text{{span}}$ here, so H2(a)'s gate was out of reach for any pull; `{P}` recovered {gain / headroom:.0%} of the headroom that existed.

**H3 (the operating point survives pooling) — holds.** On `{P}`: containment $\bar\alpha$ = {a_op1(P).mean():.3f} (gate ≤ {ex.MEAN_ALIGN_GATE:g}); retention {ret:.2f} (gate ≥ {ex.RETENTION_GATE:g}, minimum across seeds); and grading $r^2$ = {grading_r2(P):.2f} against the span mean's {grading_r2(ex.MEAN_ARM):.2f} — the gate tolerated a drop of {ex.GRADE_R2_DROP:g} and instead the response got better graded.

**H4 (the weight lands where the concept lives) — holds.** At the embedding slice `{P}` puts {pi_bar(P)[0, 0]:.2f} of the softmin weight on op1 (gate ≥ {ex.LEAD_GATE:g}; uniform is 0.25) — the only span position whose state can carry the concept there — and the weighted readability gain is {read_gain(P):+.2f} (gate ≥ {ex.READ_GAIN_GATE:g}). At depth the concentration is winner-take-all seed by seed: at the two sharper τ some seeds latch onto `+` in the first few epochs, and only τ = 0.1 keeps a graded profile on op1 in every seed (*Where the weight went*).
...
(ᾱ = {ex.EX218_REFERENCE["end90-hold30"]["alpha_op1"]:.2f}) while keeping the margin ({ex.EX218_REFERENCE["end90-hold30"]["m_op1"]:.2f}).
...
the cube-wide drift is there too. ᾱ measured at op1 is {ex.EX218_REFERENCE["end90-hold30"]["alpha_op1"]:.2f}, but the
same statistic maxed over the span roles is {ex.EX218_REFERENCE["end90-hold30"]["alpha_span"]:.2f}.[^aspan] The pooled
...
# The diagram is a static SVG beside this script; the publisher copies it into the bundle's assets.
pooling_svg = Path(__file__).parent / "public" / "pooling.svg"
pooling_url = pub.asset_url(pooling_svg, name="pooling.svg") if (pub := current_publisher()) else str(pooling_svg)
figure_html(
    f'<img src="{pooling_url}" alt="A training batch drawn as a cube: mellowmax pools within each line\'s span at every slice, then plain means collapse lines, slices, and batch rows.">',
...
# Joined with the block's own indent so every row dedents evenly under the table.
rows = "\n    ".join(
...
e218 = load_ex218()
if e218 is None:
    stop("_Ex-2.1.8's published results aren't reachable from here; the calibration figure needs them._")
assert e218 is not None
metrics218, arrays218 = e218
...
taus = np.geomspace(0.003, 1.0, 60)
lead = {c: np.array([leading_weight(a, t) for t in taus]) for c, a in ALPHA218.items()}
...
def tau_calibration_plot() -> plt.Figure:
    ...
    for ax, (cond, title) in zip(
        ...
        ax.axhspan(*ex.LEAD_BAND, color=grey, alpha=0.15, lw=0)
        for li in range(5):
            ax.plot(taus, lead[cond][:, li], color=slice_colors[li], lw=1.4, label=f"slice {li}")
        for t in ex.TAUS:
            ax.axvline(t, color=grey, ls=(0, (4, 3)), lw=0.8, alpha=0.7)
        ax.axhline(1 / ex.SPAN, color=grey, lw=0.5, alpha=0.45, zorder=-1)
        ax.set_xscale("log")
        ax.set_xlabel("τ", fontsize="x-small")
        ax.set_title(title, fontsize="small", color=grey)
...
tau_calibration_plot()

# %%

x0 = np.einsum("ct,c->t", 1.0 - ALPHA218["end90-hold30"][0, :, : ex.SPAN], ex.LABEL_W)


def seed_mean(stat, cond: str) -> float:
...
m218 = seed_mean(lambda a: ex.pooled_margin(a, ex.LABEL_W), "end90-hold30")
m218c = seed_mean(lambda a: ex.pooled_margin(a, ex.LABEL_W), "lam0")
as218 = seed_mean(ex.alpha_span, "end90-hold30")
as218c = seed_mean(ex.alpha_span, "lam0")
...
The ex-2.1.8 arrays give the reproduction targets for the `span-mean` condition, as means of per-run statistics — the convention the results are scored under: pooled margin m_span = {m218:.2f} (control {m218c:.2f}) and ᾱ_span = {as218:.2f} (control {as218c:.2f}). They also put numbers on the syntax latch: at the embedding slice, the alignment $\cos(h, \hat v)$ of the span-pull run's states, averaged over colors with label-affinity weights, is `+` {1 - x0[1]:.2f} and `=` {1 - x0[3]:.2f}, against op1 {1 - x0[0]:.2f} and op2 {1 - x0[2]:.2f} — a pool applied to *this* profile would close on the syntax tokens.
...
fixed in advance as `{ex.PRIMARY}`: on both reference profiles its τ sits at
...
results prose calls it `{ex.PRIMARY}` or "the primary condition". The other
...
match within {ex.TASK_GATE:g} (absolute) of the seed mean of the in-experiment
...
(a) $m_\text{{span}}(P) - m_\text{{span}}(\text{{span-mean}}) \ge {ex.POOL_GAIN_GATE:g}$,
about two thirds of what the op1 oracle gave at op1 in ex-2.1.7
({ex.EX217_OP1_GAIN:g}), and roughly 3× the noise of a difference of three-seed
means.
(b) The syntax-role drift falls: $\bar\alpha_\text{{span}}(P) \le
\bar\alpha_\text{{span}}(\text{{span-mean}}) - {ex.SYNTAX_DRIFT_GATE:g}$.
Partial: (b) holds and (a) lands in $[{ex.POOL_GAIN_PARTIAL:g}, {ex.POOL_GAIN_GATE:g})$; or exactly one
...
(a) containment, $\bar\alpha \le {ex.MEAN_ALIGN_GATE:g}$ at op1;
(b) retention, every run whose running maximum of $m_\text{{span}}$ reaches
{ex.RETENTION_FLOOR:g} ending at $\ge {ex.RETENTION_GATE:g}\times$ that maximum (quoted as the minimum
across seeds);
(c) grading, the squared Pearson correlation $r^2$ against `sim¹·⁵` of
P's seed-mean op1 response no more than {ex.GRADE_R2_DROP:g} below the `span-mean`
...
$\ge {ex.LEAD_GATE:g}$ (uniform is 0.25; this gates the *trained* weights, where
...
slices — is at least {ex.READ_GAIN_GATE:g}.
Partial: (a) holds and (b) lands in $[0, {ex.READ_GAIN_GATE:g})$.
...
and the weight side here. *Uniform*: no slice reaches {ex.LEAD_GATE:g} in any
...
CTRL = float(acc("lam0").mean())


def h1_cell(cond: str, set_name: str, key: str, fmt: str = "{:.3f}") -> str:
...
h1_rows = "".join(
    ...
    f"<td class='num'>{h1_cell(c, 'named_seen', 'accuracy')}</td>"
    f"<td class='num'>{h1_cell(c, 'named_holdout', 'accuracy')}</td>"
    f"<td class='num'>{h1_cell(c, 'named_holdout', 'nll')}</td>"
    f"<td class='num'>{h1_cell(c, 'open', 'guess_dist')}</td>"
    f"<td class='num'>{acc(c).mean() - CTRL:+.4f}</td>"
    f"<td>{'clean' if abs(acc(c).mean() - CTRL) <= ex.TASK_GATE else 'not clean'}</td></tr>"
    for c in CONDS
)
h1_table = f"""
...
"""
figure_html(h1_table, caption=h1_caption, class_="report-figure")

# %%

worst = max((c for c in CONDS if c != "lam0"), key=lambda c: abs(acc(c).mean() - CTRL))

rf"""
**H1 holds.** The largest gap from the control is {abs(acc(worst).mean() - CTRL):.4f}, at `{worst}`, against a gate of {ex.TASK_GATE:g}. Every condition solves the task, so H2–H4 are all scored. Concentrating the whole pull budget of a line onto one position — which at τ = 0.01 is nearly a hard min — costs nothing the task loss can see, consistent with every anchored experiment on this testbed so far.
...
# no section scored it; folded it into the paragraph above, where its targets
...
stats = {"m_span": m_span, "alpha_span": a_span, "m_op1": m_op1, "alpha_op1": a_op1}
names = {"m_span": "m<sub>span</sub>", "alpha_span": "ᾱ<sub>span</sub>", "m_op1": "m<sub>op1</sub>", "alpha_op1": "ᾱ (op1)"}  # fmt: skip


def h2_repro_row(cond: str, ref_key: str) -> str:
...
h2_repro_head = "".join(
    f"<th class='num'>{names[k]}</th><th class='num'>ex-2.1.8</th><th class='num'>Δ</th>" for k in stats
)
h2_repro_table = f"""
<table class="report-table">
  <thead><tr><th>condition</th>{h2_repro_head}</tr></thead>
  <tbody>{h2_repro_row("span-mean", "end90-hold30")}{h2_repro_row("lam0", "lam0")}</tbody>
</table>
"""
h2_repro_caption = """
...
"""
figure_html(h2_repro_table, caption=h2_repro_caption, class_="report-figure")

# %%

dm = m_span("span-mean").mean() - ex.EX218_REFERENCE["end90-hold30"]["m_span"]
da = a_span("span-mean").mean() - ex.EX218_REFERENCE["end90-hold30"]["alpha_span"]

rf"""
The largest differences — {dm:+.3f} on $m_\text{{span}}$ and {da:+.3f} on ᾱ_span — sit within a seed spread of the targets, and the same is true of the control. So the per-line mean-of-means (which differs from ex-2.1.8's flat mean only on crop-truncated lines) reproduces the operating point, and the pooled conditions can be read against `span-mean` as a like-for-like baseline.
"""

CTRL_M = float(m_span("lam0").mean())
CTRL_A = float(a_span("lam0").mean())
# Control-subtracted per-seed values, computed once — @themed draws twice.
M = {c: m_span(c) - CTRL_M for c in ANCHORED}
A = {c: a_span(c) - CTRL_A for c in ANCHORED}
GAIN_AT = float(M["span-mean"].mean())
DRIFT_AT = float(A["span-mean"].mean())
...
def h2_dots_plot() -> plt.Figure:
    # Reads top-to-bottom by effective τ: span mean (∞), the sweep, the oracle.
    rows = [ex.ORACLE_ARM, *ex.POOLED_NAMES, ex.MEAN_ARM]
    ...
    for ax, data, title in zip(
        axes, (M, A), (r"$m_\mathrm{span}$ − control", "ᾱ$_\\mathrm{span}$ − control"), strict=True
    ):
        for yi, cond in enumerate(rows):
            v = data[cond]
            ax.plot(v, [yi] * len(v), "o", ms=3.5, color=ink[cond], alpha=0.55, mew=0)
            ax.plot(v.mean(), yi, "|", ms=18, color=ink[cond])
        ax.set_title(title, fontsize="small", color=grey)
        ax.set_yticks(range(len(rows)), rows, fontsize=8)
        ax.set_xlim(0, 0.82)
        ax.tick_params(axis="x", labelsize=8)
    for x, ls in ((GAIN_AT + ex.POOL_GAIN_GATE, (0, (4, 3))), (GAIN_AT + ex.POOL_GAIN_PARTIAL, (0, (1, 2)))):
        axes[0].axvline(x, color=gate, lw=0.9, ls=ls)
    axes[1].axvline(DRIFT_AT - ex.SYNTAX_DRIFT_GATE, color=gate, lw=0.9, ls=(0, (4, 3)))
    return fig
...
def h2_sweep_row(c: str) -> str:
    gain = m_span(c).mean() - m_span(ex.MEAN_ARM).mean()
    frac = f"{gain / headroom:.0%}" if c in ex.POOLED_NAMES else "—"
    ...
    return (
        ...
        f"<td class='num'>{m_span(c).mean() - CTRL_M:.3f}</td>"
...
<table class="report-table">
  ...
  <tbody>{"".join(h2_sweep_row(c) for c in ANCHORED)}</tbody>
</table>
"""
h2_sweep_caption = f"""
The τ sweep beside its references, seed means (± seed standard deviation where quoted). Only the primary condition (<strong>bold</strong>) is gated; "oracle frac" is the fraction of the span-mean-to-oracle gap recovered, whose denominator is {headroom:.3f}. m<sub>op1</sub> is carried for continuity with ex-2.1.7/8, and r² previews the grading track scored under H3.
"""
figure_html(h2_sweep_table, caption=h2_sweep_caption, class_="report-figure")

# %%


def h2_latched(cond: str) -> list[int]:
...
share = {c: pi_bar(c)[:, 0].mean() for c in ex.POOLED_NAMES}
l30 = h2_latched(P)[0]

rf"""
**H2 is unmet.** On the primary condition `{P}`, (a) the margin gain is {gain:+.3f} against a gate of {ex.POOL_GAIN_GATE:g} (partial {ex.POOL_GAIN_PARTIAL:g}), and (b) the syntax-role drift falls by {drop:.3f} against a required {ex.SYNTAX_DRIFT_GATE:g}. Both moved the way the mechanism predicts and neither reaches its gate, so neither partial applies.

Three things the gate arithmetic hides:

1. **The margin headroom collapsed before the sweep began.** The gate was sized from the op1 oracle's +{ex.EX217_OP1_GAIN:g} margin gain in ex-2.1.7 — measured at op1, under the old schedule. Here, scored as $m_\text{{span}}$ at the ex-2.1.8 operating point, the oracle's whole advantage over the span mean is {headroom:.3f}: less than half the gate. No condition, oracle included, could have passed H2(a) as preregistered. The primary recovered {gain / headroom:.0%} of the headroom that existed.

2. **Neither statistic is monotone in τ.** In the dot chart both panels walk from `span-mean` toward the oracle only as far as τ = 0.1, then step back: the sharper the pool, the closer the condition sits to the span mean again. What the statistics do track is op1's share of the pull budget — 0.25 for the span mean by construction, then {share["pool-t100"]:.2f} at τ = 0.1, {share[P]:.2f} at τ = 0.03, {share["pool-t010"]:.2f} at τ = 0.01 (mean over slices of the weight profiles under H4), and 1.0 for the oracle. The share peaks at τ = 0.1 because a sharper pool concentrates the budget harder but risks putting it on the wrong role: at depth, each seed of the two sharper rungs commits everything to a single role, and at τ = 0.01 two seeds of three commit to `+` (at τ = 0.03, one; at τ = 0.1, none). The same decomposition holds within a condition — `{P}`'s committed-to-`+` seed (s{l30}) scores $m_\text{{span}}$ {m_span(P)[l30]:.2f} and ᾱ_span {a_span(P)[l30]:.2f} where its op1-keeping seeds score ≈{np.mean(np.delete(m_span(P), l30)):.2f} and ≈{np.mean(np.delete(a_span(P), l30)):.2f}. So the finite-τ means differ mostly through how many seeds latched, and the sweep's message is a latch-rate trend rather than a smooth dose response; with three seeds per rung the rates themselves are coarse, so carrying a τ forward should rest on the latch mechanism (*Where the weight went*), with more seeds at the chosen rung in the next design.
...
CONDS3 = [ex.MEAN_ARM, ex.PRIMARY, ex.ORACLE_ARM]
prof = {}
for c in CONDS3:
    a = alpha_map(c)
    m = np.einsum("c,lct->lt", ex.LABEL_W, a) - a.mean(axis=1)
    prof[c] = {"margin": m[:, : ex.SPAN], "drift": a[:, :, : ex.SPAN].mean(axis=1)}


@memo
def role_profiles_plot(prof: dict) -> str:
    @themed(
        name="role-profiles",
        alt_text="""
...
def position_profiles_plot(prof: dict) -> str:
    ...
    def plot() -> plt.Figure:
        ...
        x = np.arange(ex.SPAN)
        margin_ink = light_dark("#b4531f", "#dd8f5f")
        drift_ink = light_dark("#1f6fb4", "#5fa8dd")
        grey = light_dark("#666", "#999")
        ...
        for col, cond in enumerate(CONDS3):
            for row in range(5):
                sl = 4 - row  # last slice on top, the embedding at the bottom
                ax = axes[row][col]
                smooth_step_area(ax, x, prof[cond]["margin"][sl], ramp=0.5, color=margin_ink,
                                 alpha=light_dark(0.25, 0.3))  # fmt: skip
                smooth_step(ax, x, prof[cond]["margin"][sl], ramp=0.5, color=margin_ink, lw=1.2)
                smooth_step(ax, x, prof[cond]["drift"][sl], ramp=0.5, color=drift_ink, lw=1.5)
            ...
            axes[4][col].set_xticks(x, ex.ROLES)
        ...
        m_key = plt.Rectangle((0, 0), 1, 1, fc=margin_ink, alpha=0.35, ec="none")
        d_key = plt.Line2D([], [], color=drift_ink, lw=1.5)
        fig.legend([m_key, d_key], [r"margin $m(\ell, t)$", r"drift $\bar\alpha(\ell, t)$"],
                   fontsize=8.5, frameon=False, ncols=2, loc="lower center", bbox_to_anchor=(0.5, 1.0),
                   labelcolor=grey)  # fmt: skip
        return fig

    return plot()


position_profiles_plot(prof)

# %%

emb = {c: alpha_map(c)[0, :, : ex.SPAN].mean(axis=0) for c in [ex.MEAN_ARM, ex.PRIMARY, ex.ORACLE_ARM]}

rf"""
The drift survives pooling only at the `+` role. At the embedding, the primary's pooling released `=` almost entirely (drift {emb[ex.MEAN_ARM][3]:.2f} under the span mean → {emb[ex.PRIMARY][3]:.2f}) but left {emb[ex.PRIMARY][1]:.2f} on `+`, barely below the span mean's {emb[ex.MEAN_ARM][1]:.2f} — consistent with the ~0.2 of softmin weight the pull still spends there (H4 below). The op1 oracle complicates that account: it never pulls `+` at any slice, and its op1 states cannot even read the `+` embedding (the first position attends only to itself), yet its `+` embedding still ends at {emb[ex.ORACLE_ARM][1]:.2f}. So over half of the `+` drift is paid for by something other than the pull — and whatever pays for it does so against the repulsion, which runs from the first epoch and still holds at the end. The trajectory records no per-role drift, so this run can't say whether that alignment climbs early (against the repulsion's peak) or late (as the anneal decays); the candidate mechanism and the timing are both queued as exploratory questions rather than interpreted here.
...
KEYS = ("alpha_op1", "m_span")
# Seed means once, in the cell: @themed calls the plot function twice.
MEAN = {c: {k: np.mean([traj(c, s, k) for s in ex.SEEDS], axis=0) for k in KEYS} for c in [*ANCHORED, "lam0"]}
EPOCHS = np.mean([traj(ex.PRIMARY, s, "epoch") for s in ex.SEEDS], axis=0)
RET = {c: retention(c) for c in ANCHORED}
...
SE = np.linspace(0.0, float(ex.SCHEDULER.epochs), 601)
ANCHOR = ex.anchor_weight(SE)
ANTI = ex.anti_subspace_weight(SE, anneal_end=ex.ANTI_ANNEAL_END, hold_ratio=ex.ANTI_HOLD_RATIO)
GRID = [["span-mean", "pool-t010", "pool-t030"], ["pool-t100", "op1-oracle", "sched"]]
LEFT = ("span-mean", "pool-t100")
...
def trajectories_plot() -> plt.Figure:
    ...
    dash = (0, (6, 1))
    top = max(np.max(MEAN[c]["m_span"]) for c in ANCHORED)
    for cond in ANCHORED:
        ...
        ax.plot(1, ex.RETENTION_FLOOR, marker=8, ms=3, color=gate, clip_on=False, zorder=6,
                transform=ax.get_yaxis_transform())  # fmt: skip
        ax.plot(EPOCHS, MEAN["lam0"]["alpha_op1"], color=grey, lw=1.1, zorder=3)
        ax.plot(EPOCHS, MEAN["lam0"]["m_span"], color=grey, lw=1.1, ls=dash, zorder=3)
        ax.plot(EPOCHS, MEAN[cond]["m_span"], color=ink, lw=1.4, ls=dash, zorder=4)
        ax.plot(EPOCHS, MEAN[cond]["alpha_op1"], color=ink, lw=1.7, zorder=5)
        r = RET[cond]
        assert r is not None
        ax.annotate(f"{r:.2f}", (0.97, 0.6), xycoords="axes fraction", ha="right", fontsize=8,
                    color=light_dark("#444", "#bbb"))  # fmt: skip
        ax.set_title("pool-t030 (primary)" if cond == ex.PRIMARY else cond, fontsize=9.5, color=ink)
        ax.set_xlim(0, ex.SCHEDULER.epochs)
        ax.set_ylim(-0.08, top * 1.08)
        ax.tick_params(labelbottom=cond in GRID[1], labelleft=cond in LEFT, labelsize=8)
        ax.set_ylabel("cosine", fontsize="x-small") if cond in LEFT else None
        ax.set_xlabel("epoch", fontsize="x-small") if cond in GRID[1] else None
    axsched = axd["sched"]
    axsched.plot(SE, ANCHOR, color=term["anchor"], lw=1.4)
    axsched.plot(SE, ANTI, color=term["anti"], lw=1.4)
    axsched.set_yscale("log")
    axsched.set_xlim(0, ex.SCHEDULER.epochs)
    axsched.set_ylim(2e-4, 4)
    axsched.set_yticks([1e-3, 1e-2, 1e-1, 1e0])
    axsched.minorticks_off()
    axsched.tick_params(labelsize=7.5)
    axsched.set_xlabel("epoch", fontsize="x-small")
    axsched.set_title("weights (all anchored)", fontsize=9.5, color=grey)
    for x, y, txt, k in ((64, 0.13, r"$\lambda_\mathrm{a}$", "anchor"), (16, 0.5, r"$\lambda_{\bar{\mathrm{s}}}$", "anti")):  # fmt: skip
        axsched.annotate(txt, (x, y), fontsize=9, color=term[k], va="bottom")
    fig.legend(
...
ret = retention(P)
assert ret is not None
worst = min(r for c in ANCHORED if (r := retention(c)) is not None)

rf"""
**H3(a) and (b) hold.** On the primary condition, $\bar\alpha$ at op1 ends at {a_op1(P).mean():.3f} <span class='range'>±{a_op1(P).std(ddof=1):.3f}</span> against the {ex.MEAN_ALIGN_GATE:g} gate, and retention is {ret:.2f} against {ex.RETENTION_GATE:g}. The contrary outcome — a concentrated pull sliding once the anti anneal ends at epoch {ex.ANTI_ANNEAL_END:g} — shows up only as the small end-of-training dip every condition shares (the anchor's own anneal), and the worst retention anywhere in the sweep is {worst:.2f}. Pooling redirects the budget of the pull without destabilizing what ex-2.1.8 found.
"""

ALL = ["lam0", *ANCHORED]
RESP = {c: alpha_map(c)[:, :, 0].mean(axis=0) for c in ALL}
...
def level_mean(y: np.ndarray) -> tuple[np.ndarray, np.ndarray]:
...
CTRL_LEVELS = level_mean(RESP["lam0"])
GRID2 = [ALL[:3], ALL[3:]]
...
def grading_plot() -> plt.Figure:
    ...
    axes = cast(AxesGrid, axes)
    for row, conds in zip(axes, GRID2, strict=True):
        for ax, cond in zip(row, conds, strict=True):
            a = RESP[cond]
            ax.axhline(0, color=light_dark("#bbb", "#555"), lw=0.8, zorder=0)
            ax.plot(*CTRL_LEVELS, color=light_dark("#999", "#777"), lw=3, alpha=0.5, zorder=1,
            ...
            if cond != "lam0":
                ax.plot(*level_mean(fitted_target(a)), color=light_dark("#888", "#888"),
                        lw=1.0, ls=(0, (4, 2)), zorder=2)  # fmt: skip
                ax.plot(*level_mean(a), color=light_dark("#222", "#eee"), lw=1.2, zorder=4)
...
PI = {c: pi_bar(c) for c in ex.POOLED_NAMES}
PI_SEEDS = {c: np.array([np.asarray(cells[f"{c}-s{s}"]["pi"], dtype=float) for s in ex.SEEDS]) for c in ex.POOLED_NAMES}  # fmt: skip
WGAIN = {c: read_gain(c) for c in ex.POOLED_NAMES}
...
def weight_profiles_plot() -> plt.Figure:
    ...
    for col, cond in enumerate(ex.POOLED_NAMES):
        ...
        for row in range(5):
            sl = 4 - row  # last slice on top, the embedding at the bottom
            ax = axes[row][col]
            smooth_step_area(ax, x, PI[cond][sl], ramp=0.5, color=ink, alpha=light_dark(0.22, 0.28))
            smooth_step(ax, x, PI[cond][sl], ramp=0.5, color=ink, lw=1.5)
            for seed_prof in PI_SEEDS[cond][:, sl]:
                smooth_step(ax, x, seed_prof, ramp=0.5, color=hair, lw=0.5)
            smooth_step(ax, x, READ[sl], ramp=0.5, color=read_ink, lw=1.0, ls=(0, (4, 2)))
        ...
        axes[0][col].annotate(f"gain {WGAIN[cond]:+.2f}", (0.97, 0.88), xycoords="axes fraction", ha="right",
                              va="top", fontsize=8, color=light_dark("#444", "#bbb"))  # fmt: skip
        # The H4(a) gate reads exactly here: op1's weight at the embedding.
        axes[4][col].plot(0, ex.LEAD_GATE, marker=9, ms=3, color=gate, clip_on=False, zorder=6,
...
PI_T = {c: [traj(c, s, "pi") for s in ex.SEEDS] for c in ex.POOLED_NAMES}
EPOCHS = np.mean([traj(ex.PRIMARY, s, "epoch") for s in ex.SEEDS], axis=0)
...
def concentration_timing_plot() -> plt.Figure:
    ink = {
        "pool-t010": light_dark("#7fb3d8", "#4f7ea6"),
        "pool-t030": light_dark("#1f6fb4", "#5fa8dd"),
        "pool-t100": light_dark("#12497c", "#8ec4ea"),
    ...
    for ax, sl, title in zip(axes, (0, 4), ("op1 weight at the embedding", "op1 weight at slice 4"), strict=True):
        ax.axvspan(0, ex.SCHEDULER.warmup_epochs, color=grey, alpha=0.12, lw=0)
        ax.axhline(1 / ex.SPAN, color=grey, lw=0.6, alpha=0.5, zorder=0)
        for cond, c_ink in ink.items():
            for si, pi_t in enumerate(PI_T[cond]):
                ax.plot(EPOCHS, pi_t[:, sl, 0], color=c_ink, lw=1.1, alpha=0.9,
    ...
    axes[1].legend(fontsize=8, frameon=False, loc="center right", labelcolor=grey)
    return fig


concentration_timing_plot()
...
def shelf_level_mean(y: np.ndarray) -> np.ndarray:
    return np.array([y[np.round(ex.REDNESS, 6) == lv].mean() for lv in levels])


def shelf_resp(cond: str, s: int) -> np.ndarray:
    """One run's per-color op1 response: the mean over slices of alignment at op1."""
    return arrays[f"{cond}-s{s}/alpha"][:, :, 0].mean(axis=0)


def kept_op1(cond: str, s: int) -> bool:
...
def grading_by_seed_plot() -> plt.Figure:
    ink = {True: light_dark("#c1332a", "#f0665a"), False: light_dark("#1f6fb4", "#5fa8dd")}
    grey = light_dark("#666", "#999")
    ...
    for ax, cond in zip(axes, ex.POOLED_NAMES, strict=True):
        ax.axhline(0, color=light_dark("#bbb", "#555"), lw=0.8, zorder=0)
        for s in ex.SEEDS:
            ax.plot(levels, shelf_level_mean(shelf_resp(cond, s)), color=ink[kept_op1(cond, s)], lw=1.3)
        ax.set_title("pool-t030 (primary)" if cond == ex.PRIMARY else cond, fontsize=9.5)
        ax.set_xlabel("redness of op1", fontsize="x-small")
        ax.tick_params(labelsize=8)
    axes[0].set_ylabel(r"$\alpha_c$ at op1", fontsize="x-small")
    keys = [plt.Line2D([], [], color=ink[k], lw=1.3) for k in (True, False)]
    axes[2].legend(keys, ["deep pull kept op1", "latched onto +"], fontsize=8, frameon=False,
                   loc="upper left", labelcolor=grey)  # fmt: skip
    return fig


grading_by_seed_plot()

# %%


def resp_at(cond: str, s: int, level: float) -> float:
    sel = np.round(ex.REDNESS, 2) == level
    return float(arrays[f"{cond}-s{s}/alpha"][:, sel, 0].mean())


latched = [s for s in ex.SEEDS if np.asarray(cells[f"pool-t010-s{s}"]["pi"], dtype=float)[4, 0] < 0.5]
kept = [s for s in ex.SEEDS if s not in latched][0]
hi = np.mean([resp_at("pool-t010", s, 0.8) for s in latched])
lo = np.mean([resp_at("pool-t010", s, 0.72) for s in latched])

rf"""
The shelf belongs to the latched runs. In the runs whose deep slices kept op1, the response grades smoothly with redness at every τ. In the runs that latched onto `+`, it holds up only at the red end and drops sharply below redness 0.8 — `pool-t010`'s two latched runs average {hi:.2f} at redness 0.8 and {lo:.2f} at 0.72, where its op1-keeping run reads {resp_at("pool-t010", kept, 0.72):.2f} — which is also where the label affinity falls away steeply. That reading makes sense of the mechanism: in a latched run only the embedding slice still pulls op1, so a color keeps its alignment only to the extent that its lines keep getting labeled, and the graded contribution the deep slices add in healthy runs is missing. The seed-mean scatter under H3(c) blends the two shapes, which is what the `pool-t010` panel's shelf is.
"""
...
R = {c: R_SEEDS[c].mean(axis=0) for c in ANCHORED}
x = np.arange(ex.SPAN)
...
def readability_profiles_plot() -> plt.Figure:
    from matplotlib.layout_engine import ConstrainedLayoutEngine

    from mini.vis import smooth_step

    own_ink = light_dark("#b4531f", "#dd8f5f")
    read_ink = light_dark("#555", "#aaa")
    hair = light_dark("#00000040", "#ffffff40")
    grey = light_dark("#666", "#999")
    ...
    for col, cond in enumerate(ANCHORED):
        for row in range(5):
            sl = 4 - row  # last slice on top, the embedding at the bottom
            ax = axes[row][col]
            for seed_prof in R_SEEDS[cond][:, sl]:
                smooth_step(ax, x, seed_prof, ramp=0.5, color=hair, lw=0.5)
            smooth_step(ax, x, R[cond][sl], ramp=0.5, color=own_ink, lw=1.5)
            smooth_step(ax, x, READ[sl], ramp=0.5, color=read_ink, lw=1.0, ls=(0, (4, 2)))
            ax.set(ylim=(-0.08, 1.08), xlim=(-0.4, ex.SPAN - 0.6), yticks=[0.0, 0.5, 1.0])
            ax.spines[:].set_visible(False)
            ax.grid(axis="y", which="major", c="#888", alpha=0.2)
    ...
    fig.supylabel("slice", fontsize=9)
    own_key = plt.Line2D([], [], color=own_ink, lw=1.5)
    read_key = plt.Line2D([], [], color=read_ink, lw=1.0, ls=(0, (4, 2)))
    fig.legend([own_key, read_key], ["own readability $R^2$ (redness)", "control readability $R^2$"],
               fontsize=8.5, frameon=False, ncols=2, loc="lower center", bbox_to_anchor=(0.5, 1.0),
               labelcolor=grey)  # fmt: skip
    return fig


readability_profiles_plot()

# %%

op1_min = min(R[c][:, 0].min() for c in ANCHORED)
syn = np.array([R[c][1:, [1, 3]] for c in ANCHORED])  # `+` and `=` past the embedding
latched10 = [s for s in ex.SEEDS if np.asarray(cells[f"pool-t010-s{s}"]["pi"], dtype=float)[4, 0] < 0.5]
latched_op1 = np.mean([np.asarray(metrics["readability"][f"pool-t010-s{s}"], dtype=float)[4, 0] for s in latched10])

rf"""
Redness is readable almost everywhere in the anchored runs. In every anchored condition, op1 reads at $R^2$ ≥ {op1_min:.2f} at every slice, so the anchor axis itself now encodes redness. Past the embedding, `+` and `=` read at {syn.mean():.2f} on average and never below {syn.min():.2f}, much as in the control: attention copies the operand's color into each syntax role, whether or not anything pulls there.

So the weight profile is not proportional to readability, and it couldn't be: at depth π̄ is one-hot, while $R^2$ is high at three roles of four. The softmin allocates by *alignment cost* among positions nearly all of which could host the concept, which is part of why the latch is a race rather than a search. The latched runs of `pool-t010` still read op1 at {latched_op1:.2f} at the last slice: the pull wasn't blind there; it had found `+` cheaper first.

The influence also runs the other way: where the pull keeps spending, it *creates* readability. op2 is the one role the control barely reads, at {READ[3:, 2].mean():.2f} across the two deepest slices. The span mean never stops pulling op2, and there it reads {R["span-mean"][3:, 2].mean():.2f}; the primary condition's pool withdrew from op2, and there it reads {R["pool-t030"][3:, 2].mean():.2f}, back at the control's level.
