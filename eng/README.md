...
- [Environments: production, dev, and the backup](./environments.md) — why a storage profile replaces the pair and the Modal Environment and inherits the rest, why dev pins never enter `publish.lock`, and why the backup is a pull from a trust domain the development tokens cannot reach.
