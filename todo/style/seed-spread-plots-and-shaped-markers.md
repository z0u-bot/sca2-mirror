...
**Shape.** Conditions are told apart by colour alone. On paper `control` and `handover` are two greys. Ex-2.2.9 gives each condition a marker shape (`marker(cond)` in its report) and a legend; that mapping belongs in the shared palette so the same condition has the same shape across reports, the way `ink` gives it the same colour. The shapes aren't locked in, though, and could be changed — e.g. we could use `H` for handover, etc.
