...
**2026-09-15, housekeeping** — The trial exists: the ex-2.2.9 prereg drafted in [PR #176](https://github.com/z0u/sca2/pull/176) (open at the time of writing) is written in the short form throughout. Each H section is a question a colleague could restate — "Does the model still learn the task?", "Can we still take *red* out cleanly?" — and opens with a bold **Prediction.** and a sentence on why we expect it to hold, citing ex-2.2.5 and ex-2.2.7. Hard gates survive only where a decision hangs on them (the task gate in H1, the removal gate in H3); H3 then says plainly that "two further reads have an expected direction and no gate", which is the distinction this item was after. There is a Glossary section that glosses each statistic in a phrase.

So the remaining half of this item is the cheap one: the science skill's example block still shows the long form, and ex-2.2.9's H1 or H3 is the short-form example to lift into it. Worth doing once #176 merges, not before — editing the skill against a draft that is still under review invites doing it twice. Closing this item is then a matter of confirming the prereg survived review in that form.
