...
**2026-09-14, Fable** — the survey was re-run on production (84 operators after dropping the Bézier family), keeping the `t00` rows. The prices above reproduce to the digits quoted: whole-sequence edits 0.32 to 0.95, the projection 0.62, `operands` at 0.078 red accuracy and 0.024 deficit, `shaped-a0.1-p0-operands` at 0.071.

**2026-09-23, Fable, housekeeping** — closed. The anchored-op prereg ([ex-2.2.14](/docs/m2/ex-2.2.14/report.py)) inherits `recipe-short` at λ_a 0.1 (ex-2.2.11's `handover`, confirmed by ex-2.2.13), not `t00`: the post hoc read in ex-2.2.3 and the twenty-seed E6 comparison narrowed the choice to the recipe, and every experiment since has built on it. The prices above stay as the record of what `t00` would have cost.
