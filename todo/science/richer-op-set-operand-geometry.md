...
**2026-09-08, ex-2.2.3 prereg** — runs as exploratory row E4 of [ex-2.2.3](/docs/m2/ex-2.2.3/report.py): un-anchored models at one, two, and four ops (the four-op point is that experiment's control), each on 200k lines at the recipe's step count, probed with ex-2.1.12's cube probes. The line count is held fixed rather than lines per op, so a better cube at four ops is a clean positive and a worse one is confounded with lines per op; the prereg says so. Stays open until E4 is read.
