...
**2026-09-08, ex-2.2.3 prereg** — the op table is now specified in [ex-2.2.3's design module](/docs/m2/ex-2.2.3/experiment.py) as `mix`, saturating `add`, `lighten` (per-channel max), and `darken` (per-channel min), and the design's deps section says why: on the six-level `v216` grid `screen` is closed only where a channel is 0 or 15, where it equals `add`, and `multiply` only where it is 0 or the identity, so neither adds a line another op does not teach (`closure()` and `agreement()` in that module). The ops as words, the `op` field, (op, pair) bookkeeping, and the kept infix frame are the prerequisite that prereg's method names; implement to that module's `Op` table so the report's rendered counts and the corpus agree. The relevance distributions the 09-01 note asked for render live in the prereg's method.
