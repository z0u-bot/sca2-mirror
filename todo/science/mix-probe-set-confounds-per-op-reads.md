...
**2026-09-10, Claude** — one more number for the case: by E8's one-level dependence rule, `mix` reads 0.61 on its own on-grid probe set and 0.49 on the shared partner draw ([ex-2.2.4](/docs/m2/ex-2.2.4/report.py)). So the E8 comparison of `mix` against the other ops was also across partner sets, and the seventh probe set would fix that read too.
