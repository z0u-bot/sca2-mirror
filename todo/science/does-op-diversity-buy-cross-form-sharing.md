...
**2026-09-08, ex-2.2.3 prereg** — not folded into ex-2.2.3. Its richer-op arm (E4) runs on the word grammar only, and this item needs the ex-2.1.5 two-form corpus on top of the op table. With a four-op table (`mix`, `add`, `lighten`, `darken`) the diversity axis has three points, 1, 2, and 4, which is the coarse sampling the item already anticipates. Deferred until the op table has landed and a two-form multi-op corpus is cheap to build.
