...
- `mo.md(rf"…{x:.2f}…")` becomes `{{ "%.2f"|format(x) }}` in the prose. Joined lists and ranges read better precomputed as strings in the cell (`e1_ctrl`, `ctrl_spread`) than as Jinja expressions.
- One namespace, so Marimo's cell-private `_x` names go, and loop variables that several cells reuse need distinct names (`c_`, `t_`).
...
- **Publishing.** `./go publish`, `export_reports.py`, `unpublished_reports.py` (`is_marimo_notebook`), and `build_site.py` know Marimo notebooks. A literate document weaves to the same bundle shape (`index.html` + `_assets/`), so the export step is the join: pin it in `publish.lock`, give it the banner (`set_banner`: index link and source link), thumbnails and the `mini:figures` strip, rewrite `../d2.1/report.py` links to `../d2.1/`, and print the PDF through `report.css`'s `@media print` rules (the lit print is on a default page size; `mini.report_print`'s fit-to-section pass is the other half).
