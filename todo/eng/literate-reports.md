...
opened: 2026-09-18
---
# Reports as literate documents (`mini.lit`) instead of Marimo notebooks

mi-ni#86 prototyped `mini.lit`: a plain `.py` whose top-level strings are prose (a Jinja template over the cells' namespace) and whose code between them is a cell, with a `@memo` cache for the slow parts, `stop()` for a preregistration, and one self-styled HTML page. The question here is whether our reports should move to it. [Markdown publishing](./markdown-publishing.md) wants the same thing from the other end (a plain Markdown face for every report, and to drop the Marimo-native export); a literate document weaves that Markdown as a by-product.
...
- `mo.md(rf"…{x:.2f}…")` becomes `rf"""…{x:.2f}…"""` at the top level: an f-string paragraph is evaluated as Python (added during the trial; a plain string is still a Jinja template). The names it reads are real references, so ruff and ty check them and vulture stops reporting every prose-only variable as unused, which it did for 21 names in the Jinja spelling. Joined lists still read better precomputed as strings in the cell (`e1_ctrl`, `ctrl_spread`).
