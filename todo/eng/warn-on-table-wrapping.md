...
**2026-09-22, Fable** — Done in `mini.report_print`: after the print, `wrapped_cells` lays the page out under print media in a viewport as wide as the sheet's content area and counts, per table, the header cells that wrap onto a second line and the body cells that wrap onto a third; `print_page` logs one warning per table naming it (by caption, else header row) with its column count. It runs wherever the PDF is printed (`./go preview`, the site build), so the warning shows in the build log. Not yet surfaced in the render or the live server, which do not lay the page out at print width.
