...
(
    ...
    echo "This is not an environment the human has direct access to, so if you are working on an experiment, *always* publish the report before pushing. Otherwise the human won't be able to see the rendered report and its figures, which hampers collaboration. When referring to figures in conversation you should share them with 'SendUserFile', or the 'Read' tool if you also want to see it too. The human reviews drafts on a reMarkable: after './go preview --no-serve <report>', send them '_site/<key>/report.pdf' the same way. From the second round on, add '--since <commit>' naming the commit the last PDF was printed from (a marked PDF names it at the top of its first page), and the PDF bars every line changed since."
    echo
