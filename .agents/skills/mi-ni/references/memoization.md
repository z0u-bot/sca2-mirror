...
- Module-level values reached as attributes (`ex.GATE`, where `ex` is an imported project module) and NumPy array globals — not tracked for *tasks*: a task that reads them re-runs only on a code edit, so pass them as inputs. The report cache (`mini.lit.memo`) does track both (`mini.memo.reachable_values`), hashing arrays by content, and warns about a global it cannot fingerprint (a dict of arrays, a project-typed object). The two differ on purpose: a spurious figure redraw costs a second, a spurious task re-run is a training run, so the task walk changes only with a deliberate re-run budget.
